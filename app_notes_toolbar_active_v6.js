...
function getNotesInlineState(){
  const node = getNotesSelectionAnchorElement();
  ...
  return {
    bold: fwNum >= 600,
    italic: String(cs?.fontStyle || '').toLowerCase() === 'italic',
...
if(el.capoInput){
  ...
  if(el.notesToolbar){
    el.notesToolbar.querySelectorAll("[data-cmd]").forEach(btn => {
      const primeNotesCommand = (e) => {
        try{ e.preventDefault(); e.stopPropagation(); }catch{}
        restoreNotesSelection();
        cacheNotesSelection();
      };
      btn.addEventListener("pointerdown", primeNotesCommand);
      btn.addEventListener("mousedown", primeNotesCommand);
      btn.addEventListener("touchstart", primeNotesCommand, {passive:false});
      btn.addEventListener("click", (e) => {
        try{ e.preventDefault(); e.stopPropagation(); }catch{}
