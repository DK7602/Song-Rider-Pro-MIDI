...
- stable floating R button above mobile keyboard
- keeps rhyme dock lifted too
***********************/
if (window.visualViewport) {
  ...
  const getOverlap = () => {
    ...
    const raw = Math.max(0, baselineBottom - viewportBottom);
    if(raw <= 120){
      baselineBottom = Math.max(window.innerHeight, viewportBottom);
      return 0;
    }
    return raw;
  };
  ...
  const applyRhymeViewportUI = () => {
    ...
    rBtn.style.top = "auto";
    rBtn.style.bottom = `${overlap + 12}px`;
