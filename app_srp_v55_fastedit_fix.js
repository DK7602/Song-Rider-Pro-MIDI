...
function parseFullTextToSectionCards(fullText){
  ...
  for(let i=0; i<lines.length; i++){
    ...
    if(pendingChordLine){
      ...
      slots = buildSlotsFromInlineChords(inlineChords, cleaned);
    }
 ...
 updateSyllPill(syll, line.lyrics || "");
  ...
  fastEdit("lyrics", () => {
    ...
    if(state.autoSplit && String(line.rawText || "").includes("\n")){
      ...
      line.rawText = first;
      const parsedFirst = extractInlineChords(line.rawText || "");
      ...
      if(hasMarkupFirst && parsedFirst.chords && parsedFirst.chords.length){
        line.notes = buildSlotsFromInlineChords(parsedFirst.chords, line.lyrics);
      }
      ...
      if(more.length){
        ...
        more.forEach(txt => {
          const nl = newLine();
          nl.rawText = txt;
          const parsedNL = extractInlineChords(nl.rawText || "");
          ...
          if(hasMarkupNL && parsedNL.chords && parsedNL.chords.length){
            nl.notes = buildSlotsFromInlineChords(parsedNL.chords, nl.lyrics);
          }
