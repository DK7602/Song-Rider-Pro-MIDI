...
function fullViewAutoScrollFrame(ts){
  ...
  if(!state.fullViewAutoScrollLastTs) state.fullViewAutoScrollLastTs = ts;
  const dt = Math.min(64, Math.max(0, ts - state.fullViewAutoScrollLastTs)); // ms
  state.fullViewAutoScrollLastTs = ts;

  const pxPerSec = getFullViewScrollPxPerSec();
  const dy = (pxPerSec * dt) / 1000;

  // Scroll inside sheetBody
  sb.scrollTop += dy;
...
if(el.capoInput){
  ...
  if(el.bpmInput) el.bpmInput.addEventListener("input", () => {
    ...
    if(Number.isFinite(n) && n >= 40 && n <= 220){
      ...
      if(state.drumsOn) startDrums();
      restartClockIfRunning();
      if(state.fullViewAutoScrollOn && state.currentSection === "Full") startFullViewBeatTimer();
    }
  });
