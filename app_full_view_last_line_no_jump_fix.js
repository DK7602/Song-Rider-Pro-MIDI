...
function keepFullTextareaSteady(ta, prevBottom){
  if(!ta) return;
  ...
  // If autosize grew downward while the keyboard is open, compensate by the growth amount.
  if(Number.isFinite(prevBottom)){
    const growth = rect.bottom - prevBottom;
    if(growth > 0.5 && prevBottom >= (limit - 36)){
      dy = Math.max(dy, growth);
    }
  }

  // Still make sure the textarea bottom stays above the visible viewport bottom.
  if(rect.bottom > limit){
    dy = Math.max(dy, rect.bottom - limit);
  }

  if(dy > 0.5){
    scroller.scrollTop += Math.ceil(dy);
  }
}
...
if(state.currentSection === "Full"){
  ...
  blocks.forEach((b, idx) => {
    ...
    ta.addEventListener("input", () => {
      ...
      autosizeTextarea(ta);
      requestAnimationFrame(() => keepFullTextareaSteady(ta, prevBottom));
