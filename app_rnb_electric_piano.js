...
/***********************
SMOOTH ELECTRIC PIANO NOTE (R&B vibe)
Replaces old piano synth with warm tine-style electric piano
***********************/
function pianoNote(ctx, freq, durMs, vel=0.9){
  ...
  const peak = 0.055 * Math.max(0.2, Math.min(vel,1));

  env.gain.setValueAtTime(0.0001, t0);
  env.gain.linearRampToValueAtTime(peak, t0 + 0.02);
  env.gain.linearRampToValueAtTime(peak*0.75, t0 + 0.18);

  const endTime = t0 + dur;

  env.gain.setValueAtTime(peak*0.7, endTime);

  const tail = Math.max(1.2, Math.min(5, 1.4 + dur*0.7));
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);

  // subtle chorus movement
  ...
  const stopTime = endTime + tail + 1;
  o1.stop(stopTime);
  o2.stop(stopTime);
  o3.stop(stopTime);
  lfo.stop(stopTime);

  return { out: env, nodes:[o1,o2,o3,g1,g2,g3,sum,tone,env,lfo,lfoGain] };
}
