...
function insertChecklistItemAfter(existingItem){
  ...
  setCaretInsideNotesNode(made.text, false);
  cacheNotesSelection();
  saveNotesDoc();
  pushNotesHistorySnapshot();
  return made.item;
...
function normalizeNotesChecklistState(root=el.notesEditor){
  ...
  root.querySelectorAll('.notesChecklistItem').forEach((item) => {
    ...
    if(text) text.style.textDecoration = checked ? 'line-through' : 'none';
  });
}
...
function insertNotesChecklistItem(){
  ...
  normalizeNotesChecklistState();
  setCaretInsideNotesNode(text, false);
  cacheNotesSelection();
  saveNotesDoc();
  pushNotesHistorySnapshot();
}
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener("keydown", (e) => {
      const textNode = e.target?.closest?.('.notesChecklistText');
      if(!textNode) return;
