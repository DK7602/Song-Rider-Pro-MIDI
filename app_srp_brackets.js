...
function parseFullTextToSectionCards(fullText){
  ...
  // Parses bracket-style inline chords like:
  //   Amazing [G]grace how [D]sweet the sound
  // Returns:
  //   cleaned: "Amazing grace how sweet the sound"
  //   chords: [{tok:"G", pos:<index in cleaned>}, ...]
  const s = String(line ?? "");
  let out = "";
  const chords = [];
  let i = 0;
...
// Map inline chords with character positions to 8 note slots (0..7).
// Positions are indexes in the cleaned lyric string.
function buildSlotsFromInlineChords(inlineChords, cleanedLyric){
  ...
  for(const item of inlineChords){
    const tok = (typeof item === "string") ? String(item).trim()
              : (item && typeof item === "object") ? String(item.tok || "").trim()
              : "";
    if(!tok) continue;
    ...
    // fallback: left->right distribution if no pos
    const slot = (col === null) ? null : col;

    if(slot === null){
      // distribute sequentially across open slots
      let placed = false;
      for(let i=0;i<8;i++){
  ...
  return slots;
      ...
      // unmatched "[" -> fall through as normal char
    }

    // Caret style (back-compat): ^G or ^Dm7 (optional)
    if(ch === "^"){
      // eat caret + optional whitespace + chord token
      let k = i + 1;
      while(k < s.length && /\s/.test(s[k])) k++;
      // read token chars until whitespace
      let t = "";
  ...
  // Normalize spacing: keep user's spacing mostly, but avoid weird multi-spaces from edits
  // (Do NOT trim left/right too hard; cards use .trim() where needed.)
  out = out.replace(/\s+/g, " ");
  return { cleaned: out, chords };
});
  ...
  for(let i=0; i<lines.length; i++){
    ...
    if(pendingChordLine){
      ...
      // inline chords: place using character positions when available
