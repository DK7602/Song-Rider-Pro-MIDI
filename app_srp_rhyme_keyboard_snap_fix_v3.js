...
- floating R button snaps just above keyboard on Full, Cards, and Notes
- use direct top placement from visualViewport so Android Chrome is consistent
***********************/
{
  ...
  let rafId = 0;
  let settleTimers = [];
  let baselineVisibleBottom = 0;
  let baselineInnerHeight = 0;

  const BUTTON_GAP = 8;     // space above keyboard
  const DOCK_GAP = 8;       // space above keyboard
  const DEFAULT_BTN_BOTTOM = 18;
  const DEFAULT_DOCK_BOTTOM = 12;
  ...
  function ensureOverlayRoots(){
    ...
    if(dock){
      ...
      dock.style.zIndex = "2147483645";
      dock.style.transform = "none";
    }

    if(rBtn){
      rBtn.style.position = "fixed";
      rBtn.style.right = "12px";
      rBtn.style.left = "auto";
      rBtn.style.zIndex = "2147483646";
  ...
  function getActiveEditable(){
    ...
    if(a.tagName === "INPUT"){
      const t = String(a.type || "text").toLowerCase();
      if(["text","search","email","url","tel","number","password"].includes(t)) return a;
    }
  ...
  function getViewportState(){
    const innerH = Math.max(0, Math.round(window.innerHeight || 0));
    const docH = Math.max(0, Math.round((document.documentElement && document.documentElement.clientHeight) || 0));
    const vvTop = vv ? Math.max(0, Math.round(vv.offsetTop || 0)) : 0;
    const vvHeight = vv ? Math.max(0, Math.round(vv.height || 0)) : Math.max(innerH, docH);
    const visibleBottom = vvTop + vvHeight;

    baselineVisibleBottom = Math.max(baselineVisibleBottom || 0, visibleBottom || 0);
    baselineInnerHeight = Math.max(baselineInnerHeight || 0, innerH || 0, docH || 0);

    const insetFromVisibleBottom = Math.max(0, baselineVisibleBottom - visibleBottom);
    const layoutShrunk = innerH > 0 && baselineInnerHeight > 0 && (baselineInnerHeight - innerH) > 80;
    const activeEditable = !!getActiveEditable();
    const keyboardOpen = activeEditable && insetFromVisibleBottom > KEYBOARD_THRESHOLD;

    return {
      keyboardOpen,
      keyboardInset: insetFromVisibleBottom,
      vvTop,
      vvHeight,
      visibleBottom,
      layoutShrunk,
  ...
  function placeAboveKeyboard(node, gapPx, fallbackBottomPx){
    if(!node) return;

    const state = getViewportState();
    node.style.position = "fixed";
    node.style.transition = "top 120ms ease, bottom 120ms ease, opacity 120ms ease";
    node.style.transform = "none";

    if(!state.keyboardOpen){
      node.style.top = "auto";
      node.style.bottom = `${fallbackBottomPx}px`;
      return;
    }

    if(state.layoutShrunk){
      node.style.top = "auto";
      node.style.bottom = `${gapPx}px`;
      return;
    }

    node.style.top = "auto";
    node.style.bottom = `${Math.round(state.keyboardInset + gapPx)}px`;
  }

  function applyRhymeViewportUI(){
    rafId = 0;
    ensureOverlayRoots();
    ...
    if(dock){
      placeAboveKeyboard(dock, DOCK_GAP, DEFAULT_DOCK_BOTTOM);
      dock.style.maxHeight = state.keyboardOpen
        ? `${Math.max(120, Math.round(state.vvHeight * 0.42))}px`
        : "";
    }

    if(rBtn){
      placeAboveKeyboard(rBtn, BUTTON_GAP, DEFAULT_BTN_BOTTOM);
  ...
  function updateRhymeViewportUI(){
    if(rafId) cancelAnimationFrame(rafId);
    rafId = requestAnimationFrame(applyRhymeViewportUI);
  }

  function settleRhymeViewportUI(){
    clearSettleTimers();
    updateRhymeViewportUI();
    [0, 40, 100, 180, 280, 420, 650, 900].forEach(ms => {
      settleTimers.push(setTimeout(updateRhymeViewportUI, ms));
    });
  }
  ...
  if(vv){
    vv.addEventListener("resize", settleRhymeViewportUI);
    vv.addEventListener("scroll", settleRhymeViewportUI);
  }
  window.addEventListener("resize", settleRhymeViewportUI);
  window.addEventListener("orientationchange", settleRhymeViewportUI);
  document.addEventListener("focusin", settleRhymeViewportUI, true);
  document.addEventListener("focusout", settleRhymeViewportUI, true);
  document.addEventListener("selectionchange", settleRhymeViewportUI, true);
  document.addEventListener("click", settleRhymeViewportUI, true);
  window.addEventListener("scroll", settleRhymeViewportUI, { passive:true });

  if(rBtn){
    rBtn.addEventListener("click", settleRhymeViewportUI);
    rBtn.addEventListener("touchstart", settleRhymeViewportUI, { passive:true });
    rBtn.addEventListener("pointerdown", settleRhymeViewportUI, { passive:true });
  }
  if(hideBtn) hideBtn.addEventListener("click", settleRhymeViewportUI);
  if(notesBtn) notesBtn.addEventListener("click", settleRhymeViewportUI);
  if(notesCloseBtn) notesCloseBtn.addEventListener("click", settleRhymeViewportUI);

  if(dock && window.MutationObserver){
    new MutationObserver(settleRhymeViewportUI).observe(dock, {
      attributes: true,
      attributeFilter: ["style", "class"]
    });
  }
  if(notesModal && window.MutationObserver){
    new MutationObserver(settleRhymeViewportUI).observe(notesModal, {
      attributes: true,
      attributeFilter: ["style", "class", "aria-hidden"]
    });
  }

  settleRhymeViewportUI();
}
})();