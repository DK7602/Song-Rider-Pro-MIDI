...
***********************/
const BREAK_LINE = "\u2063\u2063\u2063\u2063\u2063\u2063\u2063\u2063\u2063\u2063"; // invisible break marker (legacy underscores still supported)
...
function parseFullTextToSectionCards(fullText){
  ...
  // New invisible marker (preferred)
  if(t === BREAK_LINE) return { title:"" };
  if(t.startsWith(BREAK_LINE)){
    const title = String(t.slice(BREAK_LINE.length) || "").trim();
    return { title };
  }

  // Legacy markers still supported:
  if(t === "__________") return { title:"" };
  if(t === "_") return { title:"" };
...
const hasBreaks = lines.some(l => !!_parseBreakLine(l));
  ...
  function maybeConsumeTitleLine(i){
    // IMPORTANT: Do NOT auto-steal the first lyric line as a title.
    // Titles are edited via the yellow pill (inline on the break marker).
    return { nextIndex: i, title: "" };
  }
...
function ensureFullBreaksPresent(fullText){
  ...
  // ✅ We only require ONE break line to exist (the first Song Part).
...
ta.value = normalizeFullBreakSpacing(state.project.fullText || "");
  ...
  // Ensure we always have safe spacing so pills / +× never sit on top of lyrics.
  // Rules:
  // - every break marker line must be followed by ONE blank line (reserved space under the pill)
  // - every section must end with ONE blank line before the next break (reserved space for +×)
  function normalizeFullBreakSpacing(text){
    const lines = String(text || "").replace(/\r\n/g,"\n").replace(/\r/g,"\n").split("\n");
    const out = [];
    for(let i=0;i<lines.length;i++){
      let line = String(lines[i] ?? "");

      // If someone pasted lyrics onto the SAME line as the break marker,
      // do NOT treat that as a title. Split it onto the next line.
      if(String(line).startsWith(BREAK_LINE)){
        const rest = String(line).slice(BREAK_LINE.length).trim();
        const isPlausibleTitle =
          !!rest &&
          rest.length <= 28 &&
          /^[A-Za-z0-9 ]+$/.test(rest) &&
          /[A-Za-z]/.test(rest);
        if(rest && !isPlausibleTitle){
          // keep break marker only; push the "rest" as content below
          line = BREAK_LINE;
          out.push(line);
          out.push(rest);
          continue;
      ...
      if(br){
        // guarantee one blank line right after the break marker
        const nxt = (i+1 < lines.length) ? String(lines[i+1] ?? "") : null;
        if(nxt !== ""){
          out.push("");
        }
        continue;
      ...
      // If the next line is a break marker, guarantee one blank line right before it
      if(i+1 < lines.length && _parseBreakLine(lines[i+1])){
        if(String(line).trim() !== "" && line !== ""){
          out.push("");
        }
      }
    }

    // Always start with a break marker
    if(out.length === 0 || !_parseBreakLine(out[0])){
      out.unshift(BREAK_LINE, "");
    }
  ...
  function onFullTextareaInput(){
    ...
    ta.value = normalizeFullBreakSpacing(ta.value);
    state.project.fullText = ta.value;
