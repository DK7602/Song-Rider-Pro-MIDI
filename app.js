...
/***********************
ACOUSTIC PIANO NOTE
- warmer acoustic piano voice for true piano block chords
***********************/
function acousticPianoNote(ctx, freq, durMs, vel=0.9){
  const t0 = ctx.currentTime;
  const dur = Math.max(0.12, durMs/1000);
  const f0 = clamp(freq, 35, 2400);
  const v  = clamp(vel, 0.2, 1.0);
  ...
  o3.type = "sine";
  o3.frequency.value = Math.max(30, f0 * 3);
  ...
  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.70, endTime);
  const tail = clamp(1.10 + dur * 0.85, 1.4, 5.8);
  env.gain.exponentialRampToValueAtTime(0.0001, endTime + tail);
  ...
  clickBP.frequency.value = Math.min(2400, f0 * 3.2);
  clickBP.Q.value = 0.8;

  const clickG = ctx.createGain();
  clickG.gain.setValueAtTime(0.0001, t0);
  clickG.gain.exponentialRampToValueAtTime(0.010 * v, t0 + 0.002);
  clickG.gain.exponentialRampToValueAtTime(0.0001, t0 + nLen);
  ...
  o1.start(t0); o2.start(t0); o3.start(t0); ns.start(t0);
  const stopAt = endTime + tail + 0.12;
  o1.stop(stopAt); o2.stop(stopAt); o3.stop(stopAt); ns.stop(t0 + nLen + 0.01);

  scheduleCleanup([o1,o2,o3,g1,g2,g3,mix,ns,clickBP,clickG,hp,body,presence,lp,env,outTrim], (durMs + tail*1000 + 1200));
  return { out: outTrim, nodes:[o1,o2,o3,g1,g2,g3,mix,ns,clickBP,clickG,hp,body,presence,lp,env,outTrim] };
}
/***********************
SYNTH KEYS NOTE (for first instrument)
Distinct from the electric keyboard and acoustic piano voices.
***********************/
...
function playSingleNoteForInstrument(rawChord, durMs){
  ...
  if(state.instrument === "acoustic"){
    const n = acousticPianoNote(ctx, f, durMs, 0.92);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 7200);
  }else if(state.instrument === "electric"){
    const n = pianoNote(ctx, f, durMs, 0.84);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 6000);
  }else{
    const n = grandPianoNote(ctx, f, durMs, 0.86);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 4200);
  }
}

function playKeyboardBlockChord(ch, durMs, fracMul=1, voice="synth"){
  const ctx = ensureCtx();
  ...
  if(voice === "acoustic"){
    dryBus.gain.value = 0.34;
    wet.gain.value = 0.10;
  }else if(voice === "electric"){
    dryBus.gain.value = 0.28;
    wet.gain.value = 0.07;
  }else{
    dryBus.gain.value = 0.32;
    wet.gain.value = 0.06;
  }
  ...
  const velocityCurve = (i, len) => {
    const centerBias = Math.abs((len - 1) / 2 - i);
    if(voice === "acoustic") return clamp(0.92 - i * 0.03 - centerBias * 0.015, 0.60, 0.96);
    if(voice === "electric") return clamp(0.86 - i * 0.028 - centerBias * 0.010, 0.56, 0.90);
    return clamp(0.88 - i * 0.03 - centerBias * 0.012, 0.58, 0.92);
  };

  for(let i=0;i<freqs.length;i++){
    if(!state.instrumentOn) break;
    const f = freqs[i];
    const vel = velocityCurve(i, freqs.length);
    let n;
    if(voice === "acoustic") n = acousticPianoNote(ctx, f, durMs, vel);
    else if(voice === "electric") n = pianoNote(ctx, f, durMs, vel);
    else n = grandPianoNote(ctx, f, durMs, vel);
    n.out.connect(dryBus);
    scheduleCleanup([n.out], durMs + (voice === "acoustic" ? 9000 : voice === "electric" ? 7000 : 5200));
  }

  scheduleCleanup([dryBus,wet, ...room.nodes], durMs + (voice === "acoustic" ? 10000 : voice === "electric" ? 8000 : 6000));
}

function playAcousticChord(ch, durMs, fracMul=1){
  playKeyboardBlockChord(ch, durMs, fracMul, "acoustic");
}

function playElectricChord(ch, durMs, fracMul=1){
  playKeyboardBlockChord(ch, durMs, fracMul, "electric");
}

function playPianoChord(ch, durMs, fracMul=1){
  playKeyboardBlockChord(ch, durMs, fracMul, "synth");
}
