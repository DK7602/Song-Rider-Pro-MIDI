...
function normalizeAudioMeta(meta, projectIdFallback=""){
  if(!meta || typeof meta !== "object") return null;
  const id = String(meta.id || "").trim();
  if(!id) return null;
  return {
    id,
    projectId: String(meta.projectId || projectIdFallback || "").trim(),
    kind: String(meta.kind || "upload"),
    createdAt: Number(meta.createdAt) || now(),
    title: String(meta.title || ""),
    offsetSec: Number(meta.offsetSec || 0) || 0,
    fileName: String(meta.fileName || ""),
    mimeType: String(meta.mimeType || ""),
    size: Number(meta.size || 0) || 0,
    storagePath: String(meta.storagePath || ""),
    url: String(meta.url || meta.publicUrl || "")
  };
}

function ensureProjectAudioFiles(project){
  if(!project || typeof project !== "object") return [];
  if(!Array.isArray(project.audioFiles)) project.audioFiles = [];
  project.audioFiles = project.audioFiles.map(meta => normalizeAudioMeta(meta, project.id)).filter(Boolean);
  return project.audioFiles;
}

function getCurrentProjectAudioFiles(){
  return ensureProjectAudioFiles(state.project);
}

function upsertCurrentProjectAudioMeta(meta){
  if(!state.project || !meta) return;
  const normalized = normalizeAudioMeta(meta, state.project.id);
  if(!normalized) return;
  editProject("audioMeta", () => {
    const list = ensureProjectAudioFiles(state.project);
...
function removeCurrentProjectAudioMeta(recId){
  if(!state.project || !recId) return;
  editProject("audioDelete", () => {
    const list = ensureProjectAudioFiles(state.project);
    state.project.audioFiles = list.filter(x => String(x.id) !== String(recId));
  });
}

function getCurrentProjectAudioMeta(recId){
  if(!state.project || !recId) return null;
  const list = ensureProjectAudioFiles(state.project);
  return list.find(x => String(x.id) === String(recId)) || null;
}

function fileExtensionFromNameOrType(name, type){
...
function recordingStoragePath(rec, blob){
  const userId = getAuthUserId();
  const projectId = String(rec?.projectId || state.project?.id || "").trim();
  const recId = String(rec?.id || uuid()).trim();
  const ext = fileExtensionFromNameOrType(rec?.fileName || rec?.title || "", blob?.type || rec?.mimeType || "");
  return `${userId}/${projectId}/${recId}${ext}`;
}
...
async function uploadRecordingBlobToCloud(rec, blob){
  if(!cloud.client || !getAuthUserId()) return null;
  const uploadBlob = blob || rec?.blob;
  if(!uploadBlob) return null;
  const size = Number(uploadBlob.size || 0) || 0;
  if(size > CLOUD_AUDIO_MAX_BYTES){
    throw new Error("Audio file is too large for cloud sync. Keep it under 15 MB.");
  }

  const storagePath = recordingStoragePath(rec, uploadBlob);
  const contentType = String(uploadBlob.type || rec?.mimeType || "application/octet-stream") || "application/octet-stream";

  const { error } = await cloud.client.storage
    .from(CLOUD_AUDIO_BUCKET)
  ...
  const pub = cloud.client.storage.from(CLOUD_AUDIO_BUCKET).getPublicUrl(storagePath);
  const publicUrl = String(pub?.data?.publicUrl || "").trim();
  return normalizeAudioMeta({
    id: rec?.id || uuid(),
    projectId: rec?.projectId || state.project?.id || "",
    kind: rec?.kind || "upload",
    createdAt: Number(rec?.createdAt) || now(),
    title: String(rec?.title || ""),
    offsetSec: Number(rec?.offsetSec || 0) || 0,
    fileName: String(rec?.fileName || rec?.title || ""),
    mimeType: contentType,
    size,
    storagePath,
    url: publicUrl
  }, rec?.projectId || state.project?.id || "");
}

async function deleteRecordingFromCloud(meta){
  if(!cloud.client || !getAuthUserId() || !meta) return;
  const path = String(meta.storagePath || "").trim();
  if(!path) return;
  const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).remove([path]);
  if(error) throw error;
}

async function persistRecordingToCloud(rec, opts={}){
  if(!cloud.ready || !cloud.client || !getAuthUserId() || !rec) return null;
  const projectId = String(rec.projectId || state.project?.id || "").trim();
  if(!projectId || !state.project || String(state.project.id) !== projectId) return null;

  const existing = getCurrentProjectAudioMeta(rec.id);
  if(existing?.storagePath && existing?.url && !rec.blob) return existing;

  const blob = rec.blob || (existing?.url ? await fetchBlobFromUrl(existing.url) : null);
  if(!blob) return existing || null;

  const meta = await uploadRecordingBlobToCloud({
    ...rec,
    title: String(rec.title || existing?.title || ""),
    fileName: String(rec.fileName || existing?.fileName || rec.title || ""),
    mimeType: String(rec.mimeType || existing?.mimeType || blob.type || ""),
    offsetSec: Number(rec.offsetSec ?? existing?.offsetSec ?? 0) || 0,
    createdAt: Number(rec.createdAt || existing?.createdAt) || now()
  }, blob);

  upsertCurrentProjectAudioMeta(meta);
  if(opts.render !== false) await renderRecordings();
  return meta;
}

async function syncCurrentProjectLocalRecordingsToCloud(){
  if(!cloud.ready || !cloud.client || !getAuthUserId() || !state.project?.id) return;
  let all = [];
  try{ all = await dbGetAll(); }catch{ return; }
  const pid = String(state.project.id || "");
  const list = all.filter(r => String(r.projectId || "") === pid && r.blob);
  for(const rec of list){
    const existing = getCurrentProjectAudioMeta(rec.id);
    if(existing?.storagePath && existing?.url) continue;
    try{
      await persistRecordingToCloud(rec, { render:false });
    }catch(err){
...
const CLOUD_AUDIO_MAX_BYTES = 15 * 1024 * 1024;
...
async function reloadProjectForCurrentScope(){
  ...
  if(cloud.user){
    setTimeout(() => { syncCurrentProjectLocalRecordingsToCloud().catch(err => console.warn(err)); }, 0);
  }
}
...
applyTick();
  ...
  try{
    ...
    const remote = getCurrentProjectAudioMeta(state.audioSyncRecId);
    if(remote){
      upsertCurrentProjectAudioMeta({ ...remote, offsetSec: t });
    }
    renderRecordings();
  }catch{}
}
...
async function uploadAudioFile(){
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      ...
      await renderRecordings();
      if(cloud.user){
        try{ await persistRecordingToCloud(item); }
        catch(err){ console.warn("Cloud upload failed:", err); alert(err?.message || "Cloud audio upload failed."); }
      }
      return;
    }
...
async function renderRecordings(){
  let localAll = [];
  try{
    localAll = await dbGetAll();
  }catch(e){
    console.warn('Recordings unavailable (IndexedDB blocked or failed):', e);
  }

  const pid = state.project?.id || "";
  const localMine = localAll.filter(r => String(r.projectId || "") === pid);
  const localMap = new Map(localMine.map(r => [String(r.id), r]));

  const remoteMine = getCurrentProjectAudioFiles()
    .filter(r => String(r.projectId || "") === pid)
    .map(r => ({ ...r, cloud: true }));

  const merged = new Map();
  remoteMine.forEach(rec => {
    const local = localMap.get(String(rec.id));
    merged.set(String(rec.id), { ...rec, ...(local || {}), title: (local?.title ?? rec.title ?? ""), offsetSec: Number(local?.offsetSec ?? rec.offsetSec ?? 0) || 0 });
  });
  localMine.forEach(rec => {
    if(!merged.has(String(rec.id))) merged.set(String(rec.id), rec);
  });

  const mine = Array.from(merged.values()).sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0));
  ...
  mine.forEach(rec => {
    ...
    const syncTag = rec.storagePath ? "☁︎ " : "";
    title.textContent = syncTag + ((rec.title && rec.title.trim() ? rec.title.trim() + " - " : "") + fmtDate(rec.createdAt || now()));
    ...
    edit.addEventListener("click", async () => {
      ...
      const remote = getCurrentProjectAudioMeta(rec.id);
      if(remote){
        upsertCurrentProjectAudioMeta({ ...remote, title: rec.title });
      }
      renderRecordings();
    });
    ...
    download.addEventListener("click", async () => {
      try{
        const sourceBlob = rec.blob || (rec.url ? await fetchBlobFromUrl(rec.url) : null);
        if(!sourceBlob) return;

        const ext = fileExtensionFromNameOrType(rec.fileName || rec.title || "", sourceBlob.type || rec.mimeType || "");
    ...
    del.addEventListener("click", async () => {
      ...
      const remote = getCurrentProjectAudioMeta(rec.id);
      if(remote){
        try{ await deleteRecordingFromCloud(remote); }catch(err){ console.warn(err); }
        removeCurrentProjectAudioMeta(rec.id);
      }
      const local = localMap.get(String(rec.id));
      if(local){
...
async function startAudioSyncFromRec(rec){
  ...
  const sourceBlob = rec.blob || (rec.url ? await fetchBlobFromUrl(rec.url) : null);
  const directUrl = String(rec.url || "").trim();
  const url = sourceBlob ? URL.createObjectURL(sourceBlob) : directUrl;
  if(!url) return;
  state.audioSyncUrl = url;
  state.audioSyncOwnsUrl = !!sourceBlob;
...
const item = {
  ...
  fileName: convertedOk ? "recording.mp3" : "recording.webm",
  mimeType: String(finalBlob?.type || mime || ""),
  blob: finalBlob,
  offsetSec: 0
};
...
async function init(){
  ...
  // IMPORTANT: never block first render on cloud init.
  // If Supabase/session boot is slow or hangs, the app used to look frozen.
  state.project = getCurrentProject();

  Promise.resolve().then(async () => {
    try{
      await initCloudSync();
      if(cloud.user && cloud.appMounted){
        await reloadProjectForCurrentScope();
      }
    }catch(err){
      console.error("Deferred cloud init failed", err);
      setCloudStatus("error", err?.message || String(err || "Unknown error"));
    }
  });
