...
if(state.currentSection === "Full"){
  ...
  function commitNow(){
    ...
    editProject("fullBlocks", () => {
      ...
      canonicalText = String(state.project.fullText || "");
    });
    renderTabs();
    renderSheetTitleBar();
    refreshDisplayedNoteCells();
    ...
    // If parsing normalized the Full text (for example, turning a trailing
    // parenthesized chord like (G) into the matching card chord row), rebuild
    // the visible Full editor so it shows the canonical parsed result too.
    if(String(canonicalText || "") !== String(newText || "")){
      setTimeout(() => {
        try{
          if(state.currentSection === "Full" && !state.fullPerfMode) renderSheet();
...
async function importTextToFull(text){
  ...
  editProject("importText", () => {
    ...
    applyFullTextToProjectSections(state.project.fullText || "");
    // Canonicalize Full Song View from parsed cards immediately after import.
    // This removes leftover inline chord wrappers like (G) that cards already
    // converted into chord boxes, keeping Full view and card view matched.
    syncFullTextFromSections();
    cleanupDeletedBaseSections();
  });
