...
lastChordRaw: "",
  ...
  // ✅ Full-song-view (Full tab) auto-scroll (like BSP)
  fullViewAutoScrollOn: false,
  fullViewAutoScrollRaf: null,
  fullViewAutoScrollLastTs: 0,
  fullViewBeatTimer: null,
  fullViewBeatIdx: 0,
  fullViewBaseTick8: 0,
  fullViewBaseScrollTop: 0,
  fullViewLineH: 24,
lastAutoBar: -1,
...
/***********************
✅ FULL SONG VIEW AUTO-SCROLL (Full tab) — matches BSP behavior
- Runs ONLY inside #sheetBody (not window)
- Independent of beat clock/audio/drums
- Speed is BPM-based (approx 1 lyric line per bar)
***********************/
function getFullViewScrollPxPerSec(){
  const bpm = clamp(state.bpm || 95, 40, 220);

  // Use the visible Full textarea's line-height if possible
  const ta = document.querySelector("textarea.fullSectionText, textarea.fullBox");
  let lineH = 24;
  try{
    if(ta){
  ...
  // 1 line per BAR (4 beats) => lineH px / (60/bpm*4) sec
  return (lineH * bpm) / 240;
}


// -------- Full-view highlight + beat flashing (BSP-style) --------
// Keeps a "reading line" highlight snapped to the nearest text line
// AND flashes 4 quadrants across that line in time with BPM.
function ensureFullViewAutoHighlight(){
  let hl = document.getElementById("fullAutoHL");
  if(!hl){
    // one-time CSS
    if(!document.getElementById('fullAutoHLStyle')){
      const st = document.createElement('style');
      st.id = 'fullAutoHLStyle';
      st.textContent = `
        #fullAutoHL{position:fixed;pointer-events:none;z-index:9999;opacity:0;transition:opacity 120ms ease;}
        #fullAutoHL .hlFrame{position:absolute;inset:0;border-radius:12px;border:2px solid rgba(255,213,74,.95);box-shadow:0 0 0 4px rgba(255,213,74,.22);background:rgba(255,213,74,.10);overflow:hidden;}
        #fullAutoHL .hlQ{position:absolute;top:0;bottom:0;width:25%;background:transparent;}
        #fullAutoHL .q0{left:0;}
        #fullAutoHL .q1{left:25%;}
        #fullAutoHL .q2{left:50%;}
        #fullAutoHL .q3{left:75%;}
        @keyframes fullHLFlash{0%{background:rgba(255,213,74,.05);}35%{background:rgba(255,213,74,.62);}100%{background:rgba(255,213,74,.05);}}
        #fullAutoHL .hlQ.on{animation:fullHLFlash 210ms ease;}
      `;
    ...
    hl.innerHTML = `
      <div class="hlFrame">
        <div class="hlQ q0"></div>
        <div class="hlQ q1"></div>
        <div class="hlQ q2"></div>
        <div class="hlQ q3"></div>
      </div>
    `;
...
function removeFullViewAutoHighlight(){
  const hl = document.getElementById('fullAutoHL');
  if(hl) hl.remove();
}
function fullViewPulseQuadrant(idx){
  const hl = ensureFullViewAutoHighlight();
  const qs = hl.querySelectorAll('.hlQ');
...
function stopFullViewBeatTimer(){
  if(state.fullViewBeatTimer){
    clearInterval(state.fullViewBeatTimer);
    state.fullViewBeatTimer = null;
  }
}
function startFullViewBeatTimer(){
  stopFullViewBeatTimer();
  state.fullViewBeatIdx = 0;
  const bpm = clamp(state.bpm || 95, 40, 220);
  const beatMs = Math.max(120, Math.round(60000 / bpm));
  // immediate pulse so it feels responsive
  fullViewPulseQuadrant(state.fullViewBeatIdx);
  state.fullViewBeatTimer = setInterval(()=>{
    if(!state.fullViewAutoScrollOn || state.currentSection !== 'Full') return;
    state.fullViewBeatIdx = (state.fullViewBeatIdx + 1) % 4;
    fullViewPulseQuadrant(state.fullViewBeatIdx);
  }, beatMs);
}

function updateFullViewAutoHighlight(){
  if(state.currentSection !== 'Full') return;
  const sb = el.sheetBody;
  if(!sb) return;

  // Find the visible Full textarea (for line height + padding)
  const ta = document.querySelector('textarea.fullSectionText, textarea.fullBox');
  let lineH = 24;
  let padTop = 0;
  try{
  ...
  // Target "reading line" is ~35% down the visible window (BSP-like)
  const targetY = r.top + Math.round(r.height * 0.35);

  // Snap highlight to the nearest text line in the textarea
  const relY = (targetY - r.top) + sb.scrollTop - padTop;
  const lineIdx = Math.max(0, Math.floor(relY / lineH));
  const snappedTop = r.top + (lineIdx * lineH + padTop - sb.scrollTop);

  const hl = ensureFullViewAutoHighlight();
  hl.style.left = (r.left + 8) + 'px';
  hl.style.top = (snappedTop - 1) + 'px';
  hl.style.width = Math.max(0, r.width - 16) + 'px';
  hl.style.height = Math.max(18, lineH + 2) + 'px';
  hl.style.opacity = state.fullViewAutoScrollOn ? '1' : '0';
}
function stopFullViewAutoScroll(){
  state.fullViewAutoScrollOn = false;
  // stop any legacy rAF loop if present
  if(state.fullViewRaf){
    try{ cancelAnimationFrame(state.fullViewRaf); }catch{}
    state.fullViewRaf = null;
  }
  clearTick();
  renderFullViewAutoScrollControl();
}

function fullViewAutoScrollFrame(ts){
  // Legacy pixel-scroll loop (no longer used).
  return;
}

function startFullViewAutoScroll(){
  // BSP-style: drive by the beat clock (autoAdvanceOnBar + applyTick)
  if(state.fullViewAutoScrollOn) return;
  state.fullViewAutoScrollOn = true;

  // Start from the card currently in view
  const cards = getCards();
  if(cards && cards.length){
    const cur = getNearestVisibleCard() || cards[0];
...
// Floating control (shown ONLY on Full tab)
function renderFullViewAutoScrollControl(){
  let host = document.getElementById("fullAutoScrollCtl");
  if(state.currentSection !== "Full"){
    if(host) host.remove();
    removeFullViewAutoHighlight();
    return;
  }

  if(!host){
    host = document.createElement("div");
    host.id = "fullAutoScrollCtl";
    host.className = "fullAutoScrollCtl";
    document.body.appendChild(host);

    // If user touches/scrolls the Full view, don't fight them — keep scrolling but reset dt baseline
    const sb = el.sheetBody;
    if(sb){
      const nudge = () => { state.fullViewAutoScrollLastTs = 0; };
      sb.addEventListener("wheel", nudge, { passive:true });
      sb.addEventListener("touchstart", nudge, { passive:true });
      sb.addEventListener("touchmove", nudge, { passive:true });
      sb.addEventListener("scroll", nudge, { passive:true });
    }
  }

  host.innerHTML = "";
  const btn = document.createElement("button");
  btn.className = "btn secondary fullAutoScrollBtn";
  btn.type = "button";
  btn.textContent = state.fullViewAutoScrollOn ? "Auto Scroll ON" : "Auto Scroll OFF";
  btn.style.fontWeight = "1100";
  btn.addEventListener("click", () => {
    if(state.fullViewAutoScrollOn) stopFullViewAutoScroll();
    else startFullViewAutoScroll();
  });
  host.appendChild(btn);

  // keep the main scroll icon in sync while on Full
  const isOn = !!state.fullViewAutoScrollOn;
  $("autoPlayBtn")?.classList.toggle("on", isOn);
  $("mScrollBtn")?.classList.toggle("on", isOn);
}
/***********************
...
function playInstrumentStep(){
  if(!state.instrumentOn) return;
  if(state.currentSection === "Full"){
    if(state.fullViewAutoScrollOn) fullAutoAdvanceOnBar();
    return;
  }
...
function switchToSectionForAuto(sec){
  ...
  // ✅ Full-song-view autoscroll only runs on Full tab
  if(sec !== "Full" && state.fullViewAutoScrollOn) stopFullViewAutoScroll();
  state.playCardIndex = null;
...
// ---------- Full Song View AutoScroll (BSP-style) ----------
function fullCardIsBlank(card){
  if(!card) return true;
  const beats = card.querySelectorAll("textarea.beatCell");
  const notes = card.querySelectorAll("input.noteCell");
  let hasBeats = false;
  let hasNotes = false;
  beats.forEach(b => { if(String(b && b.value || "").trim()) hasBeats = true; });
  notes.forEach(n => {
    const raw = String((n && n.dataset && n.dataset.raw) ?? (n && n.value) ?? "").trim();
    if(raw) hasNotes = true;
  });
  return !(hasBeats || hasNotes);
}

function fullAutoAdvanceOnBar(){
  const t8 = uiTick8();
  if(t8 === 0) return;
  if(t8 % 8 !== 0) return;
  ...
  if(cards[state.playCardIndex] && fullCardIsBlank(cards[state.playCardIndex])){
    for(let i=0;i<cards.length;i++){
      if(!fullCardIsBlank(cards[i])){
        state.playCardIndex = i;
  ...
  for(let i=state.playCardIndex+1; i<cards.length; i++){
    if(!fullCardIsBlank(cards[i])){
      state.playCardIndex = i;
      scrollCardIntoView(cards[i]);
      return;
...
function setAutoScroll(on){
  ...
  // ✅ In FULL SONG VIEW, the scroll button controls Full-view autoscroll (like BSP)
  if(state.currentSection === "Full"){
    if(want){
      // ensure card autoscroll is off
      state.autoScrollOn = false;
      startFullViewAutoScroll();
    }else{
      stopFullViewAutoScroll();
      state.autoScrollOn = false;
    }
  ...
  // ===== CARD VIEW AUTO SCROLL (existing behavior) =====
  state.autoScrollOn = want;
...
if(state.currentSection === "Full"){
  // ✅ Full Song View = BSP-style grid of ALL sections/lines
  if(el.sheetHint) el.sheetHint.textContent = "";

  const host = document.createElement("div");
  host.className = "cards";

  const order = getVisibleSectionPages().filter(s => s !== "Full");
  ...
  order.forEach(sec => {
    ...
    pill.value = String((state.project.sectionTitles && state.project.sectionTitles[sec]) ? state.project.sectionTitles[sec] : sec);
    ...
    pill.addEventListener("input", ()=>{
      ...
      editProject("sectionTitleFullGrid", ()=>{
        ensurePageMeta();
    ...
    arr.forEach((line, idx) => {
      ...
      const hasNotes = line.notes.some(n => String(n||"").trim());
      const hasBeats = line.beats.some(b => String(b||"").trim());
      if(!hasNotes && !hasBeats) return;

      const card = document.createElement("div");
      card.className = "card";
      card.dataset.fullSec = sec;
      card.dataset.fullIdx = String(idx);
      ...
      for(let i=0;i<8;i++){
        ...
        inp.addEventListener("focus", ()=>{ lastActiveCardEl = card; inp.value = inp.dataset.raw || ""; });
        ...
        inp.addEventListener("blur", ()=>{
          ...
          inp.dataset.raw = rawNow;
          line.notes[i] = rawNow;
          editProject("noteFullGrid", ()=>{ syncFullTextFromSections(); });
          updateKeyFromAllNotes();
          updateFullIfVisible();
      ...
      for(let i=0;i<4;i++){
        ...
        inp.addEventListener("pointerdown", (e)=>{ e.stopPropagation(); });
        inp.addEventListener("focus", ()=>{ lastActiveCardEl = card; });
        inp.addEventListener("input", ()=>{
          line.beats[i] = String(inp.value || "").trim();
          line.lyrics = line.beats.join(" ").replace(/\s+/g, " ").trim();
          editProject("beatFullGrid", ()=>{ syncFullTextFromSections(); });
          updateFullIfVisible();
          refreshRhymesFromActive();
        });
...
if(el.capoInput){
  ...
  if(el.bpmInput) el.bpmInput.addEventListener("input", () => {
    ...
    if(Number.isFinite(n) && n >= 40 && n <= 220){
      ...
      if(state.drumsOn) startDrums();
      restartClockIfRunning();
      // Full auto-scroll is synced to the main beat clock (no separate timer)
    }
  });
...
const v = Number.isFinite(raw) ? raw : 0; 
  ...
  if(el.autoPlayBtn) el.autoPlayBtn.addEventListener("click", () => {
    const next = (state.currentSection === "Full") ? !state.fullViewAutoScrollOn : !state.autoScrollOn;
    setAutoScroll(next);
  });
  if(el.mScrollBtn) el.mScrollBtn.addEventListener("click", () => {
    const next = (state.currentSection === "Full") ? !state.fullViewAutoScrollOn : !state.autoScrollOn;
    setAutoScroll(next);
  });
