...
if(state.currentSection === "Full"){
  ...
  function quickSaveFullText(){
    // Persist immediately, but keep it LIGHT while typing in Full view.
    // Saving through the full upsert/cloud path on every keystroke can cause
    // Android to collapse the keyboard or make freshly-added sections flicker.
    try{
      const newText = blocksToFullText(blocks);
      state.project.fullText = newText;
      ...
      const all = loadAllProjectsForScope(scope).map(normalizeProject).filter(Boolean);
      const i = all.findIndex(x => String(x.id||"") === String(state.project.id||""));
      const snap = deepClone(state.project);
      if(i >= 0) all[i] = snap;
      else all.unshift(snap);
      saveAllProjectsForScope(scope, all);
      setCurrentProjectIdForScope(scope, state.project.id);
      writeNotesBackup(state.project);
  ...
  blocks.forEach((b, idx) => {
    ...
    autosizeTextarea(ta);
    function restoreFullSectionFocus(){
      try{
        if(document.activeElement === ta) return;
        const start = (typeof ta.__lastSelStart === "number") ? ta.__lastSelStart : ta.value.length;
        const end = (typeof ta.__lastSelEnd === "number") ? ta.__lastSelEnd : start;
    ...
    ta.addEventListener("input", () => {
      ...
      autosizeTextarea(ta);
      requestAnimationFrame(() => {
        keepFullTextareaSteady(ta, snap);
        restoreFullSectionFocus();
      });
      setTimeout(restoreFullSectionFocus, 0);
      setTimeout(restoreFullSectionFocus, 60);
