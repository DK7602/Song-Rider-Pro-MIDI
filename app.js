...
function isLikelyTitleLine(cand){
    const t = String(cand || "").trim();
    if(!t) return false;

    // Allow common headings, optionally numbered
    const u = t.toUpperCase().replace(/:$/,"").replace(/\s+/g," ").trim();
    if(/^(INTRO|BRIDGE|OUTRO|INTERLUDE|TAG)$/.test(u)) return true;
    if(/^(VERSE|CHORUS)\s+\d+$/.test(u)) return true;
    if(/^(PRE-?CHORUS)\s*\d*$/.test(u)) return true;

    // Generic short ALL-CAPS titles (kept strict to avoid eating lyrics)
    if(u === t && u.length <= 18 && u.split(" ").length <= 3) return true;

    return false;
  }

  function maybeConsumeTitleLine(i){
    // Back-compat only: after a break line, the next non-empty line CAN be treated as a title,
    // but ONLY if it really looks like a section heading. Otherwise, do NOT consume it.
    let k = i;
    ...
    if(!isLikelyTitleLine(cand)){
      // Don't eat lyrics anymore
...
function buildFullScaffold(){
  ...
  // ✅ We include 2 blank lines after the break for pill + buttons clearance.
  return `${BREAK_LINE}
...
function ensureFullBreaksPresent(fullText){
  ...
  // Treat ANY line that starts with underscores as a break (supports "__________TITLE" too)
  const isBreak = (l) => {
    const t = String(l || "").trim();
    if(!t) return false;
    if(t === BREAK_LINE || t === "_") return true;
    return /^_{1,}/.test(t);
  };

  // If no breaks exist, insert one at the top with 2 blank lines for pill+buttons clearance.
  const any = lines.some(isBreak);
  if(!any){
    const prefix = `${BREAK_LINE}\n\n\n`;
    return prefix + text.replace(/^\s+/, "");
  }

  // Normalize: after every break line, ensure there are AT LEAST 2 blank lines
  const out = [];
  for(let i=0;i<lines.length;i++){
    out.push(lines[i]);
    if(isBreak(lines[i])){
      ...
      // add missing blanks
      for(let k=blanks;k<2;k++) out.push("");
    }
  }

  // Also ensure the file starts with a break line
  if(!isBreak(out[0])){
...
function syncFullTextFromSections(){
  ...
  try{
    ...
    getAllSectionOrder().filter(s => s !== "Full").forEach(sec => {
      ...
      // ✅ Keep 2 blank lines under the pill (room for pill + +/× buttons overlay)
      out.push("");
      out.push("");
...
ta.value = state.project.fullText || "";
  ...
  function positionOverlayPills(){
    ...
    const pillH = 34;
    const rowH  = Math.max(pillH + 58, lineH * 3); // masks break text + leaves room for +/× under pill
...
// + / × buttons live directly under the pill (inside the masked break-row area).
      const act = document.createElement("div");
      act.className = "breakActionsInline";
