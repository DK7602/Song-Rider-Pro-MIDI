...
function findFirstVisibleFullPerfLineIndex(){
  ...
  const midY = hostRect.top + hostRect.height * 0.30; // slightly above center like BSP
...
function setAutoScroll(on){
  ...
  if(state.autoScrollOn){
    ...
    if(state.currentSection === "Full"){
      ...
      // Start from the line currently in view
      state.fullPerfLineIndex = findFirstVisibleFullPerfLineIndex();
    }else{
...
if(state.currentSection === "Full"){
  ...
  if(state.fullPerfMode){
    ...
    blocks.forEach((b) => {
      ...
      // ✅ BSP-match: treat paragraph blocks (separated by blank lines) as ONE bar,
      // pick ONE lyric line per bar, then split into 4 beat boxes.
      const body = Array.isArray(b.body) ? b.body : [];

      // Build "bars" inside this section: chunks separated by blank lines.
      const bars = [];
      let buf = [];
      const flushBar = ()=>{
      ...
      for(const barText of bars){
        const lyricLine = _fullPerf_pickLyricLineFromBar(barText);
        if(!String(lyricLine||"").trim()) continue;

        const beats = _fullPerf_computeBeats(lyricLine);
    ...
    // Keep nearest line selection updated on user manual scroll
