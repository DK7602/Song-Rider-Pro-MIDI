  ...
  if(state.fullPerfMode){
    ...
    blocks.forEach((b) => {
      ...
      body.forEach((rawLine) => {
        ...
        // Skip obvious chord-only lines from beat-splitting (still display them)
        const looksChordy = /\[[^\]]+\]/.test(ln) || /^\s*\d+\s*[A-G]/.test(ln) || /^\s*\(?[A-G][#b]?m?(?:maj|min|dim|aug|sus|add)?\d*/i.test(ln.trim());

        if(looksChordy){
          const row = document.createElement("div");
          row.className = "fullPerfChordRow";
          row.textContent = ln;
          sec.appendChild(row);
          return;
        }

        const beats = manualBeatsFromSlashes(ln) || autosplitBeatsFromLyrics(ln);
        const row = document.createElement("div");
