...
function sanitizeAudioFileSegment(v){
  return String(v || "")
    .replace(/[\/:*?"<>|]+/g, "-")
    .replace(/\s+/g, " ")
    .trim();
...
function normalizeAudioMeta(meta){
  ...
  const cloudPath = String(meta.cloudPath || "").trim();
  const cloudUrl = String(meta.cloudUrl || "").trim();
  if(!id || !projectId || !cloudPath || !cloudUrl) return null;
  return {
    id,
    projectId,
    ...
    offsetSec: Number(meta.offsetSec) || 0,
    cloudPath,
    cloudUrl
  };
}
...
function getProjectAudioFiles(project){
  const list = Array.isArray(project?.audioFiles) ? project.audioFiles : [];
  return list.map(normalizeAudioMeta).filter(Boolean).sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0));
}

function setProjectAudioMeta(projectId, meta){
  const clean = normalizeAudioMeta(meta);
  if(!clean || !projectId) return false;
  ...
  const list = getProjectAudioFiles(project).filter(x => String(x.id || "") !== clean.id);
  list.unshift(clean);
  project.audioFiles = list;
  project.updatedAt = Math.max(Number(project.updatedAt)||0, now());
  all[idx] = project;
  saveAllProjects(all);
  if(state.project && String(state.project.id || "") === String(projectId || "")){
    state.project.audioFiles = list;
    state.project.updatedAt = project.updatedAt;
  }
  return true;
}

function removeProjectAudioMeta(projectId, audioId){
  const all = loadAllProjects().map(normalizeProject).filter(Boolean);
  const idx = all.findIndex(p => String(p.id || "") === String(projectId || ""));
  if(idx < 0) return null;
  const project = all[idx];
  const list = getProjectAudioFiles(project);
  const found = list.find(x => String(x.id || "") === String(audioId || "")) || null;
  project.audioFiles = list.filter(x => String(x.id || "") !== String(audioId || ""));
  project.updatedAt = now();
  all[idx] = project;
  saveAllProjects(all);
  if(state.project && String(state.project.id || "") === String(projectId || "")){
    state.project.audioFiles = project.audioFiles.slice();
    state.project.updatedAt = project.updatedAt;
  }
  return found;
}

function findProjectAudioMeta(projectId, audioId){
  const all = loadAllProjects().map(normalizeProject).filter(Boolean);
  const project = all.find(p => String(p.id || "") === String(projectId || ""));
  if(!project) return null;
  return getProjectAudioFiles(project).find(x => String(x.id || "") === String(audioId || "")) || null;
}

async function uploadRecordingBlobToCloud(rec){
  if(!rec || !rec.blob || !cloud.client || !getAuthUserId()) return null;
  const projectId = String(rec.projectId || "").trim();
  if(!projectId) return null;
  const userId = getAuthUserId();
  const ext = audioBlobExtension(rec.blob, rec.title || rec.blob?.name || "");
  const path = `${sanitizeAudioFileSegment(userId)}/${sanitizeAudioFileSegment(projectId)}/${sanitizeAudioFileSegment(rec.id || uuid())}${ext}`;
  const opts = { upsert: true };
  const ct = String(rec.blob?.type || "").trim();
  if(ct) opts['contentType'] = ct;
  const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).upload(path, rec.blob, opts);
  if(error) throw error;
  const pub = cloud.client.storage.from(CLOUD_AUDIO_BUCKET).getPublicUrl(path);
  const cloudUrl = String(pub?.data?.publicUrl || "").trim();
  if(!cloudUrl) throw new Error('Could not get public URL for uploaded audio.');
  return { cloudPath: path, cloudUrl };
}

async function ensureRecBlob(rec){
  if(rec?.blob) return rec.blob;
  const url = String(rec?.cloudUrl || "").trim();
  if(!url) return null;
...
async function syncSingleRecordingToCloud(rec, force=false){
  if(!rec || !rec.blob || !cloud.client || !getAuthUserId()) return null;
  const existingMeta = findProjectAudioMeta(rec.projectId, rec.id);
  if(existingMeta && existingMeta.cloudUrl && !force && String(rec.cloudUrl || '').trim() === String(existingMeta.cloudUrl || '').trim()){
    if(!String(rec.cloudUrl || '').trim()){
      rec.cloudUrl = existingMeta.cloudUrl;
      rec.cloudPath = existingMeta.cloudPath;
      try{ await dbPut(rec); }catch{}
    }
    return existingMeta;
  }
  const uploaded = await uploadRecordingBlobToCloud(rec);
  ...
  rec.cloudPath = meta.cloudPath;
  rec.cloudUrl = meta.cloudUrl;
  try{ await dbPut(rec); }catch{}
  setProjectAudioMeta(rec.projectId, meta);
  return meta;
}

async function backfillAllLocalAudioToCloud(){
  if(!cloud.client || !getAuthUserId()) return;
  let allRecs = [];
  try{ allRecs = await dbGetAll(); }catch{ return; }
  if(!Array.isArray(allRecs) || !allRecs.length) return;
  const knownProjects = new Set(loadAllProjects().map(p => String(p?.id || '')).filter(Boolean));
  let touched = false;
  for(const rec of allRecs){
    ...
    try{
      const before = findProjectAudioMeta(projectId, rec.id);
      const meta = await syncSingleRecordingToCloud(rec, false);
      if(meta && (!before || before.cloudUrl !== meta.cloudUrl || Number(before.offsetSec||0)!==Number(meta.offsetSec||0) || String(before.title||'')!==String(meta.title||''))){
        touched = true;
      }
    }catch(err){
      console.error('Audio backfill failed', rec?.id, err);
      setCloudStatus('error', err?.message || String(err || 'Audio upload failed'));
    }
  }
  if(touched){
    try{ await syncAllProjectsToCloud('audioBackfill'); }catch(err){ console.error(err); }
  }
  if(state.project) renderRecordings();
}

async function deleteCloudAudioObject(meta){
  if(!meta?.cloudPath || !cloud.client || !getAuthUserId()) return;
  try{
    const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).remove([String(meta.cloudPath)]);
    if(error) throw error;
  }catch(err){
...
async function handleAuthSessionChange(session, reason=""){
  ...
  if(cloud.user){
    ...
    try{
      ...
      backfillAllLocalAudioToCloud().catch(err => {
...
function normalizeProject(p){
  ...
  p.audioFiles = p.audioFiles.map(normalizeAudioMeta).filter(Boolean);
...
applyTick();
  ...
  try{
    ...
    if(rec){
      rec.offsetSec = t;
      await dbPut(rec);
      if(rec.cloudUrl || findProjectAudioMeta(rec.projectId, rec.id)){
        setProjectAudioMeta(rec.projectId, {
          id: rec.id,
          projectId: rec.projectId,
          kind: rec.kind,
          createdAt: rec.createdAt,
          title: rec.title,
          offsetSec: rec.offsetSec,
          cloudPath: rec.cloudPath || findProjectAudioMeta(rec.projectId, rec.id)?.cloudPath || '',
          cloudUrl: rec.cloudUrl || findProjectAudioMeta(rec.projectId, rec.id)?.cloudUrl || ''
        });
        queueCloudSync('audioBeat1');
      }
      renderRecordings();
    }
  }catch{}
...
card.appendChild(top);
    ...
    for(let i=0;i<8;i++){
      ...
      inp.addEventListener("pointerdown", (e) => {
        ...
        // Keep keyboard closed while deciding between tap-to-edit and long-press copy.
        e.preventDefault();
...
async function uploadAudioFile(){
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      ...
      if(getAuthUserId()){
        try{
          await syncSingleRecordingToCloud(item, true);
          await syncAllProjectsToCloud('audioUpload');
        }catch(err){
          console.error('Cloud audio upload failed', err);
          setCloudStatus('error', err?.message || String(err || 'Audio upload failed'));
        }
      }
...
async function renderRecordings(){
  ...
  const cloudMine = getProjectAudioFiles(state.project);

  const merged = [];
  const seen = new Set();
  for(const rec of localMine){
    const id = String(rec.id || "");
    if(!id || seen.has(id)) continue;
    const meta = cloudMine.find(x => String(x.id || "") === id);
    merged.push(Object.assign({}, meta || {}, rec));
    seen.add(id);
  }
  for(const meta of cloudMine){
    const id = String(meta.id || "");
    if(!id || seen.has(id)) continue;
    merged.push(Object.assign({}, meta));
    seen.add(id);
  }
  ...
  merged.forEach(rec => {
    ...
    title.textContent = (rec.title && String(rec.title).trim() ? String(rec.title).trim() + " - " : "") + fmtDate(rec.createdAt || now());
    ...
    edit.addEventListener("click", async () => {
      ...
      if(name === null) return;
      rec.title = (name.trim() || "");
      if(localMap.has(String(rec.id || ""))){
        const localRec = localMap.get(String(rec.id || ""));
        localRec.title = rec.title;
        await dbPut(localRec);
      }
      if(rec.cloudUrl){
        setProjectAudioMeta(rec.projectId || pid, {
          id: rec.id,
          projectId: rec.projectId || pid,
          kind: rec.kind,
          createdAt: rec.createdAt,
          title: rec.title,
          offsetSec: rec.offsetSec,
    ...
    play.addEventListener("click", async () => {
      try{
        await ensureRecBlob(rec);
        if(!rec.blob) return;
        if(state.audioSyncOn) stopAudioSync();
        await startAudioSyncFromRec(rec);
      }catch(err){
        console.error(err);
        alert('Could not load this audio on this device.');
      }
    });
    ...
    download.addEventListener("click", async () => {
      try{
        const blob = await ensureRecBlob(rec);
        if(!blob) return;
        const type = String(blob.type || "").toLowerCase();
        let ext = audioBlobExtension(blob, rec.title || 'recording');
        const safeBase = (rec.title && String(rec.title).trim() ? String(rec.title).trim() : "recording")
          .replace(/[\/:*?"<>|]+/g,"")
          .trim() || "recording";
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        ...
        console.error(err);
        alert('Could not download this audio.');
      }
    });
    ...
    del.addEventListener("click", async () => {
      ...
      const id = String(rec.id || '');
      const projectId = String(rec.projectId || pid || '');
      const meta = removeProjectAudioMeta(projectId, id) || normalizeAudioMeta(rec);
      if(localMap.has(id)) await dbDelete(id);
      if(meta && meta.cloudPath) await deleteCloudAudioObject(meta);
      queueCloudSync('audioDelete');
      renderRecordings();
    });
...
if(getAuthUserId()){
  try{
    await syncSingleRecordingToCloud(item, true);
    await syncAllProjectsToCloud('audioRecord');
  }catch(err){
    console.error('Cloud audio record upload failed', err);
    setCloudStatus('error', err?.message || String(err || 'Audio upload failed'));
  }
}
await renderRecordings();
