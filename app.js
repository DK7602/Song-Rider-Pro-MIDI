...
arr.forEach((line, idx) => {
      ...
      if(String(chordLine || "").trim()){
        blocks.push(
          `<pre style="margin:0 0 0 0;font-family:'Courier New',Courier,monospace;font-weight:900;color:#7f1d1d;white-space:pre;">${escapeHtml(`    ${chordLine}`)}</pre>`
        );
      }

      // Lyrics line (fixed width, spacing preserved)
      blocks.push(
        `<pre style="margin:0 0 10px 0;font-family:'Courier New',Courier,monospace;font-weight:400;color:#111;white-space:pre;">${escapeHtml(`    ${lyricLine}`)}</pre>`
      );
    });
