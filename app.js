  ...
  blocks.forEach((b, idx) => {
    ...
    bindTap(addBtn, () => {
      withPillActionLock(() => {
        ...
        requestAnimationFrame(() => {
          try{
            const all = Array.from((el.sheetInner || document).querySelectorAll(".fullSectionText"));
            const nextTa = all[idx + 1];
            if(nextTa){
              nextTa.focus();
              const end = String(nextTa.value || "").length;
              try{ nextTa.setSelectionRange(end, end); }catch{}
            }
          }catch{}
        });
      });
