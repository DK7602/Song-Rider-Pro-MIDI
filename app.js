...
- floating R button snaps just above keyboard consistently
***********************/
if (window.visualViewport) {
  ...
  const applyRhymeViewportUI = () => {
    ...
    if(rBtn){
      const btnSize = rBtn.offsetHeight || 52;
      const gap = keyboardOpen ? 14 : 18;
      const visibleBottom = vv.offsetTop + vv.height;
      const targetTop = Math.max(12, Math.round(visibleBottom - btnSize - gap));

      rBtn.style.transition = "top 140ms ease, opacity 120ms ease, transform 140ms ease";
      rBtn.style.transform = "translateY(0px)";
      rBtn.style.bottom = "auto";
      rBtn.style.top = `${targetTop}px`;
  ...
  const scheduleRhymeViewportUI = () => {
    if(rafId) cancelAnimationFrame(rafId);
    rafId = requestAnimationFrame(applyRhymeViewportUI);

    // Android keyboard viewport values often settle over a few frames.
    clearTimeout(settleT1);
    clearTimeout(settleT2);
    clearTimeout(settleT3);
    settleT1 = setTimeout(() => {
      if(rafId) cancelAnimationFrame(rafId);
      rafId = requestAnimationFrame(applyRhymeViewportUI);
    }, 90);
  ...
  visualViewport.addEventListener("resize", scheduleRhymeViewportUI);
  visualViewport.addEventListener("scroll", scheduleRhymeViewportUI);
  window.addEventListener("resize", scheduleRhymeViewportUI);
  window.addEventListener("scroll", scheduleRhymeViewportUI, { passive:true });
  window.addEventListener("orientationchange", scheduleRhymeViewportUI);
  window.addEventListener("focusin", scheduleRhymeViewportUI);
  window.addEventListener("focusout", scheduleRhymeViewportUI);

  // Re-run after the dock is toggled so the R button visibility/position stays correct.
  if(rBtn){
    rBtn.addEventListener("click", () => setTimeout(scheduleRhymeViewportUI, 0));
  }
  const hideBtn = document.getElementById("hideRhymeBtn");
  if(hideBtn){
    hideBtn.addEventListener("click", () => setTimeout(scheduleRhymeViewportUI, 0));
  }

  // Initial paint.
  setTimeout(scheduleRhymeViewportUI, 0);
}
