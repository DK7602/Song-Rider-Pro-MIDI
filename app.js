...
function audioBlobExtension(blob, fallbackName=""){
  const name = String(fallbackName || (blob && blob.name) || "").trim();
...
function audioBlobMime(blob, fallbackName=""){
  const mime = String(blob?.type || "").toLowerCase();
  if(mime) return mime;
  const ext = audioBlobExtension(blob, fallbackName);
...
function normalizeRecording(rec){
  ...
  out.kind = String(out.kind || "upload");
  out.createdAt = Number(out.createdAt) || now();
  out.updatedAt = Number(out.updatedAt) || out.createdAt || now();
  out.offsetSec = Number(out.offsetSec || 0) || 0;
  out.title = String(out.title || "");
  out.mimeType = String(out.mimeType || audioBlobMime(out.blob, out.title || out.fileName || "")) || "application/octet-stream";
  out.fileExt = String(out.fileExt || audioBlobExtension(out.blob, out.title || out.fileName || ""));
  out.cloudPath = String(out.cloudPath || "");
  out.cloudSyncedAt = Number(out.cloudSyncedAt) || 0;
  return out;
}

function normalizeRecordingManifestEntry(entry){
  ...
  out.fileExt = String(out.fileExt || "");
  out.path = String(out.path || "");
  out.deletedAt = Number(out.deletedAt) || 0;
...
const CLOUD_AUDIO_MANIFEST_VERSION = 1;
...
const cloud = {
  ...
  authMode: "signin",
  recoveryMode: false,
  recordingSyncingProjects: new Set(),
  recordingSyncQueued: new Map(),
  recordingManifestCache: new Map(),
  recordingSyncReason: ""
};
...
async function handleAuthSessionChange(session, reason=""){
  ...
  if(cloud.user){
    ...
    try{
      ...
      for(const p of loadAllProjects().map(normalizeProject).filter(Boolean)){
        queueProjectRecordingSync(p.id, reason || "auth");
      }
      updateAuthModalUI();
    }catch(err){
      if(seq !== cloud.authChangeSeq) return;
...
function wireAuthModal(){
  ...
  document.getElementById("authSyncNowBtn")?.addEventListener("click", async () => {
    try{
      ...
      for(const p of loadAllProjects().map(normalizeProject).filter(Boolean)){
        await syncProjectRecordingsWithCloud(p.id, "manual");
      }
...
async function dbPut(rec){
  ...
  const toSave = normalizeRecording(rec) || rec;
  return new Promise((resolve, reject) => {
...
async function getLocalRecordingsForProject(projectId){
  const pid = String(projectId || "");
  const all = await dbGetAll();
  return all.map(normalizeRecording).filter(Boolean).filter(r => String(r.projectId || "") === pid);
}

function canUseCloudRecordingSync(projectId){
  return !!(cloud.client && cloud.ready && getAuthUserId() && projectId);
}

function recordingManifestPath(projectId){
  return `users/${getAuthUserId()}/projects/${String(projectId || "")}/recordings/manifest.json`;
}

function recordingStoragePath(projectId, recId, ext=".audio"){
  const safeExt = String(ext || ".audio").startsWith(".") ? String(ext || ".audio") : ("." + String(ext || "audio"));
  return `users/${getAuthUserId()}/projects/${String(projectId || "")}/recordings/${String(recId || "")}${safeExt}`;
}

function recordingMetaFromLocal(rec){
  const r = normalizeRecording(rec);
  return {
    id: r.id,
    ...
    mimeType: r.mimeType,
    fileExt: r.fileExt,
    path: r.cloudPath || recordingStoragePath(r.projectId, r.id, r.fileExt),
    size: Number(r.blob?.size || 0) || 0,
    deletedAt: 0
  };
}

function recordingLocalFromMeta(meta, blob){
  const m = normalizeRecordingManifestEntry(meta);
  if(!m) return null;
...
async function readRecordingManifest(projectId){
  if(!canUseCloudRecordingSync(projectId)){
    return { version:CLOUD_AUDIO_MANIFEST_VERSION, projectId:String(projectId || ""), updatedAt:0, items:[] };
  }
  const path = recordingManifestPath(projectId);
  try{
    const { data, error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).download(path);
    if(error) throw error;
    const text = await data.text();
    const parsed = JSON.parse(text || "{}");
    const items = Array.isArray(parsed.items) ? parsed.items.map(normalizeRecordingManifestEntry).filter(Boolean) : [];
    const manifest = {
      version: Number(parsed.version) || CLOUD_AUDIO_MANIFEST_VERSION,
      projectId: String(parsed.projectId || projectId || ""),
      updatedAt: Number(parsed.updatedAt) || 0,
      items
    };
    cloud.recordingManifestCache.set(String(projectId || ""), manifest);
    return manifest;
  }catch(err){
    const msg = String(err?.message || err || "").toLowerCase();
    if(msg.includes("not found") || msg.includes("404") || msg.includes("object not found")){
      const empty = { version:CLOUD_AUDIO_MANIFEST_VERSION, projectId:String(projectId || ""), updatedAt:0, items:[] };
      cloud.recordingManifestCache.set(String(projectId || ""), empty);
      return empty;
    }
    throw err;
  }
...
async function writeRecordingManifest(projectId, manifest){
  if(!canUseCloudRecordingSync(projectId)) return;
  const cleanItems = Array.isArray(manifest?.items) ? manifest.items.map(normalizeRecordingManifestEntry).filter(Boolean) : [];
  const payload = {
    version: CLOUD_AUDIO_MANIFEST_VERSION,
    projectId: String(projectId || ""),
    updatedAt: now(),
    items: cleanItems
  };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type:"application/json" });
  const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).upload(
    recordingManifestPath(projectId),
    blob,
    { upsert:true, contentType:"application/json", cacheControl:"0" }
  );
  if(error) throw error;
  cloud.recordingManifestCache.set(String(projectId || ""), payload);
}

async function uploadRecordingBlobToCloud(rec){
  const r = normalizeRecording(rec);
  if(!canUseCloudRecordingSync(r.projectId) || !r.blob) return recordingMetaFromLocal(r);
  const path = recordingStoragePath(r.projectId, r.id, r.fileExt || audioBlobExtension(r.blob, r.title || ""));
  const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).upload(
    path,
    r.blob,
    { upsert:true, contentType:r.mimeType || audioBlobMime(r.blob, r.title || ""), cacheControl:"3600" }
  );
  if(error) throw error;
  r.cloudPath = path;
  r.cloudSyncedAt = now();
  await dbPut(r);
  return recordingMetaFromLocal(r);
}

async function downloadRecordingBlobFromCloud(meta){
  const m = normalizeRecordingManifestEntry(meta);
  if(!canUseCloudRecordingSync(m?.projectId) || !m?.path) return null;
  const { data, error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).download(m.path);
  if(error) throw error;
  const blob = data instanceof Blob ? data : new Blob([data], { type:m.mimeType || "application/octet-stream" });
  return recordingLocalFromMeta(m, blob);
}

function queueProjectRecordingSync(projectId, reason=""){
  const pid = String(projectId || "");
  if(!canUseCloudRecordingSync(pid)) return;
  if(cloud.recordingSyncQueued.has(pid)){
    try{ clearTimeout(cloud.recordingSyncQueued.get(pid)); }catch{}
  }
  const timer = setTimeout(() => {
    cloud.recordingSyncQueued.delete(pid);
    syncProjectRecordingsWithCloud(pid, reason).catch(err => {
      console.error("Recording cloud sync failed", err);
      setCloudStatus("error", err?.message || String(err || "Recording sync failed"));
    });
  }, 550);
  cloud.recordingSyncQueued.set(pid, timer);
}

async function syncProjectRecordingsWithCloud(projectId, reason=""){
  const pid = String(projectId || "");
  if(!canUseCloudRecordingSync(pid)) return;
  if(cloud.recordingSyncingProjects.has(pid)) return;
  cloud.recordingSyncingProjects.add(pid);
  try{
    setCloudStatus("syncing");
    const manifest = await readRecordingManifest(pid);
    const remoteItems = Array.isArray(manifest.items) ? manifest.items.map(normalizeRecordingManifestEntry).filter(Boolean) : [];
    const remoteMap = new Map(remoteItems.map(item => [String(item.id), item]));
    const localList = await getLocalRecordingsForProject(pid);
    ...
    for(const remote of remoteItems){
      ...
      if(!local){
        const downloaded = await downloadRecordingBlobFromCloud(remote);
        if(downloaded){
          await dbPut(downloaded);
      ...
      const remoteUpdated = Number(remote.updatedAt) || Number(remote.createdAt) || 0;
      if(remoteUpdated > localUpdated || Number(local.offsetSec || 0) !== Number(remote.offsetSec || 0) || String(local.title || "") !== String(remote.title || "")){
        const merged = normalizeRecording({
          ...local,
          title: remote.title,
          updatedAt: remoteUpdated,
          offsetSec: remote.offsetSec,
          mimeType: remote.mimeType || local.mimeType,
          fileExt: remote.fileExt || local.fileExt,
          cloudPath: remote.path || local.cloudPath,
          cloudSyncedAt: now()
        });
        await dbPut(merged);
        localMap.set(merged.id, merged);
        localChanged = true;
      }else if(localUpdated > remoteUpdated){
        const meta = await uploadRecordingBlobToCloud(local);
    ...
    if(manifestChanged){
      await writeRecordingManifest(pid, {
        projectId: pid,
        items: Array.from(remoteMap.values())
      });
    }
    if(localChanged && state.project?.id === pid){
      await renderRecordings();
    }
    setCloudStatus("ok");
  } finally {
    cloud.recordingSyncingProjects.delete(pid);
  }
}

async function deleteRecordingEverywhere(rec){
  const item = normalizeRecording(rec);
  if(!item) return;
  await dbDelete(item.id);

  if(canUseCloudRecordingSync(item.projectId)){
    const manifest = await readRecordingManifest(item.projectId);
    const items = Array.isArray(manifest.items) ? manifest.items.map(normalizeRecordingManifestEntry).filter(Boolean) : [];
    const existing = items.find(x => x.id === item.id);
    const tombstone = normalizeRecordingManifestEntry({
      ...(existing || recordingMetaFromLocal(item)),
      path: existing?.path || item.cloudPath || recordingStoragePath(item.projectId, item.id, item.fileExt),
      deletedAt: now(),
      updatedAt: now()
    });
    const nextItems = items.filter(x => x.id !== item.id);
    nextItems.push(tombstone);
    try{
      if(tombstone.path){
        const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).remove([tombstone.path]);
        if(error && !String(error.message || "").toLowerCase().includes("not found")) throw error;
      }
    }catch(err){
      console.warn("Cloud recording delete file warning:", err);
    }
    await writeRecordingManifest(item.projectId, { projectId:item.projectId, items:nextItems });
  }
}
...
applyTick();
  ...
  try{
    ...
    if(rec){
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      if(rec.projectId) queueProjectRecordingSync(rec.projectId, "beat1");
      renderRecordings();
    }
  }catch{}
}
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      const item = {
        ...
        blob: file,
        mimeType: audioBlobMime(file, file.name || "Audio"),
        fileExt: audioBlobExtension(file, file.name || "Audio"),
        offsetSec: 0
      };

      await dbPut(item);
      if(item.projectId) queueProjectRecordingSync(item.projectId, "uploadAudio");
      await renderRecordings();
      return;
    }
...
async function renderRecordings(){
  ...
  mine.forEach(rec => {
    ...
    edit.addEventListener("click", async () => {
      ...
      rec.updatedAt = now();
      await dbPut(rec);
      if(rec.projectId) queueProjectRecordingSync(rec.projectId, "renameRecording");
      renderRecordings();
    });
...
stop.addEventListener("click", () => {
  ...
  setTimeout(() => URL.revokeObjectURL(url), 800);
    ...
    del.addEventListener("click", async () => {
      ...
      await deleteRecordingEverywhere(rec);
      if(rec.projectId) queueProjectRecordingSync(rec.projectId, "deleteRecording");
      renderRecordings();
    });
...
async function startAudioSyncFromRec(rec){
  if(!rec) return;
  if(!rec.blob && rec.projectId && rec.cloudPath && canUseCloudRecordingSync(rec.projectId)){
    try{
      const downloaded = await downloadRecordingBlobFromCloud({
        id: rec.id,
      ...
      if(downloaded && downloaded.blob){
        rec = { ...rec, blob: downloaded.blob };
        await dbPut({ ...rec, cloudSyncedAt: now() });
      }
    }catch(err){
...
const item = {
  ...
  blob: finalBlob,
  mimeType: audioBlobMime(finalBlob, convertedOk ? "recording.mp3" : "recording.webm"),
  fileExt: audioBlobExtension(finalBlob, convertedOk ? "recording.mp3" : "recording.webm"),
  offsetSec: 0
};

await dbPut(item);
if(item.projectId) queueProjectRecordingSync(item.projectId, "recordStop");
await renderRecordings();
...
function loadProjectById(id){
  ...
  applyProjectSettingsToUI();
  renderAll();
  queueProjectRecordingSync(p.id, "loadProject");
}
...
const v = Number.isFinite(raw) ? raw : 0; 
  ...
  if(el.newProjectBtn) el.newProjectBtn.addEventListener("click", () => {
    ...
    applyProjectSettingsToUI();
    renderAll();
    queueProjectRecordingSync(p.id, "newProject");
  });
...
async function init(){
  ...
  await nextTick();
  renderRecordings();
  queueProjectRecordingSync(state.project?.id || "", "init");
  await nextTick();
  renderInstrumentUI();
  await nextTick();
  renderDrumUI();
