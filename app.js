...
function applyFullPerfTick(){
  ...
  // IMPORTANT: On engage, we must ALWAYS start at beat 1 of the current line
  // (quadrant 1), even if the underlying clock is mid-beat.
  const t8 = uiTick8();
  const nIdx = ((t8 % 8) + 8) % 8;
  const beat4Raw = Math.floor(nIdx / 2); // 0..3
  const beat4 = state.fullPerfFirstBeat ? 0 : beat4Raw;

  // Force the first visual beat to render immediately
  if(state.fullPerfFirstBeat){
