...
if(state.currentSection === "Full"){
  ...
  function commitDebounced(){
    ...
    commitTimer = setTimeout(() => {
      const active = document.activeElement;
      const isTypingFull = !!(active && active.tagName === "TEXTAREA" && active.classList && active.classList.contains("fullSectionText"));
      if(isTypingFull){
        try{ state._fullSectionCommitPending = true; }catch{}
        return;
      }
      commitNow();
      refreshRhymesFromActive();
  ...
  blocks.forEach((b, idx) => {
    ...
    ta.addEventListener("input", () => {
      ...
      // ✅ Save raw Full text immediately, but postpone the heavier section-sync work
      // until the user pauses or leaves the textarea. That keeps the mobile keyboard open.
      try{
        state.project.fullText = blocksToFullText(blocks);
        saveCurrentProject();
        state._fullSectionCommitPending = true;
      }catch(err){ console.error(err); }
    ...
    ta.addEventListener("blur", () => {
      setTimeout(() => {
        try{
          if(state._fullSectionCommitPending){
            state._fullSectionCommitPending = false;
            commitNow();
            refreshRhymesFromActive();
          }
        }catch(err){ console.error(err); }
...
async function init(){
  ...
  // IMPORTANT: never block first paint / local project load on cloud startup.
  // If Supabase is slow or hangs, the app should still open immediately.
  state.project = getCurrentProject();
  ...
  // Kick cloud init AFTER the app is already interactive.
  // This prevents a blank/frozen startup when network/auth is slow.
  initCloudSync()
    .then(async () => {
      try{
        updateAuthModalUI();
        if(cloud.user){
          await reloadProjectForCurrentScope();
          scheduleDeferredCloudAudioSync("post-init");
        }
      }catch(err){
        console.error("Post-init cloud refresh failed", err);
      }
    })
    .catch(err => {
      console.error("Deferred cloud init failed", err);
      try{ setCloudStatus("error", err?.message || String(err || "Unknown error")); }catch{}
    });
