...
.cards{
  ...
  padding-bottom: calc(clamp(140px, 35vh, 260px) + env(safe-area-inset-bottom));
    ...
    @media (min-width: 431px){
      .cardTop{ gap: 8px !important; }
      .cardTopLeft{ gap: 10px !important; }
      .cardTopRight{ gap: 6px !important; }
      .syllPill{
...
function injectChordUtilityBtnStyle(){
  const old = document.getElementById("srpChordUtilityBtnStyle");
  if(old) old.remove();

  const style = document.createElement("style");
  style.id = "srpChordUtilityBtnStyle";
  style.textContent = `
    .cardChordBtn{
      position: static !important;
...
function cloneChordRow(notes){
  return Array.from({length:8}, (_,i)=> String((Array.isArray(notes) ? notes[i] : "") || "").trim());
}

function applyChordRowToLine(line, notes){
  if(!line || typeof line !== "object") return;
  line.notes = cloneChordRow(notes);
}

function chordRowToClipboardText(notes){
  const arr = cloneChordRow(notes);
  return arr.map((n, i) => n ? `${i+1}${n}` : "").filter(Boolean).join(" ");
}

async function writeChordRowClipboard(notes){
  const arr = cloneChordRow(notes);
  state.chordRowClipboard = arr;
  const txt = chordRowToClipboardText(arr);
  try{
...
function normalizeChordPasteToken(tok){
  return String(tok || "")
    .trim()
    .replace(/^[\[\(\{]+/,"").replace(/[\]\)\}]+$/,"")
    .replace(/[|]+/g,"")
...
function parsePositionalChordPasteToken(tok){
  const t = normalizeChordPasteToken(tok);
  const m = t.match(/^([1-8])(.+)$/);
  if(!m) return null;
  const chord = String(m[2] || "").trim();
  if(!parseChordToken(chord)) return null;
  return { slot: clamp(parseInt(m[1],10) - 1, 0, 7), chord };
}

function parsePastedChordRowText(rawText){
  const src = String(rawText || "").trim();
  if(!src) return null;

  const notes = Array(8).fill("");
  const toks = src.split(/[\n\t,| ]+/).map(normalizeChordPasteToken).filter(Boolean);
  ...
  for(const tok of toks){
    const pos = parsePositionalChordPasteToken(tok);
    if(pos){
      notes[pos.slot] = pos.chord;
      any = true;
      continue;
    }
    if(parseChordToken(tok)){
      if(seq < 8){
        while(seq < 8 && notes[seq]) seq++;
        if(seq < 8){
          notes[seq] = tok;
          seq++;
          any = true;
...
async function readChordRowFromClipboard(){
  if(Array.isArray(state.chordRowClipboard) && state.chordRowClipboard.some(v => String(v || "").trim())){
    return cloneChordRow(state.chordRowClipboard);
  }
  try{
    if(navigator.clipboard?.readText){
      const txt = await navigator.clipboard.readText();
      const parsed = parsePastedChordRowText(txt);
...
function extractInlineChordMapFromLyrics(text){
  const src = String(text || "");
  const notes = Array(8).fill("");
  let found = false;
  let working = src;

  // Leading chord row support: 1Am 5Dm lyrics... or Am Dm G lyrics...
  const prefixMatch = src.match(/^\s*((?:(?:[1-8])?[A-Ga-g][#b]?(?:maj|min|m|dim|aug|sus|add|[0-9+#b\/]*)?(?:\/[A-Ga-g][#b]?)?\s+){1,8})(.+)$/s);
  if(prefixMatch){
    const prefix = String(prefixMatch[1] || "").trim();
    const remainder = String(prefixMatch[2] || "");
    const parsed = parsePastedChordRowText(prefix);
    if(parsed){
      for(let i=0;i<8;i++) if(parsed[i]) notes[i] = parsed[i];
      working = remainder;
      found = true;
    }
  }
  ...
  cleaned = cleaned.replace(/[\[\(\{]\s*([^\]\)\}]+?)\s*[\]\)\}]/g, (m, inner, offset) => {
    const tok = normalizeChordPasteToken(inner);
    if(parseChordToken(tok)){
      inline.push({ chord: tok, pos: offset });
      found = true;
    }
    return " ";
  });
  ...
  if(inline.length){
    const denom = Math.max(1, cleaned.length - 1);
    inline.forEach((item, idx) => {
      let slot = (inline.length === 1)
        ? 0
        : clamp(Math.round((Math.min(item.pos, denom) / denom) * 7), 0, 7);
      while(slot < 8 && notes[slot]) slot++;
      if(slot > 7) slot = Math.max(0, 7 - idx);
      if(!notes[slot]) notes[slot] = item.chord;
    });
  }

  return { lyrics: cleaned, notes, found: found || notes.some(v => String(v || "").trim()) };
}

function maybeApplyInlineChordsFromLyrics(line, textareaEl, notesInputs){
  if(!line || !textareaEl) return false;
  const parsed = extractInlineChordMapFromLyrics(textareaEl.value || "");
  if(!parsed || !parsed.found) return false;

  const nextLyrics = String(parsed.lyrics || "");
  const nextNotes = cloneChordRow(parsed.notes);
  const changedLyrics = nextLyrics !== String(line.lyrics || "");
  const changedNotes = JSON.stringify(nextNotes) !== JSON.stringify(cloneChordRow(line.notes));
  if(!changedLyrics && !changedNotes) return false;

  line.lyrics = nextLyrics;
  applyChordRowToLine(line, nextNotes);
  textareaEl.value = nextLyrics;
  if(Array.isArray(notesInputs)){
    notesInputs.forEach((inp, i) => {
      if(!inp) return;
      const raw = nextNotes[i] || "";
      inp.dataset.raw = raw;
...
const copyChordBtn = document.createElement("button");
copyChordBtn.className = "cardAdd cardChordBtn";
copyChordBtn.type = "button";
copyChordBtn.textContent = "⧉";
copyChordBtn.title = "Copy chord row";
copyChordBtn.addEventListener("click", async (e) => {
  e.preventDefault();
  e.stopPropagation();
  await writeChordRowClipboard(line.notes || []);
});

const pasteChordBtn = document.createElement("button");
pasteChordBtn.className = "cardAdd cardChordBtn";
pasteChordBtn.type = "button";
pasteChordBtn.textContent = "📋";
pasteChordBtn.title = "Paste chord row";
pasteChordBtn.addEventListener("click", async (e) => {
  e.preventDefault();
  e.stopPropagation();
  const parsed = await readChordRowFromClipboard();
  if(!parsed) return;
  editProject("pasteChordRow", () => {
    applyChordRowToLine(line, parsed);
    syncFullTextFromSections();
  });
  renderSheet();
  updateFullIfVisible();
...
const repeatChordBtn = document.createElement("button");
repeatChordBtn.className = "cardAdd cardChordBtn";
repeatChordBtn.type = "button";
repeatChordBtn.textContent = "↻";
repeatChordBtn.title = "Repeat previous card chords";
repeatChordBtn.addEventListener("click", (e) => {
  e.preventDefault();
  e.stopPropagation();
  if(idx <= 0 || !arr[idx - 1]) return;
  editProject("repeatPreviousChordRow", () => {
    applyChordRowToLine(line, arr[idx - 1].notes || []);
    syncFullTextFromSections();
  });
  renderSheet();
  updateFullIfVisible();
...
if(idx <= 0) repeatChordBtn.disabled = true;

topRight.appendChild(copyChordBtn);
topRight.appendChild(pasteChordBtn);
topRight.appendChild(repeatChordBtn);
...
card.appendChild(top);
    ...
    for(let i=0;i<8;i++){
      ...
      inp.addEventListener("paste", (e) => {
        ...
        noteInputs.forEach((cell, idx2) => {
          const raw2 = parsed[idx2] || "";
          if(!cell) return;
          cell.dataset.raw = raw2;
        });
        refreshDisplayedNoteCells();
        updateKeyFromAllNotes();
        updateFullIfVisible();
  ...
  // line.notes[i] already set above — just commit snapshot + save
    ...
    lyr.addEventListener("blur", () => {
      let changed = false;
      editProject("inlineChordAutoMap", () => {
        changed = maybeApplyInlineChordsFromLyrics(line, lyr, noteInputs);
        if(changed) syncFullTextFromSections();
      });
      if(changed){
        refreshDisplayedNoteCells();
        updateKeyFromAllNotes();
  ...
  let appliedInlineChords = false;
  editProject("lyrics", () => {
    // 1) set lyrics
    line.lyrics = lyr.value;

    // 1b) inline chords typed in the lyric box auto-map into the blue boxes
    appliedInlineChords = maybeApplyInlineChordsFromLyrics(line, lyr, noteInputs);
