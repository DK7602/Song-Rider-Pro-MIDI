...
function parseFullTextToSectionCards(fullText){
  ...
  function chordTokensFromLine(line){
    ...
    while(i < raw.length){
      ...
      const start = i;
      while(i < raw.length && raw[i] !== " ") i++;
      let tok = raw.slice(start, i).trim();
      tok = tok
        .replace(/^[^A-Za-z0-9#b\/\.]+/,"")
        .replace(/[^A-Za-z0-9#b\/\.]+$/,"")
        .replace(/[|]+/g,"")
      ...
      if(pos) toks.push(pos.chord);
      else if(isChordToken(tok)) toks.push(tok);
    }
    return toks;
  }

  function isSingleChordLine(line){
    return lineIsMostlyChords(line) && chordTokensFromLine(line).length === 1;
  }

  function appendChordLine(baseLine, extraLine){
    const base = String(baseLine || "").replace(/\s+$/g, "");
  ...
  function collectWrappedLyricContinuation(startIndex, firstLine, stopOnHeading){
    let merged = String(firstLine || "").trim();
    let i = startIndex;

    while(i + 1 < lines.length){
      const rawNext = String(lines[i + 1] ?? "");
      const next = rawNext.trim();
      if(!next) break;
      if(_parseBreakLine(next)) break;
      if(typeof stopOnHeading === "function" && stopOnHeading(next)) break;
      if(lineIsMostlyChords(next)) break;

      const inline = extractInlineChords(next);
      if((inline.chords || []).length) break;

      if(!merged) merged = inline.cleaned || next;
      else merged = (merged + " " + (inline.cleaned || next)).replace(/\s+/g, " ").trim();
      i++;
    }

    return { cleaned: merged, nextIndex: i };
  }
  ...
  if(hasBreaks){
    ...
    for(let i=0; i<lines.length; i++){
      ...
      if(lineIsMostlyChords(line)){
        if(pendingChordLine && lineIsMostlyChords(pendingChordLine) && isSingleChordLine(line)){
          pendingChordLine = appendChordLine(pendingChordLine, line);
          continue;
        }
        if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
          pushCard(cur, "", buildSlotsFromAlignedChordLine(pendingChordLine, ""));
      ...
      let { cleaned, chords: inlineChords } = extractInlineChords(line);
      const wrapped = collectWrappedLyricContinuation(i, cleaned, null);
      cleaned = wrapped.cleaned;
      i = wrapped.nextIndex;
    ...
    for(let i=0; i<lines.length; i++){
      ...
      let { cleaned, chords: inlineChords } = extractInlineChords(line);
      const wrapped = collectWrappedLyricContinuation(i, cleaned, headingTitleFromLine);
      cleaned = wrapped.cleaned;
      i = wrapped.nextIndex;
