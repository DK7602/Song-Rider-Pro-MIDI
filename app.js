...
***********************/
{
  ...
  function getViewportState(){
    const layoutH = Math.max(window.innerHeight || 0, document.documentElement ? document.documentElement.clientHeight || 0 : 0);
    if(!vv){
      return {
        keyboardOpen: false,
        keyboardInset: 0,
    ...
    const keyboardInset = Math.max(0, layoutH - visibleBottom);
    return {
  ...
  function placeAboveKeyboard(node, gapPx, fallbackBottomPx){
    ...
    const nodeH = Math.max(40, Math.round(rect.height || node.offsetHeight || 48));
    const targetTop = Math.max(
      8,
      Math.round(state.visibleBottom - nodeH - gapPx)
    );
  ...
  function applyRhymeViewportUI(){
    ...
    if(dock){
      ...
      dock.style.maxHeight = getViewportState().keyboardOpen
        ? `${Math.max(120, Math.round(getViewportState().vvHeight * 0.42))}px`
        : "";
    }
  ...
  function settleRhymeViewportUI(){
    ...
    [40, 100, 180, 280, 420, 650].forEach(ms => {
  ...
  if(rBtn) rBtn.addEventListener("click", settleRhymeViewportUI);
