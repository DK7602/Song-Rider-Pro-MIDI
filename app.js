...
.cards{
  ...
  padding-bottom: calc(clamp(140px, 35vh, 260px) + env(safe-area-inset-bottom));
    ...
    /* ===== BEAT TICK FLASH (yellow like BSP) ===== */
    .tickFlash{
      animation: srpTickFlash 120ms ease-out;
    }
    @keyframes srpTickFlash{
      0%   { background: rgba(250, 204, 21, .95); box-shadow: 0 0 0 6px rgba(250, 204, 21, .35); }
...
/***********************
CHORD UTILITIES (GLOBAL)
- Used by BOTH Full-page parser AND Card edits
- Fixes: chords in [ ] boxes disappearing / not syncing / instruments not playing
***********************/
const CHORD_TOKEN_RE_GLOBAL =
...
function isChordTokenGlobal(tok){
  ...
  return CHORD_TOKEN_RE_GLOBAL.test(t);
}

// Extract inline chords like: Amazing [G]grace  or  ^G Amazing
// Returns: { cleaned, chords:[{tok,pos}, ...] }
  ...
  while(i < s.length){
    ...
    if(ch === "["){
      ...
      if(j !== -1){
        ...
        if(inner && isChordTokenGlobal(inner)){
          chords.push({ tok: inner, pos: out.length });
          i = j + 1;
          continue;
        }
        // not a chord token -> keep literal
        out += s.slice(i, j+1);
        i = j + 1;
        continue;
      }
    ...
    if(ch === "^"){
      ...
      const tok = t.trim();
      if(tok && isChordTokenGlobal(tok)){
        chords.push({ tok, pos: out.length });
        i = k;
        continue;
      }
      // not chord -> keep caret
    }
...
// Map inline chord positions into 8 note slots (0..7)
function buildSlotsFromInlineChords(inlineChords, cleanedLyric){
  const lyric = String(cleanedLyric ?? "");
  const slots = Array(8).fill("");
  if(!inlineChords || !inlineChords.length) return slots;

  const len = Math.max(1, lyric.length);

  for(const item of inlineChords){
    const tok = (item && typeof item === "object") ? String(item.tok || "").trim()
              : String(item || "").trim();
    if(!tok) continue;

    let pos = null;
    if(item && typeof item === "object" && Number.isFinite(item.pos)) pos = item.pos;

    const slot = (pos === null)
      ? null
      : clamp(Math.round((pos / Math.max(1, len-1)) * 7), 0, 7);
    ...
    if(slot === null){
      ...
      if(!placed) slots[7] = (String(slots[7]||"") + " " + tok).trim();
    }else{
      if(!slots[slot]) slots[slot] = tok;
      else slots[slot] = (slots[slot] + " " + tok).trim();
    }
  }
...
function parseFullTextToSectionCards(fullText){
  ...
  // chord token matcher: A, Am, A7, Amaj7, D/F#, Bb, F#m7b5, etc + N.C.
  ...
  // heading detector:
  // - matches known sections like INTRO / VERSE 1 / CHORUS 2 / OUTRO / BRIDGE
  // - also matches custom titles stored in state.project.sectionTitles (so Full page headings can be renamed)
  function headingFromLine(line){
    ...
    // remove wrapping [](){} once
    t = t.replace(/^[\[\(\{]+\s*/, "").replace(/\s*[\]\)\}]+$/, "");

    t = t.toUpperCase().replace(/\s+/g," ").trim();
    t = t.replace(/^(VERSE|CHORUS)\s*(\d+)$/, "$1 $2");

    // direct match on known section keys
    if(FULL_EDIT_SECTIONS.includes(t)) return t;

    // match custom titles -> section key
    try{
      const map = state?.project?.sectionTitles;
  ...
  // Extract inline chords like:
  //   Amazing [G]grace how [D]sweet the sound
  //   ^G Amazing ^D sweet (legacy caret)
  // Returns cleaned lyric (no chord markers) + chord list with positions in the cleaned lyric.
  function extractInlineChords(line){
    const s = String(line ?? "");
    let out = "";
    const chords = [];
    ...
    while(i < s.length){
      ...
      // caret style (legacy): ^G or ^Dm7 (optional whitespace after caret)
      if(ch === "^"){
        let k = i + 1;
        while(k < s.length && /\s/.test(s[k])) k++;
        let t = "";
    ...
    // normalize spacing a bit (but don’t smash everything)
    const cleaned = out.replace(/\s+/g, " ");
    return { cleaned, chords };
  }
  ...
  function buildSlotsFromInlineChords(inlineChords, cleanedLyric){
    ...
    for(const item of inlineChords){
      ...
      if(slot === null){
        ...
        if(!placed){
          slots[7] = (String(slots[7]||"") + " " + tok).trim();
        }
      }else{
        if(!slots[slot]) slots[slot] = tok;
        else slots[slot] = (slots[slot] + " " + tok).trim();
  ...
  // detect “mostly chords” line (for chord-line-above-lyrics)
  function lineIsMostlyChords(line){
    const raw0 = String(line || "");
    const raw = raw0.trim();
    if(!raw) return false;

    // if lots of non A-G letters, likely lyrics
  ...
  // Build slots from “aligned” chord line above lyrics line.
  // If chord line spacing is collapsed (no multi-space runs), we fall back to left->right anchoring.
  function buildSlotsFromAlignedChordLine(chordLineRaw, lyricLineRaw){
    ...
    // collect chord tokens + their start column in chord line
    const toks = [];
    let i = 0;
    ...
    // compute lyric word starts (for collapsed chord lines)
    let wordStarts = [];
    if(looksCollapsed){
      const reWord = /\S+/g;
      let m;
    ...
    const denomChord = Math.max(1, chordLine.length - 1);
    const n = toks.length;
    ...
    toks.forEach((item, idx) => {
      ...
      if(looksCollapsed){
        ...
        const denomLyric = Math.max(1, lyricLine.length - 1);
        slot = clamp(Math.round((col / denomLyric) * 7), 0, 7);
      }else{
        slot = n > 8
          ? clamp(Math.floor(idx * 8 / n), 0, 7)
          : clamp(Math.round((item.col / denomChord) * 7), 0, 7);
      }
  ...
  function pushCard(rawTextLine, cleanedLyricsRaw, slots){
    const rawText = String(rawTextLine || "").replace(/\s+$/g,"");
    const lyrics  = String(cleanedLyricsRaw || "").replace(/\s+/g," ").trim();

    const hasLyrics = !!lyrics;
    const hasSlots  = !!(slots && slots.some(x => String(x||"").trim()));
    if(!hasLyrics && !hasSlots) return;
  ...
  for(let i=0; i<lines.length; i++){
    ...
    if(heading){
      // flush a dangling chord line as its own card (rare)
    ...
    if(line.trim() === ""){
      // if we have a chord-only line waiting and next non-empty is lyrics, keep it pending
      if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
        let k = i + 1;
        while(k < lines.length && String(lines[k] || "").trim() === "") k++;
        const nxt = k < lines.length ? String(lines[k] || "") : "";
        const nxtHeading = headingFromLine(nxt);
        if(nxt && !nxtHeading && !lineIsMostlyChords(nxt)){
          continue;
        }
      }

      // blank ends any pending chord line
      if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
        pushCard("", "", buildSlotsFromAlignedChordLine(pendingChordLine, ""));
      }
      pendingChordLine = null;
    ...
    // lyric line
    const { cleaned, chords: inlineChords } = extractInlineChordsGlobal(line);
    ...
    if(pendingChordLine){
      ...
      slots = buildSlotsFromInlineChordsGlobal(inlineChords, cleaned);
    }

    pushCard(line, cleaned, slots);
  }
...
function applyFullTextToProjectSections(fullText){
  ...
  FULL_EDIT_SECTIONS.forEach(sec => {
    ...
    for(let i=0; i<want.length; i++){
      ...
      base.rawText = String((w.rawText != null ? w.rawText : "") || "");
...
function normalizeProject(p){
  ...
  SECTIONS.filter(s=>s!=="Full").forEach(sec => {
    ...
    p.sections[sec] = p.sections[sec].map(line => {
      ...
      if(typeof L.rawText !== "string") L.rawText = "";
...
/***********************
FAST EDIT (debounced history + save)
- Prevents huge lag from deepClone() on every keystroke
- Keeps edits safe (auto-saves ~220ms after typing stops)
***********************/
const _fastEdit = { tmr:null, before:null, beforeSig:null };

function fastEdit(reason, mutatorFn){
  if(!state.project) return;
  if(history.lock){
    mutatorFn();
    return;
  ...
  // capture BEFORE snapshot once per “typing burst”
  if(!_fastEdit.before){
    _fastEdit.before = deepClone(state.project);
    _fastEdit.beforeSig = projectSignature(_fastEdit.before);
  }

  mutatorFn();

  if(_fastEdit.tmr) clearTimeout(_fastEdit.tmr);
  _fastEdit.tmr = setTimeout(() => {
    try{
      const afterSig = projectSignature(state.project);
      if(afterSig !== _fastEdit.beforeSig){
        historyCommitBeforeSnapshot(_fastEdit.before);
      }
    }catch{}
    try{ upsertProject(state.project); }catch{}
    _fastEdit.before = null;
    _fastEdit.beforeSig = null;
    _fastEdit.tmr = null;
  }, 220);
}

// If the user refreshes/closes the tab mid-typing, flush the pending save immediately.
window.addEventListener("beforeunload", () => {
  try{
    if(_fastEdit && _fastEdit.tmr){
      clearTimeout(_fastEdit.tmr);
      _fastEdit.tmr = null;
      try{
        const afterSig = projectSignature(state.project);
        if(_fastEdit.before && _fastEdit.beforeSig && afterSig !== _fastEdit.beforeSig){
          historyCommitBeforeSnapshot(_fastEdit.before);
        }
      }catch{}
      try{ upsertProject(state.project); }catch{}
...
// Also flush on mobile-style lifecycle events (more reliable than beforeunload on some browsers)
...
function clearTick(){
  (state.lastTickEls || []).forEach(elm => {
    try{ elm.classList.remove("tick");
    try{ elm.classList.remove("tickFlash"); }catch{} }catch{}
  });
  state.lastTickEls = [];
}

// ✅ BSP-style "flash" highlight (restarts every beat)
function flashTickEl(elm){
  if(!elm) return;
  try{
...
/***********************
CARD INPUT DELEGATION (HARD FIX)
- Guarantees card edits always update the real project model
- Fixes: card -> Full sync, syllable counts, notes, beats, instruments
***********************/
let _cardDelegatesInstalled = false;

function findLineById(sec, lineId){
  if(!state.project || !state.project.sections) return null;
  const arr = ensureSectionArray(sec);
  return arr.find(x => x && x.id === lineId) || null;
}

function installCardDelegates(){
  if(_cardDelegatesInstalled) return;
  _cardDelegatesInstalled = true;

  const root = el.sheetBody || document.body;
  if(!root) return;

  // INPUT (captures typing in cards even if the DOM re-renders)
  root.addEventListener("input", (e) => {
    const t = e.target;
    if(!t) return;

    // ✅ If this element is using direct per-element listeners, don't double-handle here
    if(t.dataset && t.dataset.direct === "1") return;

    // ignore Full page textarea here
    if(t.classList && t.classList.contains("fullBox")) return;

    const card = t.closest ? t.closest(".card") : null;
    if(!card) return;

    const sec = card.getAttribute("data-section") || state.currentSection;
    const lineId = card.getAttribute("data-line-id");
    if(!sec || !lineId) return;

    const line = findLineById(sec, lineId);
    if(!line) return;

    // NOTES
    if(t.classList && t.classList.contains("noteCell")){
      const n = Number(t.getAttribute("data-n"));
      if(!Number.isFinite(n)) return;

      const rawNow = String(t.value || "").trim();
      fastEdit("note", () => {
        if(!Array.isArray(line.notes)) line.notes = Array(8).fill("");
        line.notes[n] = rawNow;
        try{ t.dataset.raw = rawNow; }catch{}
        syncFullTextFromSections();
      });

      try{ updateKeyFromAllNotes(); }catch{}
      try{ updateFullIfVisible(); }catch{}
      try{ refreshDisplayedNoteCells(); }catch{}
      return;
    }

    // BEATS
    if(t.classList && t.classList.contains("beatCell")){
    ...
    // LYRICS (RAW TEXT WITH [CHORD] MARKUP)
    if(t.classList && t.classList.contains("lyrics")){
      fastEdit("lyrics", () => {
        line.rawText = String(t.value || "");

        const parsed = extractInlineChordsGlobal(line.rawText || "");
        line.lyrics = String(parsed.cleaned || "");

        const hasMarkup = /[\[\^]/.test(String(line.rawText || ""));
        if(hasMarkup && parsed.chords && parsed.chords.length){
          line.notes = buildSlotsFromInlineChordsGlobal(parsed.chords, line.lyrics);
        }
      ...
      // ✅ SRP inline-chords: keep hidden note inputs in sync so instruments + key detection work
      try{
        const noteCells = card.querySelectorAll("input.noteCell");
        if(noteCells && noteCells.length){
          for(let i=0;i<noteCells.length;i++){
            const raw = (Array.isArray(line.notes) ? String(line.notes[i] || "") : "");
            noteCells[i].dataset.raw = raw;
            noteCells[i].value = raw; // refreshDisplayedNoteCells() will transpose display
          }
        }
      }catch{}
// UI: syllables
      try{
        const pill = card.querySelector(".syllPill");
        if(pill) updateSyllPill(pill, line.lyrics || "");
      }catch{}

      // UI: beats reflect autosplit
      if(state.autoSplit){
        try{
          const beatCells = card.querySelectorAll(".beatCell");
          for(let i=0;i<4;i++){
            if(beatCells[i]) beatCells[i].value = line.beats[i] || "";
          }
        }catch{}
      }
      ...
      try{ updateKeyFromAllNotes(); }catch{}
      try{ refreshDisplayedNoteCells(); }catch{}
      try{ refreshRhymesFromActive(); }catch{}
      return;
    }
  }, true);
}
...
function buildFullPreviewText(){
  ...
  SECTIONS.filter(s => s !== "Full").forEach(sec => {
    ...
    const hasAny = arr.some(line => {
      const lyr = lineToFullTextLine(line);
...
function updateFullIfVisible(){
  // ✅ Keep Full page textarea in sync with cards (and keep Preview in sync if present)
  try{
    const ta = document.querySelector("textarea.fullBox");
    if(ta && state.project){
      const want = String(state.project.fullText || "");
      // only overwrite if it changed (prevents cursor jumps if user is actively typing in Full)
      if(document.activeElement !== ta && ta.value !== want){
        ta.value = want;
      }
    }
  }catch{}

  // Optional preview (if your HTML includes it)
  try{
    const pv = document.querySelector("textarea.fullPreview");
    if(pv){
      const wantP = String(buildFullPreviewText() || "");
      if(pv.value !== wantP) pv.value = wantP;
    }
  }catch{}
}
...
// =========================
// FullText line builder (keeps chords from note boxes)
// - If rawText contains inline chord markup ([...] or ^), use it verbatim
// - Else if note slots have chords, synthesize inline [Chord] markers over lyrics
// - Else use plain lyrics
// =========================
function synthesizeInlineFromNotesAndLyrics(notes, lyrics){
  const lyr = String(lyrics ?? "");
  const ns  = Array.isArray(notes) ? notes : [];
  const toks = Array.from({length:8}, (_,i)=> String(ns[i] ?? "").trim()).filter(Boolean);

  if(!toks.length) return lyr.trimEnd();

  // If no lyrics, just emit chords as a line
  if(!lyr.trim()){
    return toks.map(t => `[${t}]`).join(" ").trim();
  }

  const s = lyr.replace(/\s+$/g, ""); // keep user's spacing inside the line
  const L = s.length;

  // plan inserts (position -> token)
  const inserts = [];
  for(let i=0;i<8;i++){
    const tok = String(ns[i] ?? "").trim();
    if(!tok) continue;

    // proportional position across the line
    let pos = (L <= 0) ? 0 : Math.round((i / 7) * L);
    pos = clamp(pos, 0, L);

    // nudge to a word boundary to avoid splitting a word
    let p = pos;
    while(p > 0 && p < L && s[p-1] !== " " && s[p] !== " ") p--;
    while(p > 0 && s[p-1] !== " ") p--;

    inserts.push({ pos: p, tok });
  }

  if(!inserts.length) return s;

  // apply from right -> left so indexes stay valid
  ...
  for(const it of inserts){
    ...
    // spacing rules so it reads nice: "... [G]word" / "word [G] ..."
    const before = out[p-1] || "";
    const after  = out[p] || "";
    if(p > 0 && before !== " ") mark = " " + mark;
    if(p < out.length && after !== " ") mark = mark + " ";

    out = out.slice(0, p) + mark + out.slice(p);
  }

  // collapse accidental double spaces created by multiple nearby inserts
  out = out.replace(/\s{2,}/g, " ").trimEnd();
  return out;
}

function lineToFullTextLine(line){
  if(!line || typeof line !== "object") return "";
  const raw = String(line.rawText ?? "");
  const hasMarkup = /[\[\^]/.test(raw);
  if(hasMarkup) return raw.trim();

  const lyr = String(line.lyrics ?? "").trimEnd();
  const ns  = Array.isArray(line.notes) ? line.notes : [];
  const hasNotes = ns.some(x => String(x ?? "").trim());
  if(hasNotes){
    return synthesizeInlineFromNotesAndLyrics(ns, lyr).trim();
  }
  return lyr.trim();
}
...
function renderSheetTitleBar(){
  ...
  try{ el.sheetTitle.classList.add("sheetTitleRow"); }catch{}
  ...
  // Title input only for EXTRA pages (base pages are fixed names)
  let titleInput = null;
  const _isExtraPage = (state.currentSection !== "Full") && Array.isArray(state.project?.extraSections) && state.project.extraSections.includes(state.currentSection);
  if(_isExtraPage){
  ...
  // - On FULL: + enables next hidden page (or creates EXTRA), ✕ deletes the last visible page (prefers EXTRA)
  // - On a page: + adds next page after this one, ✕ deletes this page
  {
    ...
    delBtn.textContent = "✕";
    delBtn.title = (state.currentSection === "Full") ? "Delete last page" : "Delete this page";
    delBtn.style.width = "38px";
    delBtn.style.height = "34px";
    delBtn.style.borderRadius = "999px";
    delBtn.style.fontWeight = "1100";

    function lastVisiblePageExcludingFull(){
      const pages = getVisibleSectionPages().filter(s => s !== "Full");
      if(!pages.length) return null;

      // Prefer last EXTRA page
      const extras = Array.isArray(state.project?.extraSections) ? state.project.extraSections : [];
      for(let i=pages.length-1;i>=0;i--){
        if(extras.includes(pages[i])) return pages[i];
      }
      return pages[pages.length-1];
    }

    addBtn.addEventListener("click", () => {
      // FULL page: if no pages are visible yet, enable INTRO first.
      const visibleNonFull = getVisibleSectionPages().filter(s => s !== "Full");
      const noneVisible = (state.currentSection === "Full") && (visibleNonFull.length === 0);
      ...
      editProject("addPage", () => {
        if(noneVisible){
          ...
          // enable after last visible; page view: enable after current
          const fromSec = (state.currentSection === "Full")
            ? (lastVisiblePageExcludingFull() || "INTRO")
            : state.currentSection;

          const next = findNextSectionToEnable(fromSec);
    ...
    delBtn.addEventListener("click", () => {
      if(state.currentSection === "Full"){
        const victim = lastVisiblePageExcludingFull();
        if(!victim) return;

        if(!confirm(`Delete page "${victim}"? This clears its cards and hides the page.`)) return;

        editProject("deletePage", () => {
          deleteSectionPage(victim);
          syncFullTextFromSections();
        });
    ...
    // Only show ✕ when there's something to delete (avoid confusion on empty projects)
    if(state.currentSection !== "Full" || lastVisiblePageExcludingFull()){
...
if(state.currentSection === "Full"){
  el.sheetHint.textContent = 'Paste + edit. Use headings: INTRO, VERSE 1, CHORUS 1, VERSE 2, CHORUS 2, VERSE 3, CHORUS 3, BRIDGE. Blank line = next card.';
...
// ✅ Needed for stable 2-way sync (DOM -> project data)
if(!line.id) line.id = uuid();
card.setAttribute("data-section", state.currentSection);
card.setAttribute("data-sec", state.currentSection);
...
card.appendChild(delBtn);
 ...
 updateSyllPill(syll, line.lyrics || "");
    ...
    for(let i=0;i<8;i++){
      ...
      inp.setAttribute("data-n", String(i));

      const raw = String(line.notes[i] || "").trim();
      inp.dataset.raw = raw;
      inp.dataset.direct = "1";
      ...
      inp.addEventListener("input", () => {
        ...
        // ✅ FAST: update model immediately, debounce history + save
        fastEdit("note", () => {
          if(!Array.isArray(line.notes)) line.notes = Array(8).fill("");
          line.notes[i] = rawNow;
          syncFullTextFromSections();
      ...
      editProject("note", () => {
  // line.notes[i] already set above
  syncFullTextFromSections();
});
        updateKeyFromAllNotes();
        updateFullIfVisible();
    ...
    lyr.dataset.direct = "1";
    lyr.placeholder = "Type lyrics (AutoSplit on)…";
    lyr.value = (line.rawText != null && String(line.rawText).length) ? line.rawText : (line.lyrics || "");
    ...
    for(let i=0;i<4;i++){
      ...
      inp.setAttribute("data-b", String(i));
      inp.dataset.direct = "1";
  ...
  fastEdit("lyrics", () => {
    // 1) store raw text (may include inline chords like [Bm7])
    line.rawText = lyr.value;

    // 1B) parse inline chords -> cleaned lyrics + auto-fill note slots
    const parsed = extractInlineChordsGlobal(line.rawText || "");
    line.lyrics = String(parsed.cleaned || "");
    ...
    // keep syll pill accurate (uses lyrics only)
    try{ updateSyllPill(syll, line.lyrics || ""); }catch{}
    ...
    if(state.autoSplit && String(line.rawText || "").includes("\n")){
      const parts = String(line.rawText || "").split("\n");
      const first = parts.shift() || "";
      line.rawText = first;
      const parsedFirst = extractInlineChordsGlobal(line.rawText || "");
      line.lyrics = String(parsedFirst.cleaned || "");
      const hasMarkupFirst = /[\[\^]/.test(String(line.rawText || ""));
      if(hasMarkupFirst && parsedFirst.chords && parsedFirst.chords.length){
        line.notes = buildSlotsFromInlineChordsGlobal(parsedFirst.chords, line.lyrics);
      }
      // update beats for trimmed first line
      if(state.autoSplit) applyBeatsFromLyrics(line);

      const more = parts.map(x=>String(x||"")).filter(x=>x.trim().length);
      if(more.length){
        let insertAt = idx + 1;
        more.forEach(txt => {
          const nl = newLine();
          nl.rawText = txt;
          const parsedNL = extractInlineChordsGlobal(nl.rawText || "");
          nl.lyrics = String(parsedNL.cleaned || "");
          const hasMarkupNL = /[\[\^]/.test(String(nl.rawText || ""));
          if(hasMarkupNL && parsedNL.chords && parsedNL.chords.length){
            nl.notes = buildSlotsFromInlineChordsGlobal(parsedNL.chords, nl.lyrics);
          }
...
function findNextSectionToEnable(fromSec){
  ...
  const _idx = base.indexOf(fromSec);
  const curIdx = (_idx < 0) ? -1 : _idx;
...
if(el.capoInput){
  ...
  // ✅ Robust 2-way sync: delegated input handler (catches any card inputs even if re-rendered)
  if(!state._delegatedCardInputWired && el.sheetBody){
    state._delegatedCardInputWired = true;
    el.sheetBody.addEventListener("input", (e)=>{
      ...
      let sec = card.getAttribute("data-sec") || card.getAttribute("data-section");
      const idxStr = card.getAttribute("data-idx");
      let idx = idxStr != null ? parseInt(idxStr, 10) : NaN;

      // Fallback: if idx missing, use line-id to locate the card in the section array
      if(!Number.isFinite(idx)){
        const lid = card.getAttribute("data-line-id");
        const arr2 = state.project?.sections?.[sec];
        if(Array.isArray(arr2) && lid){
          const fi = arr2.findIndex(x => String(x?.id||"") === String(lid));
          if(fi >= 0) idx = fi;
        }
      }

      if(!sec || !Number.isFinite(idx)) return;
      ...
      if(t.classList && t.classList.contains("beatCell")){
        ...
        const bi = b != null ? parseInt(b, 10) : NaN;
        if(!Number.isFinite(bi)) return;
        editProject("beat_deleg", ()=>{
          if(!Array.isArray(line.beats)) line.beats = Array(4).fill("");
          line.beats[bi] = String(t.value || "").trim();
          syncFullTextFromSections();
        });
        updateFullIfVisible();
        return;
      ...
      if(t.classList && t.classList.contains("noteCell")){
        ...
        const ni = n != null ? parseInt(n, 10) : NaN;
        if(!Number.isFinite(ni)) return;
        const rawNow = String(t.value ?? "").trim();
         t.dataset.raw = rawNow;
        editProject("note_deleg", ()=>{
          if(!Array.isArray(line.notes)) line.notes = Array(8).fill("");
          line.notes[ni] = rawNow;
          syncFullTextFromSections();
        });
        updateKeyFromAllNotes();
        updateFullIfVisible();
