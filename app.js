...
if(state.currentSection === "Full"){
  ...
  // ✅ Break title pills (visual page breaks) — synced 2-ways with page headers + Full text
  const pillsWrap = document.createElement("div");
  pillsWrap.className = "fullBreakPills";

  const pillPages = getVisibleSectionPages().filter(s => s !== "Full");
  pillPages.forEach((sec) => {
    ...
    inp.value = (state.project.sectionTitles && state.project.sectionTitles[sec]) ? String(state.project.sectionTitles[sec] || "") : "";
