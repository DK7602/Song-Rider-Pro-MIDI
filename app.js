...
const state = {
  ...
  capo: 0,
  autoSplit: true,
  fullEditorMode: "full", // "full" | "music"
...
function htmlToImportText(html){
  ...
  div.innerHTML = raw
    .replace(/<\s*br\s*\/?\s*>/gi, "\n")
    .replace(/<\/(p|div|section|article|li|ul|ol|h1|h2|h3|h4|h5|h6|blockquote|pre)\s*>/gi, "\n")
    .replace(/<\s*(p|div|section|article|li|ul|ol|h1|h2|h3|h4|h5|h6|blockquote|pre)[^>]*>/gi, "");
  let txt = String(div.textContent || div.innerText || "");
  txt = txt.replace(/\u00a0/g, " ");
  txt = txt.replace(/\r\n/g, "\n").replace(/\r/g, "\n");
  txt = txt.replace(/\n{3,}/g, "\n\n");
  return txt.trim();
...
function parseMusicChordTextareaToNotes(chordText, lyricText){
  const raw = String(chordText || "").replace(/\u00A0/g, " ").trim();
  const lyric = String(lyricText || "");
  const slots = Array(8).fill("");
  if(!raw) return slots;

  const CHORD_TOKEN_RE = /^([A-G](?:#|b)?)(?:(?:maj|min|m|dim|aug|\+|sus|add)?(?:2|4|5|6|7|9|11|13)?(?:maj7|M7|m7|m9|m11|m13|sus2|sus4|add9|add11|add13|dim7|hdim7|m7b5)?(?:b5|#5|b9|#9|b11|#11|b13|#13)?)?(?:\/[A-G](?:#|b)?)?$|^N\.?C\.?$/i;
  const cleanTok = (tok) => String(tok || "")
    .trim()
    .replace(/^[\[\(\{]+/, "")
    .replace(/[\]\)\}]+$/, "")
    .replace(/♯/g, "#")
  ...
  const parsePositional = (tok) => {
    ...
    if(!m) return null;
    const chord = String(m[2] || "").trim();
    if(!CHORD_TOKEN_RE.test(chord)) return null;
    return { slot: Math.max(0, Math.min(7, parseInt(m[1], 10) - 1)), chord };
  };
  ...
  while(i < raw.length){
    ...
    if(!tok) continue;
    const pos = parsePositional(tok);
    if(pos){
      if(!slots[pos.slot]) slots[pos.slot] = pos.chord;
      else slots[pos.slot] = (slots[pos.slot] + " " + pos.chord).trim();
      continue;
    }
    const cleaned = cleanTok(tok);
    if(CHORD_TOKEN_RE.test(cleaned)) parts.push({ tok: cleaned, col: start });
  }

  if(!parts.length) return slots;

  const denom = Math.max(1, raw.length - 1);
  const wordStarts = [];
  const reWord = /\S+/g;
  let m;
  while((m = reWord.exec(lyric))){ wordStarts.push(m.index); }
  const looksCollapsed = !/  +/.test(raw) && parts.length >= 2 && wordStarts.length >= 2;

  parts.forEach((item, idx) => {
    let slot = 0;
    if(looksCollapsed){
      const wIdx = (parts.length === 1) ? 0 : Math.round(idx * (wordStarts.length - 1) / (parts.length - 1));
      const col = wordStarts[Math.max(0, Math.min(wordStarts.length - 1, wIdx))] || 0;
      const denomL = Math.max(1, lyric.length - 1);
      slot = Math.max(0, Math.min(7, Math.round((col / denomL) * 7)));
    }else{
      slot = parts.length > 8
        ? Math.max(0, Math.min(7, Math.floor(idx * 8 / parts.length)))
        : Math.max(0, Math.min(7, Math.round((item.col / denom) * 7)));
    }
    if(!slots[slot]) slots[slot] = item.tok;
    else slots[slot] = (slots[slot] + " " + item.tok).trim();
  });
...
async function importHtmlToFull(html){
  const txt = htmlToImportText(html);
  if(!txt){
    alert("No readable text found in that HTML.");
    return;
  }
...
function renderMusicView(){
  const host = (el.sheetInner || el.sheetBody);
  if(!host || !state.project) return;
  host.innerHTML = "";

  const wrap = document.createElement("div");
  wrap.className = "musicView";

  const importCard = document.createElement("div");
  importCard.className = "musicRow";
  const importTop = document.createElement("div");
  importTop.className = "musicRowTop";
  const importLabel = document.createElement("div");
  importLabel.style.fontWeight = "1000";
  importLabel.textContent = "Music import";
  const importTools = document.createElement("div");
  importTools.className = "musicRowTools";

  const uploadBtn = document.createElement("button");
  uploadBtn.type = "button";
  uploadBtn.className = "musicMiniBtn";
  uploadBtn.textContent = "⬆";
  uploadBtn.title = "Upload PDF, TXT, or HTML";
  uploadBtn.addEventListener("click", () => {
    const inp = document.createElement("input");
    inp.type = "file";
    inp.accept = ".pdf,.txt,.html,.htm,text/plain,text/html,application/pdf";
    inp.onchange = async () => {
      ...
      const isHtml = type === "text/html" || name.endsWith('.html') || name.endsWith('.htm');
      if(isPdf){
        await importPdfToFull(file);
      }else if(isHtml){
        await importHtmlToFull(await file.text());
      }else{
        await importTextToFull(await file.text());
      }
      state.fullEditorMode = "music";
      renderSheet();
    };
    inp.click();
  });
  ...
  clipBtn.type = "button";
  clipBtn.className = "musicMiniBtn";
  clipBtn.textContent = "📋";
  clipBtn.title = "Paste from clipboard";

  const applyBtn = document.createElement("button");
  applyBtn.type = "button";
  applyBtn.className = "musicMiniBtn";
  applyBtn.textContent = "↵";
  applyBtn.title = "Import the pasted song below";

  importTools.appendChild(uploadBtn);
  importTools.appendChild(clipBtn);
  importTools.appendChild(applyBtn);
  importTop.appendChild(importLabel);
  importTop.appendChild(importTools);
  importCard.appendChild(importTop);

  const importBox = document.createElement("textarea");
  importBox.className = "musicLyricLine";
  importBox.placeholder = "Paste lyrics, chord sheets, or raw HTML here. Then tap ↵ to import into Music View / Full Song / Cards.";
  importBox.style.minHeight = "110px";
  importCard.appendChild(importBox);

  clipBtn.addEventListener("click", async () => {
    try{
      const txt = await navigator.clipboard.readText();
      if(txt) importBox.value = txt;
      importBox.focus();
    }catch{
      importBox.focus();
      alert("Paste into the box, then tap ↵.");
    }
  });

  importBox.addEventListener("paste", async (e) => {
    try{
      const items = e.clipboardData && e.clipboardData.items ? Array.from(e.clipboardData.items) : [];
      const pdfItem = items.find(it => String(it.type || '').toLowerCase() === 'application/pdf');
      if(pdfItem && pdfItem.getAsFile){
        e.preventDefault();
  ...
  applyBtn.addEventListener("click", async () => {
    ...
    if(!raw){
      alert('Paste a song, chord sheet, PDF text, or HTML first.');
      return;
    }
    if(/<\s*[a-z][\s\S]*>/i.test(raw)) await importHtmlToFull(raw);
    else await importTextToFull(raw);
    state.fullEditorMode = 'music';
    renderSheet();
  });
  ...
  const visible = getVisibleSectionPages().filter(s => s !== "Full");
  visible.forEach(sec => {
    ...
    head.className = "musicSectionHead";
    const l1 = document.createElement("div");
    l1.className = "musicSectionLine";
    const l2 = document.createElement("div");
    l2.className = "musicSectionLine";
    const pill = document.createElement("input");
    pill.type = "text";
    pill.className = "songPartPill musicSectionPill";
    pill.placeholder = "Song Part";
    pill.value = String((state.project.sectionTitles && state.project.sectionTitles[sec]) || "");
    pill.addEventListener("input", () => {
      editProject("musicSectionTitle", () => {
        ensurePageMeta();
        const v = String(pill.value || '').trim();
    ...
    const lines = arr.length ? arr : [newLine()];
    lines.forEach((line, idx) => {
      const row = document.createElement("div");
      row.className = "musicRow";
      const top = document.createElement("div");
      top.className = "musicRowTop";
      const badge = document.createElement("div");
      badge.className = "musicRowBadge";
      badge.textContent = String(idx + 1);
      const tools = document.createElement("div");
      tools.className = "musicRowTools";

      const copyBtn = document.createElement("button");
      copyBtn.type = "button";
      copyBtn.className = "musicMiniBtn";
      copyBtn.textContent = "⧉";
      copyBtn.title = "Copy this chord row";
      copyBtn.addEventListener("click", () => { copyChordRowFromLine(line); });

      const pasteBtn = document.createElement("button");
      pasteBtn.type = "button";
      pasteBtn.className = "musicMiniBtn";
      pasteBtn.textContent = "📋";
      pasteBtn.title = "Paste copied chord row";
      pasteBtn.addEventListener("click", () => {
        if(!hasChordClipboard()) return;
        editProject("musicPasteChordRow", () => {
      ...
      addBtn.type = "button";
      addBtn.className = "musicMiniBtn";
      addBtn.textContent = "+";
      addBtn.title = "Add a row below";
      addBtn.addEventListener("click", () => {
        editProject("musicAddRow", () => {
          const target = Array.isArray(state.project.sections?.[sec]) ? state.project.sections[sec] : (state.project.sections[sec] = []);
          target.splice(idx + 1, 0, newLine());
          syncFullTextFromSections();
        });
        renderSheet();
      });
      ...
      delBtn.type = "button";
      delBtn.className = "musicMiniBtn";
      delBtn.textContent = "×";
      delBtn.title = "Delete this row";
      delBtn.addEventListener("click", () => {
        editProject("musicDeleteRow", () => {
          const target = Array.isArray(state.project.sections?.[sec]) ? state.project.sections[sec] : [];
          if(target.length <= 1){
            target[0] = newLine();
          }else{
            target.splice(idx, 1);
      ...
      const chordTa = document.createElement("textarea");
      chordTa.className = "musicChordLine";
      chordTa.placeholder = "Chord row — paste or type chords here";
      chordTa.value = getLineNotes8(line).filter(Boolean).join('    ');
      autosizeTextarea(chordTa);
      let holdTimer = null;
      const clearHold = () => { if(holdTimer){ clearTimeout(holdTimer); holdTimer = null; } };
      chordTa.addEventListener('pointerdown', () => {
        clearHold();
        holdTimer = setTimeout(() => {
          copyChordRowFromLine(line);
          try{ chordTa.classList.add('musicChordCopied'); setTimeout(()=>chordTa.classList.remove('musicChordCopied'), 260); }catch{}
        }, 380);
      });
      chordTa.addEventListener('pointermove', clearHold, {passive:true});
      chordTa.addEventListener('pointerup', clearHold, {passive:true});
      chordTa.addEventListener('pointercancel', clearHold, {passive:true});
      chordTa.addEventListener('focus', () => {
        if(!String(chordTa.value || '').trim() && hasChordClipboard()){
          editProject('musicTapPasteChordRow', () => {
            pasteChordRowToLine(line);
            syncFullTextFromSections();
          });
          chordTa.value = getLineNotes8(line).filter(Boolean).join('    ');
      ...
      chordTa.addEventListener('input', () => {
        autosizeTextarea(chordTa);
        editProject('musicChordEdit', () => {
          setLineNotes8(line, parseMusicChordTextareaToNotes(chordTa.value, lyricTa.value));
          syncFullTextFromSections();
        });
        updateKeyFromAllNotes();
      });
      row.appendChild(chordTa);

      const lyricTa = document.createElement("textarea");
      lyricTa.className = "musicLyricLine";
      lyricTa.placeholder = "Lyrics";
      lyricTa.value = String(line.lyrics || '');
      autosizeTextarea(lyricTa);
      lyricTa.addEventListener('input', () => {
        autosizeTextarea(lyricTa);
        editProject('musicLyricEdit', () => {
          line.lyrics = String(lyricTa.value || '');
...
function renderSheetTitleBar(){
  ...
  if(state.currentSection === "Full"){
    ...
    musicBtn.className = "btn secondary";
    musicBtn.textContent = "♫";
    musicBtn.title = (state.fullEditorMode === "music") ? "Switch to Full Song View" : "Open Music View";
    musicBtn.style.width = "38px";
    musicBtn.style.height = "34px";
    musicBtn.style.borderRadius = "999px";
    musicBtn.style.fontWeight = "1100";
    if(state.fullEditorMode === "music"){
      musicBtn.style.background = "#111";
      musicBtn.style.color = "#fff";
      musicBtn.style.borderColor = "#111";
    }
    musicBtn.addEventListener("click", () => {
      state.fullEditorMode = (state.fullEditorMode === "music") ? "full" : "music";
      renderSheetTitleBar();
      renderSheet();
    });
...
if(state.currentSection === "Full"){
  if(state.fullEditorMode === "music" && !state.fullPerfMode){
...
async function uploadAudioFile(){
  ...
  const inp = document.createElement("input");
  inp.type = "file";
  inp.accept = "audio/*,application/pdf,text/plain,text/html,.pdf,.txt,.html,.htm";
  inp.click();
  ...
  inp.onchange = async () => {
    ...
    alert("Unsupported file. Upload an audio file (.mp3/.wav/.m4a), a .pdf, .txt, or .html file.");
  };
}
