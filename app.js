...
function playSingleNoteForInstrument(rawChord, durMs){
  ...
  if(state.instrument === "acoustic"){
    const n = acousticPluckSafe(ctx, f, durMs, 0.92);
    n.out.connect(getOutNode());
    scheduleCleanup([n.out], durMs + 2800);
...
function playAcousticChord(ch, durMs, fracMul=1){
  ...
  const bpm = clamp(state.bpm||95, 40, 220);
  const strumMs = clamp(Math.round(23_000 / bpm), 10, 26);

  order.forEach((srcIdx, i) => {
    const f = freqs[srcIdx];
    const delayMs = getHumanizedMs(i * strumMs, 4);

    setTimeout(() => {
      if(token !== state.audioToken) return;
      if(!state.instrumentOn) return;

      const vel = clamp(0.94 - i*0.06 + ((Math.random()*2-1)*0.035), 0.55, 0.98);
      const n = acousticPluckSafe(ctx, f, durMs, vel);
      n.out.connect(dryBus);
      scheduleCleanup([n.out], durMs + 2600);
    }, Math.max(0, delayMs));
  });

  scheduleCleanup([dryBus, wet, ...room.nodes], durMs + 3200);
}
