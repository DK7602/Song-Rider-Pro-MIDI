...
function normalizeProject(p){
  ...
  if(!p.midi || typeof p.midi !== "object") p.midi = null;
...
async function uploadAudioFile(){
  ...
  const inp = document.createElement("input");
  inp.type = "file";
  inp.accept = "audio/*,audio/midi,application/x-midi,application/midi,application/octet-stream,application/pdf,text/plain,.pdf,.txt,.mid,.midi";
  inp.click();
  ...
  inp.onchange = async () => {
    ...
    const isMidi = type === "audio/midi" || type === "application/midi" || type === "application/x-midi" || name.endsWith(".mid") || name.endsWith(".midi");
    ...
    alert("Unsupported file. Upload an audio file (.mp3/.wav/.m4a), a .pdf, a .txt, or a MIDI file (.mid/.midi).");
  };
}
...
// - Works with any DAW that can export MIDI
// - Uses tempo, markers, and (if present) lyric meta events
// - Generates chord slots by “listening” to held notes at each 1/8 step
// ----------------------
async function importMidiToFull(file){
  if(!state.project) return;
  ...
  let midi;
  try{
    midi = parseMidiBuffer(buf);
  }catch(e){
    console.error(e);
    alert("That MIDI file could not be parsed.");
    return;
  }

  // pick initial BPM from tempo map (default 120 if missing)
  const bpm = clamp(Math.round(midi.bpm || 120), 40, 240);

  // Build SRP Full text (headings + chord-line + lyric-line blocks)
  const fullText = buildFullTextFromMidi(midi);

  if(!String(fullText || "").trim()){
    alert("MIDI imported, but no notes/markers were found to build a song sheet.");
    return;
  }

  editProject("importMIDI", () => {
    // store lightweight MIDI metadata (NOT full raw event list)
    state.project.midi = {
      sourceName: String(file?.name || "midi").slice(0, 160),
      ppq: midi.ppq || 480,
      bpm,
      timeSig: midi.timeSig || { num:4, den:4 },
      markers: (midi.markers || []).slice(0, 200).map(m => ({ tick: m.tick|0, text: String(m.text||"").slice(0,80) })),
      hasLyrics: !!(midi.lyrics && midi.lyrics.length)
    };
    ...
    state.project.fullText = fullText;
    applyFullTextToProjectSections(state.project.fullText || "");
  });
...
- Supports: tempo, time sig, markers, lyrics, note on/off
***********************/
function parseMidiBuffer(arrayBuffer){
  ...
  function u16(){ const v = dv.getUint16(off); off += 2; return v; }
  function u32(){ const v = dv.getUint32(off); off += 4; return v; }
  function str4(){
    const s = String.fromCharCode(u8(),u8(),u8(),u8());
    return s;
  }
  function readBytes(n){
    const a = new Uint8Array(arrayBuffer, off, n);
    off += n;
    return a;
  }
  function vlq(){
  ...
  // header
  if(str4() !== "MThd") throw new Error("Bad MIDI header");
  const headerLen = u32();
  const fmt = u16();
  const ntr = u16();
  const div = u16();
  off += Math.max(0, headerLen - 6);

  const ppq = (div & 0x8000) ? 480 : div; // if SMPTE, fall back
  const tracks = [];

  const tempoMap = [{ tick:0, usPerQ: 500000 }]; // default 120 BPM
  ...
  for(let t=0; t<ntr; t++){
    if(str4() !== "MTrk") throw new Error("Bad track header");
    const trkLen = u32();
    ...
    // active notes per channel+pitch -> startTick
    ...
    while(off < end){
      ...
      if(status === 0xFF){
        ...
        if(type === 0x51 && len === 3){
          const usPerQ = (data[0]<<16) | (data[1]<<8) | data[2];
          tempoMap.push({ tick, usPerQ });
      ...
      const evt = status & 0xF0;
      const ch = status & 0x0F;

      if(evt === 0x80 || evt === 0x90){
        const pitch = u8();
        const vel = u8();
        const isOn = (evt === 0x90 && vel > 0);
        ...
        if(isOn){
          ...
          if(st !== undefined){
            notes.push({ start: st, end: tick, pitch, ch });
            active.delete(key);
          }
        }
        continue;
      ...
      // other channel messages consume bytes
      if(evt === 0xA0 || evt === 0xB0 || evt === 0xE0){ u8(); u8(); continue; }
      if(evt === 0xC0 || evt === 0xD0){ u8(); continue; }

      // unknown - bail out safely
      break;
    }
  ...
  const usPerQ0 = tempoMap[0]?.usPerQ || 500000;
  const bpm0 = Math.round(60000000 / usPerQ0);

  const maxTick = Math.max(0, ...notes.map(n=>n.end||0), ...markers.map(m=>m.tick||0), ...lyrics.map(l=>l.tick||0));

  return { fmt, ppq, tempoMap, bpm:bpm0, timeSig, markers, lyrics, notes, maxTick };
}

/***********************
Build Full text from MIDI
***********************/
function buildFullTextFromMidi(midi){
  const ppq = midi.ppq || 480;
  const ts = midi.timeSig || { num:4, den:4 };

  // SRP assumes 4/4 for “8 boxes = one bar (8 eighth notes)”
  // If DAW time sig isn't 4/4, we still map to SRP bars using num beats (best-effort).
  const beatsPerBar = clamp(ts.num || 4, 1, 12);
  const ticksPerBeat = ppq; // quarter note
  const ticksPerBar = ticksPerBeat * beatsPerBar;
  const ticksPerEighth = Math.max(1, Math.round(ppq / 2)); // 1/8 note in 4/4 feel

  // Determine section markers (best-effort)
  const rawMarks = Array.isArray(midi.markers) ? midi.markers : [];
  const secMarks = [];
  for(const m of rawMarks){
    const txt = String(m.text || "").trim();
    const sec = isSectionHeadingLine(txt);
    if(sec) secMarks.push({ tick: m.tick|0, sec });
  }
  secMarks.sort((a,b)=>a.tick-b.tick);

  function sectionAtTick(tick){
    let cur = "VERSE 1";
    for(const m of secMarks){
      if(m.tick <= tick) cur = m.sec;
      else break;
    }
    return cur;
  }

  // Pre-index lyric events by bar
  const lyricsByBar = new Map();
  if(Array.isArray(midi.lyrics)){
    for(const ev of midi.lyrics){
      const bar = Math.max(0, Math.floor((ev.tick|0) / ticksPerBar));
      const s = String(ev.text || "").replace(/\s+/g," ").trim();
      if(!s) continue;
      const prev = lyricsByBar.get(bar) || "";
      lyricsByBar.set(bar, (prev ? (prev + " " + s) : s).trim());
    }
  }

  // Build chord grid for each bar (8 slots)
  const nBars = Math.max(1, Math.ceil((midi.maxTick || 0) / ticksPerBar));
  ...
  for(let bar=0; bar<nBars; bar++){
    ...
    const sec = sectionAtTick(barStart);
    ...
    // chord per eighth slot
    let lastChord = "";
    for(let s=0; s<8; s++){
      const t = barStart + s * ticksPerEighth;
      const pcs = getPitchClassesAtTick(midi.notes, t);
      const chord = detectChordName(pcs);
    ...
    const chordLine = slotsToChordLine(slots);
    const lyricLine = lyricsByBar.get(bar) || ""; // blank if none

    // only add the bar if it has something (chords or lyrics)
    const hasChords = slots.some(x => String(x||"").trim());
    const hasLyric = String(lyricLine||"").trim().length > 0;

    if(hasChords || hasLyric){
      out.push(chordLine);
      out.push(lyricLine);
      out.push(""); // blank line = new card
    }
  }
...
function getPitchClassesAtTick(notes, tick){
  const pcs = new Set();
  if(!Array.isArray(notes) || notes.length === 0) return pcs;

  // small look window so short notes still register
  const t0 = tick;
  const t1 = tick + 1;
  ...
  for(const n of notes){
    if(!n) continue;
    const st = n.start|0;
    const en = (n.end|0) || (st+1);
    if(en <= t0) continue;
    if(st >= t1) continue;
    pcs.add((n.pitch|0) % 12);
    if(pcs.size >= 8) break;
  }
  return pcs;
}
...
function detectChordName(pcs){
  if(!pcs || pcs.size === 0) return "";
  const set = pcs;

  const templates = [
    { name:"", ints:[0,4,7] },        // maj
    { name:"m", ints:[0,3,7] },       // min
    { name:"7", ints:[0,4,7,10] },    // dom7
    { name:"maj7", ints:[0,4,7,11] }, // maj7
    { name:"m7", ints:[0,3,7,10] },   // min7
  ...
  let best = { score:-999, root:0, name:"" };

  for(let root=0; root<12; root++){
    for(const tpl of templates){
      let hit = 0;
      let miss = 0;
      for(const iv of tpl.ints){
        const pc = (root + iv) % 12;
        if(set.has(pc)) hit++;
        else miss++;
      }
      // extra tones are OK but reduce score slightly
      const extra = Math.max(0, set.size - tpl.ints.length);

      const score = hit*2 - miss*2 - extra*0.25;

      if(score > best.score){
        best = { score, root, name: tpl.name };
      }
    }
  }

  // require at least 2 chord tones to avoid random labels
  const minHit = 2;
  const chosen = best;
  const rootName = PC_NAMES[chosen.root] || "C";

  // quick sanity: ensure we have at least 2 tones from that chord
  // (recalculate hits)
  let hits = 0;
  const tpl = templates.find(t => t.name === chosen.name) || templates[0];
  for(const iv of tpl.ints){
    if(set.has((chosen.root + iv) % 12)) hits++;
  }
  if(hits < minHit) return "";

  return rootName + chosen.name;
}

// Turn 8 slot strings into a chord line with visible spacing so SRP parser keeps placement
function slotsToChordLine(slots){
  const cellW = 8; // characters per slot
  let line = Array(cellW*8).fill(" ");
  for(let i=0;i<8;i++){
    const tok = String(slots[i] || "").trim();
    if(!tok) continue;
    const at = i * cellW;
    for(let k=0;k<tok.length && (at+k)<line.length;k++){
      line[at+k] = tok[k];
    }
  }
