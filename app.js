...
function parseFullTextToSectionCards(fullText){
  ...
  function extractChordTokensWithCols(chordLineRaw){
    ...
    while(i < chordLine.length){
      ...
      if(pos){
        toks.push({ tok: pos.chord, col: start, forcedSlot: pos.slot });
      }else if(isChordToken(tok)){
        toks.push({ tok, col: start, forcedSlot: null });
      }
    }
    return toks;
  }
  ...
  function buildSlotsFromAlignedChordLine(chordLineRaw, lyricLineRaw){
    ...
    const toks = extractChordTokensWithCols(chordLineRaw);
    if(!toks.length) return slots;

    toks.forEach((item) => {
      if(Number.isInteger(item.forcedSlot)){
        const s = clamp(item.forcedSlot, 0, 7);
        if(!slots[s]) slots[s] = item.tok;
        else slots[s] = (slots[s] + " " + item.tok).trim();
      }
    });

    const freeToks = toks.filter(item => !Number.isInteger(item.forcedSlot));
    if(!freeToks.length) return slots;

    let looksCollapsed = !/  +/.test(chordLine) && freeToks.length >= 2;

    const denom = Math.max(1, chordLine.length - 1);
    const n = freeToks.length;
  ...
  function getLastBucketCard(sec){
    const arr = buckets[sec];
    return Array.isArray(arr) && arr.length ? arr[arr.length - 1] : null;
  }

  function mergeChordSlotsIntoCard(card, slots, opts={}){
    if(!card || !Array.isArray(card.chords) || !Array.isArray(slots)) return;
    const preferEnd = !!opts.preferEnd;
    const compact = slots.map(v => String(v || "").trim());
    const filled = compact.map((v,i) => ({ v, i })).filter(x => x.v);
    if(!filled.length) return;

    if(preferEnd && filled.length === 1){
      const only = filled[0].v;
      for(let i = card.chords.length - 1; i >= 0; i--){
        if(!String(card.chords[i] || "").trim()){
          card.chords[i] = only;
          return;
        }
      }
      card.chords[card.chords.length - 1] = only;
      return;
    }

    filled.forEach(({ v, i }) => {
      if(!String(card.chords[i] || "").trim()) card.chords[i] = v;
      else if(!String(card.chords[i] || "").includes(v)) card.chords[i] = (String(card.chords[i] || "") + " " + v).trim();
    });
  }

  function shouldMergeLyricContinuation(sec, lyricRaw, opts={}){
    const prev = getLastBucketCard(sec);
    if(!prev) return false;
    const lyric = String(lyricRaw || "").replace(/\s+/g, " ").trim();
    if(!lyric) return false;
    if(!String(prev.lyrics || "").trim()) return false;

    const hasPendingChordLine = !!opts.pendingChordLine;
    const chordTokens = Array.isArray(opts.pendingChordTokens) ? opts.pendingChordTokens.filter(Boolean) : [];
    const hasInlineChords = !!opts.hasInlineChords;
    const prevEndsHard = /[.!?;:]$/.test(String(prev.lyrics || "").trim());
    const startsSoft = /^[a-z0-9'"\)\]\},]/.test(lyric);
    const shortTail = lyric.length <= 18 || lyric.split(/\s+/).filter(Boolean).length <= 3;

    if(hasInlineChords) return false;

    if(hasPendingChordLine){
      if(chordTokens.length === 1 && (startsSoft || shortTail) && !prevEndsHard) return true;
      return false;
    }

    return (startsSoft || shortTail) && !prevEndsHard;
  }

  function appendToPreviousCard(sec, lyricsRaw, slots, opts={}){
    const prev = getLastBucketCard(sec);
    if(!prev) return false;
    const lyric = String(lyricsRaw || "").replace(/\s+/g, " ").trim();
    if(lyric){
      prev.lyrics = String(prev.lyrics || "").trim();
      prev.lyrics = prev.lyrics ? (prev.lyrics + " " + lyric).replace(/\s+/g, " ").trim() : lyric;
    }
    if(Array.isArray(slots)) mergeChordSlotsIntoCard(prev, slots, opts);
    return true;
  }
  ...
  if(hasBreaks){
    ...
    for(let i=0; i<lines.length; i++){
      ...
      const pendingChordTokens = pendingChordLine ? extractChordTokensWithCols(pendingChordLine).map(x => x.tok) : [];
      if(pendingChordLine){
      ...
      if(shouldMergeLyricContinuation(cur, cleaned, {
        pendingChordLine: pendingChordTokens.length > 0,
        pendingChordTokens,
        hasInlineChords: inlineChords.length > 0
      })){
        appendToPreviousCard(cur, cleaned, slots, { preferEnd: pendingChordTokens.length === 1 });
      }else{
