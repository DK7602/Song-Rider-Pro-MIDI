...
function getFullPreviewExportSections(){
  const raw = pruneLegacyTrailingBlankFullBlocks(state.project?.fullText || "");
  ...
  const flush = () => {
    ...
    if(cur.title || cur.body.some(line => String(line || "").trim())) sections.push(cur);
    cur = null;
  };
  ...
  const visibleKeys = getAllSectionOrder().filter(s => s !== "Full");
  const semis = getTransposeSemis() || 0;

  return sections.map((sec, idx) => {
    const fallbackKey = visibleKeys[idx] || "";
    const fallbackTitle = (state.project?.sectionTitles && fallbackKey)
      ? String(state.project.sectionTitles[fallbackKey] || "").trim()
      : "";
    return {
      title: String(sec.title || fallbackTitle || "").trim(),
      body: transposeFullSectionBodyForDisplay(
        Array.isArray(sec.body) ? sec.body.join("\n") : "",
        semis
      )
    };
  }).filter(sec => sec.title || String(sec.body || "").trim());
}

function buildFullPreviewText(){
  const out = [];
  const sections = getFullPreviewExportSections();
  if(!sections.length) return "(No lyrics/notes yet - start typing in a section)";

  sections.forEach((sec, idx) => {
    const title = String(sec.title || "").trim();
    const body = String(sec.body || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n").trimEnd();
    if(title) out.push(title);
    if(title && body) out.push("");
...
function buildFullPreviewHtmlDoc(title){
  // ✅ Export from Full Song View text so imported / pasted chord spacing
  // stays where the user put it instead of being rebuilt from card buckets.
  ...
  if(sections.length){
    sections.forEach((sec, idx) => {
      const headingText = String(sec.title || "").trim();
      const bodyText = String(sec.body || "").replace(/\r\n/g, "\n").replace(/\r/g, "\n").trimEnd();

      if(headingText){
        blocks.push(
          `<div style="font-family:Arial,Helvetica,sans-serif;font-weight:900;margin:${idx === 0 ? 0 : 20}px 0 8px;letter-spacing:.3px;">${escapeHtml(headingText)}</div>`
        );
      }

      if(bodyText){
        blocks.push(
          `<pre style="margin:0 0 12px 0;font-family:'Courier New',Courier,monospace;font-size:16px;line-height:1.35;font-weight:400;color:#111;white-space:pre-wrap;">${escapeHtml(bodyText)}</pre>`
        );
      }
    });
  }
