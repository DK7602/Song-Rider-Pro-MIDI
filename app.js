...
const COMMON_SONGWORDS = new Set([
  "again","band","can","friend","hand","land","man","plan","ran","sand","stand","than","understand","when"
]);
...
const OBSCURE_RHYME_WORDS = new Set([
  "abye","anigh","briar","brier","clime","deadpan","direful","fen","glade","japan","lisle","mire","nigh","oer","pyre","quire","sere","thee","thine","thy","trepan","turbofan","twyer","ware","whilom","yon","yore"
]);
const DISCOURAGED_RHYME_WORDS = new Set([
  "began","deadpan","japan","lifespan","trepan","turbofan","underplan","overplan","counterplan"
]);
...
function scoreLyricFriendliness(candidate, seedWord){
  ...
  score += Math.min(32, rawScore / 40);

  if(LYRIC_FRIENDLY_WORDS.has(word)) score += 120;
  if(COMMON_SONGWORDS.has(word)) score += 140;
  if(/^(love|heart|night|light|fire|rain|home|alone|time|grace|faith|higher|desire|pain|dream|stay|gone|strong|song|soul|man|can|ran|hand|land|plan|stand)$/i.test(word)) score += 55;
  ...
  if(OBSCURE_RHYME_WORDS.has(word)) score -= 170;
  if(DISCOURAGED_RHYME_WORDS.has(word)) score -= 260;
  ...
  if(/(?:anti|auto|counter|electro|hyper|micro|multi|neuro|over|psycho|retro|super|tele|trans|turbo|ultra|under)/i.test(word) && !COMMON_SONGWORDS.has(word)) score -= 42;
