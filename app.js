...
ta.value = state.project.fullText || "";
  ...
  function positionOverlayPills(){
    ...
    for(let i=0;i<breaks.length;i++){
      ...
      row.style.top = `${Math.max(0, y - 4)}px`;
      ...
      // + / × action row at end of this section (inside Full window)
      const endLine = (i + 1 < breaks.length) ? breaks[i + 1].lineIndex : parsed.lines.length;
      let yBtn = padT + (endLine * lineH) - scrollTop - (lineH * 0.9);
      yBtn = Math.max(y + (lineH * 1.4), yBtn);

      if(yBtn >= -80 && yBtn <= viewH + 80){
        const act = document.createElement("div");
        act.className = "breakActions";
        act.style.top = `${Math.max(0, yBtn)}px`;
