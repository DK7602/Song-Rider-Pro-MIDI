    ...
    if(ft.trim() && !hasAnyCards){
      ...
      setTimeout(()=>{
        try{
          ...
          // Rebuild canonical Full text from the parsed sections so Full view matches cards
          // after the initial seed as well.
          syncFullTextFromSections();
...
async function importTextToFull(text){
  ...
  editProject("importText", () => {
    ...
    applyFullTextToProjectSections(state.project.fullText || "");
    // Canonicalize Full view from parsed card data so wrapped / inline trailing chords
    // do not linger only in Full view after import.
    syncFullTextFromSections();
    cleanupDeletedBaseSections();
  });
