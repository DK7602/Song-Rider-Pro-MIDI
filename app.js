...
function safeRecordingTitle(rec){
  return String((rec && rec.title) || "").trim() || "Audio";
}
...
function normalizeCloudAudioMeta(meta){
  ...
  return {
    id: String(m.id || uuid()),
    projectId: String(m.projectId || state.project?.id || ""),
...
function getCurrentProjectAudioFiles(project){
  return getProjectAudioFiles(project || state.project);
}

function setProjectAudioFiles(list){
  if(!state.project) return;
  state.project.audioFiles = Array.isArray(list) ? list.map(normalizeCloudAudioMeta) : [];
}
...
async function upsertProjectAudioMeta(meta){
  if(!state.project || !meta || !meta.id) return;
  const next = getProjectAudioFiles();
  const i = next.findIndex(x => String(x.id) === String(meta.id));
  const row = normalizeCloudAudioMeta(meta);
  if(i >= 0) next[i] = { ...next[i], ...row };
  else next.unshift(row);
  editProject("audioMeta", () => {
    setProjectAudioFiles(next.sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0)));
  });
  ...
  try{ renderRecordings(); }catch(err){ console.warn("renderRecordings failed after audio meta update:", err); }
}

async function removeProjectAudioMeta(id){
  if(!state.project || !id) return;
  editProject("audioDeleteMeta", () => {
    setProjectAudioFiles(getProjectAudioFiles().filter(x => String(x.id) !== String(id)));
  });
  renderRecordings();
}

async function uploadRecordingBlobToCloud(item){
  if(!cloud.client || !getAuthUserId() || !item?.blob || !state.project?.id) return null;
  ...
  const fileOpts = {
    cacheControl: "3600",
    upsert: true,
    contentType: String(item.blob.type || "") || undefined
  };
  ...
  try{
    if(item && item.id){
      await dbPut({ ...item, id: recId, cloudPath: path, cloudUrl: publicUrl, mimeType: meta.mimeType, fileSize: meta.fileSize, offsetSec: meta.offsetSec, title: meta.title, projectId });
    }
  }catch(err){
    console.warn("Local recording update after cloud upload failed:", err);
  }
  await upsertProjectAudioMeta(meta);
  return meta;
}

async function deleteRecordingFromCloud(rec){
  const meta = rec?.cloudPath ? rec : findProjectAudioMetaById(rec?.id || rec);
  if(!cloud.client || !getAuthUserId() || !meta?.cloudPath) return;
  try{
    const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).remove([meta.cloudPath]);
    if(error) throw error;
  }catch(err){
    console.warn("Cloud audio delete failed:", err);
  }
}
...
async function renameRecordingEverywhere(rec, name){
  ...
  rec.title = nextTitle;
  if(rec.blob || rec.projectId){
    try{ await dbPut(rec); }catch{}
  }
  const meta = findProjectAudioMetaById(rec.id) || rec;
  if(meta){
    await upsertProjectAudioMeta({ ...meta, title: nextTitle });
  }
}
...
async function updateRecordingOffsetEverywhere(recId, offsetSec){
  ...
  if(meta){
    await upsertProjectAudioMeta({ ...meta, offsetSec: t });
  }
}
...
applyTick();
  ...
  // save into local + cloud project metadata
  await updateRecordingOffsetEverywhere(state.audioSyncRecId, t);
  renderRecordings();
}
...
async function uploadAudioFile(){
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      ...
      await dbPut(item);
      await ensureLocalRecCloudBackfill(item);
...
stop.addEventListener("click", () => {
  ...
  if(rec.blob){
    ...
    if(!ext) ext = guessAudioExtFromMimeOrName(type, ".audio");
  ...
  if(rec.cloudUrl){
    const a = document.createElement("a");
    a.href = rec.cloudUrl;
    a.download = safeBase + guessAudioExtFromMimeOrName(rec.mimeType || rec.cloudUrl, ".audio");
    a.target = "_blank";
    document.body.appendChild(a);
    a.click();
    a.remove();
    ...
    del.addEventListener("click", async () => {
      ...
      await deleteRecordingFromCloud(rec);
      await removeProjectAudioMeta(rec.id);
      renderRecordings();
    });
...
async function startAudioSyncFromRec(rec){
  ...
  state.audioSyncUrl = rec.blob ? url : null;

  const audio = new Audio(url);
  audio.preload = "auto";
  audio.playsInline = true;
  audio.crossOrigin = rec.cloudUrl ? "anonymous" : "";
  audio.muted = false;
  audio.volume = 1;
