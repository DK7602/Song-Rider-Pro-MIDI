...
function getFullPerfScrollHost(){
  return document.getElementById("fullPerfScroll");
}
...
function findFirstVisibleFullPerfLineIndex(){
  ...
  // ✅ BSP-style: pick the TOP-most visible line (below the top of the scroll viewport),
  // not the "closest to center" line. This prevents skipping line 1 and prevents
  // jumping back to line 2 when engaging AutoScroll.
  const viewTop = hr.top + 8;
  const viewBottom = hr.bottom - 8;
  ...
  for(let i=0;i<items.length;i++){
    ...
    // Prefer the row whose top is at/just below viewTop.
    const score = (r.top >= viewTop - 6) ? (r.top - viewTop) : (viewTop - r.top) + 1000;
    if(score < bestScore){
      bestScore = score;
      best = i;
    }
...
function scrollFullPerfRowIntoView(rowEl){
  ...
  try{
    ...
    const hr = host.getBoundingClientRect();
    const targetTop = (rowEl.offsetTop - (host.clientHeight * 0.28));
    state.fullPerfProgramScrollUntil = Date.now() + 500;
...
function applyFullPerfTick(){
  ...
  // Use quarter-note beats (4 per bar) derived from uiTick8 (8 per bar)
  const t8 = uiTick8();
  const nIdx = ((t8 % 8) + 8) % 8;
  const beat4 = Math.floor(nIdx / 2); // 0..3
  ...
  if(box){
    ...
    // restart flash
    box.classList.remove("tickFlash");
    void box.offsetWidth;
    box.classList.add("tickFlash");
    try{
      clearTimeout(state._fullPerfFlashTimer);
      state._fullPerfFlashTimer = setTimeout(()=>{ try{ box.classList.remove("tickFlash"); }catch{} }, 160);
    }catch{}
  ...
  if(state.autoScrollOn){
    ...
    if(state.currentSection === "Full"){
      ...
      const prevScrollHost =
        document.getElementById("fullPerfScroll") ||
        el.sheetInner ||
        el.sheetBody ||
