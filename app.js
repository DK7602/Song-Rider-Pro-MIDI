...
function cleanupNotesEditorArtifacts(){
  if(!el?.notesEditor) return;
  try{
    const walker = document.createTreeWalker(el.notesEditor, NodeFilter.SHOW_TEXT);
    const nodes = [];
    while(walker.nextNode()) nodes.push(walker.currentNode);
    nodes.forEach((node) => {
      if((node.nodeValue || '').includes('​')) node.nodeValue = (node.nodeValue || '').replace(/​/g, '');
    });
    el.notesEditor.querySelectorAll('span').forEach((span) => {
      if(!span.textContent && !span.children.length) span.remove();
    });
  }catch{}
}
...
function restoreNotesSelection(){
  try{
    ...
    if(state.notesLastRange){
      try{
        const range = state.notesLastRange.cloneRange ? state.notesLastRange.cloneRange() : state.notesLastRange;
        let focusNode = range.commonAncestorContainer;
        if(focusNode && focusNode.nodeType === Node.TEXT_NODE) focusNode = focusNode.parentNode;
        el.notesEditor.focus({ preventScroll:true });
        if(focusNode?.focus) {
          try{ focusNode.focus({ preventScroll:true }); }catch{}
        }
        sel.addRange(range);
        return true;
      }catch{}
...
function setCaretInsideNotesNode(node, toEnd=true){
  try{
    ...
    if(node.nodeType === Node.TEXT_NODE){
      ...
      range.setStart(node, toEnd ? len : 0);
    }else{
      range.selectNodeContents(node);
...
function findNotesChecklistItem(node){
  let cur = node;
  while(cur && cur !== el?.notesEditor){
    if(cur.nodeType === 1 && cur.classList?.contains('notesChecklistItem')) return cur;
    cur = cur.parentNode;
  }
  return null;
}

function makeNotesChecklistItem(){
  const item = document.createElement('div');
  item.className = 'notesChecklistItem';
  item.setAttribute('contenteditable', 'false');

  const cb = document.createElement('input');
  cb.type = 'checkbox';
  cb.className = 'notesCheckbox';
  cb.setAttribute('contenteditable', 'false');
  cb.setAttribute('tabindex', '-1');

  const text = document.createElement('span');
  text.className = 'notesChecklistText';
...
function getActiveNotesChecklistText(){
  try{
    const sel = window.getSelection?.();
    let node = (sel && sel.rangeCount) ? sel.getRangeAt(0).commonAncestorContainer : null;
    if(node && node.nodeType === Node.TEXT_NODE) node = node.parentNode;
    while(node && node !== el?.notesEditor){
      if(node.nodeType === 1 && node.classList?.contains('notesChecklistText')) return node;
...
function getClosestNotesBlock(node){
  let cur = node;
  if(cur && cur.nodeType === Node.TEXT_NODE) cur = cur.parentNode;
  while(cur && cur !== el?.notesEditor){
    if(cur.parentNode === el?.notesEditor) return cur;
    cur = cur.parentNode;
  }
  return null;
}

function insertNotesBlockAfter(refNode, newNode){
  if(!el?.notesEditor || !newNode) return;
  if(refNode && refNode.parentNode === el.notesEditor){
    el.notesEditor.insertBefore(newNode, refNode.nextSibling);
  }else{
    el.notesEditor.appendChild(newNode);
  }
}

function focusChecklistText(item, toEnd=false){
  const text = item?.querySelector?.('.notesChecklistText');
  if(text) setCaretInsideNotesNode(text, toEnd);
}


function setCaretInListItem(li, toEnd=false){
  if(!li) return false;
  if(!li.firstChild) li.appendChild(document.createTextNode(''));
  return setCaretInsideNotesNode(li, toEnd);
}

function insertChecklistItemAfter(existingItem){
  if(!el.notesEditor) return null;
  const made = makeNotesChecklistItem();
  if(existingItem && existingItem.parentNode){
    existingItem.parentNode.insertBefore(made.item, existingItem.nextSibling);
  }else{
    el.notesEditor.appendChild(made.item);
  }
  normalizeNotesChecklistState();
  focusChecklistText(made.item, false);
  cacheNotesSelection();
  saveNotesDoc();
  pushNotesHistorySnapshot();
  return made.item;
...
function normalizeNotesChecklistState(root=el.notesEditor){
  if(!root) return;
  root.querySelectorAll('.notesChecklistItem').forEach((item) => {
    const cb = item.querySelector('.notesCheckbox');
    const text = item.querySelector('.notesChecklistText');
    ...
    const indent = Math.max(0, parseInt(item.dataset.indent || '0', 10) || 0);
    item.style.marginLeft = `${indent * 24}px`;
    if(text){
      text.setAttribute('contenteditable', 'true');
...
function insertNotesChecklistItem(){
  ...
  try{
    ...
    const existingItem = findNotesChecklistItem(range?.commonAncestorContainer || null);
    if(existingItem){
      insertChecklistItemAfter(existingItem);
      return;
    }
    if(range){
      range.collapse(false);
  ...
  if(!inserted) el.notesEditor.appendChild(item);

  normalizeNotesChecklistState();
  focusChecklistText(item, false);
  cacheNotesSelection();
  saveNotesDoc();
  pushNotesHistorySnapshot();
}
...
function applyNotesColor(color){
  ...
  try{
    if(range && !range.collapsed){
      ...
      setCaretInsideNotesNode(span, true);
    }else if(range){
      const span = document.createElement('span');
      span.style.color = use;
      ...
      newRange.setStart(zwsp, zwsp.nodeValue ? zwsp.nodeValue.length : 0);
      newRange.collapse(true);
      if(sel){
        sel.removeAllRanges();
        sel.addRange(newRange);
      }
      state.notesLastRange = newRange.cloneRange ? newRange.cloneRange() : newRange;
    }else{
      const span = document.createElement('span');
      span.style.color = use;
      span.appendChild(document.createTextNode('\u200B'));
      el.notesEditor.appendChild(span);
      setCaretInsideNotesNode(span.firstChild || span, true);
    }
  }catch{
    try{
      document.execCommand('styleWithCSS', false, true);
...
function getNotesSelectionRange(){
  try{
    ...
    if(sel && sel.rangeCount){
      const range = sel.getRangeAt(0);
      if(el?.notesEditor?.contains(range.commonAncestorContainer)) return range;
    }
  }catch{}
  if(state?.notesLastRange){
    try{ return state.notesLastRange.cloneRange ? state.notesLastRange.cloneRange() : state.notesLastRange; }catch{}
  }
  return null;
}

function getNotesSelectionAnchorElement(){
  const range = getNotesSelectionRange();
  let node = range?.commonAncestorContainer || document.activeElement || el?.notesEditor;
  if(node && node.nodeType === Node.TEXT_NODE) node = node.parentNode;
  if(!node) return el?.notesEditor || null;
  if(el?.notesEditor?.contains(node)) return node;
  return el?.notesEditor || node;
}


function getNotesInlineState(){
  const stateOut = { bold:false, italic:false, underline:false, strikeThrough:false };

  try{
    if(document.queryCommandState){
      try{ stateOut.bold = !!document.queryCommandState('bold'); }catch{}
      try{ stateOut.italic = !!document.queryCommandState('italic'); }catch{}
      try{ stateOut.underline = !!document.queryCommandState('underline'); }catch{}
      try{ stateOut.strikeThrough = !!document.queryCommandState('strikeThrough'); }catch{}
    }
  }catch{}

  let node = getNotesSelectionAnchorElement();
  while(node && node !== el?.notesEditor){
    if(node.nodeType === Node.TEXT_NODE) node = node.parentNode;
    if(!node || node.nodeType !== 1) break;

    const tag = String(node.tagName || '').toUpperCase();
    if(tag === 'B' || tag === 'STRONG') stateOut.bold = true;
    if(tag === 'I' || tag === 'EM') stateOut.italic = true;
    if(tag === 'U') stateOut.underline = true;
    if(tag === 'S' || tag === 'STRIKE' || tag === 'DEL') stateOut.strikeThrough = true;

    const cs = getComputedStyle(node);
    const deco = String(cs?.textDecorationLine || cs?.textDecoration || '').toLowerCase();
    const fw = String(cs?.fontWeight || '').toLowerCase();
    let fwNum = parseInt(fw, 10);
    if(!Number.isFinite(fwNum)) fwNum = (fw === 'bold') ? 700 : 400;

    if(fwNum >= 600) stateOut.bold = true;
    if(String(cs?.fontStyle || '').toLowerCase() === 'italic') stateOut.italic = true;
    if(deco.includes('underline')) stateOut.underline = true;
    if(deco.includes('line-through')) stateOut.strikeThrough = true;
...
function updateNotesToolbarState(){
  if(!el?.notesToolbar) return;
  const st = getNotesInlineState();
...
function buildNotesDecorationForToggle(cmd, enable, baseState){
  let underline = !!baseState.underline;
  let strike = !!baseState.strikeThrough;
  if(cmd === 'underline') underline = !!enable;
  if(cmd === 'strikeThrough') strike = !!enable;
  const parts = [];
...
function unwrapNotesStyleFromSelection(cmd){
  const range = getNotesSelectionRange();
  ...
  while(node && node !== el?.notesEditor){
    if(node.nodeType === 1){
      ...
      if(
        (cmd === 'bold' && fwNum >= 600) ||
        (cmd === 'italic' && String(cs?.fontStyle || '').toLowerCase() === 'italic') ||
        (cmd === 'underline' && deco.includes('underline')) ||
        (cmd === 'strikeThrough' && deco.includes('line-through'))
      ){
        matches.push(node);
      }
    }
...
function applyNotesInlineToggle(cmd){
  if(!el?.notesEditor) return;

  restoreNotesSelection();
  try{ el.notesEditor.focus({ preventScroll:true }); }catch{}
  ...
  const baseState = getNotesInlineState();
  const enable = !baseState[cmd];

  const applyStyleToSpan = (span) => {
    if(!span) return;
    if(cmd === 'bold'){
      span.style.fontWeight = enable ? '700' : '400';
    }else if(cmd === 'italic'){
      span.style.fontStyle = enable ? 'italic' : 'normal';
    }else if(cmd === 'underline' || cmd === 'strikeThrough'){
      const deco = buildNotesDecorationForToggle(cmd, enable, baseState);
      span.style.textDecoration = deco;
      span.style.textDecorationLine = deco;
    }
  };
  ...
  try{
    if(range.collapsed){
      ...
      try{
        const anchorEl = getNotesSelectionAnchorElement();
        const cs = anchorEl ? getComputedStyle(anchorEl) : null;
        const col = String(cs?.color || '').trim();
...
function insertNotesList(listType){
  if(!el?.notesEditor) return false;
  restoreNotesSelection();

  const range = getNotesSelectionRange();
  const activeChecklistText = getActiveNotesChecklistText();
  const activeChecklistItem = activeChecklistText?.closest?.('.notesChecklistItem') || findNotesChecklistItem(range?.commonAncestorContainer || null);
  ...
  try{
    if(activeChecklistItem){
      insertNotesBlockAfter(activeChecklistItem, list);
    }else if(range){
      const block = getClosestNotesBlock(range.commonAncestorContainer);
      if(block){
        insertNotesBlockAfter(block, list);
      }else{
        range.collapse(false);
        range.insertNode(list);
      }
    }else{
      el.notesEditor.appendChild(list);
    }
  }catch{
    el.notesEditor.appendChild(list);
  }

  setCaretInListItem(li, false);
  cacheNotesSelection();
  saveNotesDoc();
  pushNotesHistorySnapshot();
  updateNotesToolbarState();
...
function adjustChecklistIndent(delta){
  const textNode = getActiveNotesChecklistText();
  const item = textNode?.closest?.('.notesChecklistItem');
  if(!item) return false;
  const current = parseInt(item.dataset.indent || '0', 10) || 0;
  const next = Math.max(0, Math.min(8, current + delta));
  item.dataset.indent = String(next);
  item.style.marginLeft = `${next * 24}px`;
  normalizeNotesChecklistState();
  cacheNotesSelection();
  saveNotesDoc();
...
function execNotesCommand(cmd){
  ...
  if(cmd === 'indent'){
    if(adjustChecklistIndent(1)) return;
  }
  if(cmd === 'outdent'){
    if(adjustChecklistIndent(-1)) return;
  }
  ...
  if(cmd === 'bold' || cmd === 'italic' || cmd === 'underline' || cmd === 'strikeThrough'){
    applyNotesInlineToggle(cmd);
    return;
  }

  try{ document.execCommand('styleWithCSS', false, true); }catch{}
  let ok = false;
  try{ ok = !!document.execCommand(cmd, false, null); }catch{}
  if(!ok){
    restoreNotesSelection();
...
if(el.capoInput){
  ...
  if(el.notesEditor){
    ...
    el.notesEditor.addEventListener("change", (e) => {
      const cb = e.target?.closest?.('.notesCheckbox');
      if(!cb) return;
      normalizeNotesChecklistState();
      saveNotesDoc();
      pushNotesHistorySnapshot(true);
      const item = cb.closest('.notesChecklistItem');
      setTimeout(() => focusChecklistText(item, true), 0);
    });
    el.notesEditor.addEventListener("keydown", (e) => {
      const textNode = e.target?.closest?.('.notesChecklistText') || getActiveNotesChecklistText();
      if(!textNode) return;
      const item = textNode.closest('.notesChecklistItem');
      if(!item) return;
      ...
      if(e.key === 'Enter'){
        e.preventDefault();
        insertChecklistItemAfter(item);
        return;
      }

      if(e.key === 'Backspace'){
        const txt = String(textNode.textContent || '').replace(/ /g, ' ').trim();
        const cb = item.querySelector('.notesCheckbox');
        if(!txt && !cb?.checked){
          e.preventDefault();
          ...
          item.remove();
          normalizeNotesChecklistState();
          const target = prev?.querySelector?.('.notesChecklistText') || next?.querySelector?.('.notesChecklistText') || el.notesEditor;
          setCaretInsideNotesNode(target, true);
          saveNotesDoc();
          pushNotesHistorySnapshot(true);
        }
      }
    });
    let lastChecklistToggleTs = 0;
    const toggleChecklistFromPointer = (e) => {
      const cb = e.target?.closest?.('.notesCheckbox');
      if(!cb) return;
      const nowTs = Date.now();
      if(nowTs - lastChecklistToggleTs < 250) return;
      lastChecklistToggleTs = nowTs;
      e.preventDefault();
      e.stopPropagation();
      cb.checked = !cb.checked;
      cb.dispatchEvent(new Event('change', { bubbles:true }));
    };
    el.notesEditor.addEventListener("pointerdown", toggleChecklistFromPointer);
    ...
    ["keyup","mouseup","input","focus","click","touchend"].forEach((evt) => {
      el.notesEditor.addEventListener(evt, () => {
        cacheNotesSelection();
        updateNotesToolbarState();
      }, true);
  ...
  if(el.notesToolbar){
    let lastNotesToolTouchTs = 0;
    const runToolBtn = (btn, e) => {
      try{ e?.preventDefault?.(); e?.stopPropagation?.(); }catch{}
      cacheNotesSelection();
      execNotesCommand(btn.getAttribute("data-cmd"));
    };
    ...
    el.notesToolbar.querySelectorAll("[data-cmd]").forEach(btn => {
      ...
      btn.addEventListener("click", (e) => {
        if(Date.now() - lastNotesToolTouchTs < 700) {
          try{ e.preventDefault(); e.stopPropagation(); }catch{}
          return;
        }
        runToolBtn(btn, e);
  ...
  if(el.notesFontSize){
    el.notesFontSize.addEventListener("mousedown", () => cacheNotesSelection());
    el.notesFontSize.addEventListener("touchstart", () => cacheNotesSelection(), {passive:true});
    el.notesFontSize.addEventListener("change", () => {
      cacheNotesSelection();
      applyNotesFontSize(parseInt(el.notesFontSize.value, 10) || 18);
    });
  }

  if(el.notesTextColor){
    let lastNotesColorApply = "";
    const applyPickedColor = () => {
      const use = el.notesTextColor.value || "#151515";
      if(use === lastNotesColorApply) return;
      lastNotesColorApply = use;
      cacheNotesSelection();
      applyNotesColor(use);
      setTimeout(() => { lastNotesColorApply = ""; }, 50);
    };
    el.notesTextColor.addEventListener("click", () => cacheNotesSelection());
    el.notesTextColor.addEventListener("mousedown", () => cacheNotesSelection());
    el.notesTextColor.addEventListener("touchstart", () => cacheNotesSelection(), {passive:true});
    el.notesTextColor.addEventListener("input", applyPickedColor);
    el.notesTextColor.addEventListener("change", applyPickedColor);
  }
