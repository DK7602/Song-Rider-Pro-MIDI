...
function buildAlignedChordSlotsForMusicView(chordLineRaw, lyricLineRaw){
  const chordLine = String(chordLineRaw || "").replace(/\u00A0/g," ").replace(/\t/g," ");
  const lyricLine = String(lyricLineRaw || "");
  const slots = Array(8).fill("");
  ...
  while(i < chordLine.length){
    ...
    const posMatch = tok.match(/^([1-8])(.+)$/);
    if(posMatch && parseChordToken(posMatch[2])){
      const slot = clamp(parseInt(posMatch[1],10) - 1, 0, 7);
      const chord = String(posMatch[2] || "").trim();
      slots[slot] = slots[slot] ? (slots[slot] + " " + chord).trim() : chord;
      continue;
    }
    if(parseChordToken(tok)){
      toks.push({ tok, col: start });
    }
  }
  ...
  const denom = Math.max(1, chordLine.length - 1);
  const collapsed = !/  +/.test(chordLine) && toks.length >= 2;
  let wordStarts = [];
  if(collapsed){
    const re = /\S+/g;
    let m;
  ...
  toks.forEach((item, idx) => {
    let slot;
    if(collapsed && wordStarts.length){
      const wIdx = (toks.length === 1) ? 0 : Math.round(idx * (wordStarts.length - 1) / Math.max(1, toks.length - 1));
      const col = wordStarts[clamp(wIdx, 0, wordStarts.length - 1)] || 0;
      slot = clamp(Math.round((col / Math.max(1, lyricLine.length - 1)) * 7), 0, 7);
    }else{
      slot = toks.length > 8
        ? clamp(Math.floor(idx * 8 / toks.length), 0, 7)
        : clamp(Math.round((item.col / denom) * 7), 0, 7);
    }
    slots[slot] = slots[slot] ? (slots[slot] + " " + item.tok).trim() : item.tok;
  });

  return slots;
}

function notes8ToMusicChordLine(notes){
  const vals = Array.from({length:8}, (_,i)=> String((notes && notes[i]) ?? "").trim());
  if(!vals.some(Boolean)) return "";
  return vals.map((v, i) => v ? `${i + 1}${v}` : "").filter(Boolean).join("  ");
}

function setMusicChordClipboardText(text){
  const slots = buildAlignedChordSlotsForMusicView(String(text || ""), "");
  chordRowClipboard.notes = slots;
}

function getMusicPagesForRender(){
  ensurePageMeta();
  const visible = (typeof getVisibleSectionPages === "function" ? getVisibleSectionPages() : getAllSectionOrder())
    .filter(sec => sec && sec !== "Full");
  return visible.length ? visible : ["PAGE 1"];
}
...
function renderMusicView(){
  ...
  const pages = getMusicPagesForRender();
  ...
  pages.forEach((sec) => {
    ...
    pill.value = String(state.project?.sectionTitles?.[sec] || "").trim();
    ...
    const lines = ensureSectionArray(sec);
    lines.forEach((line, idx) => {
      ...
      pasteBtn.title = "Paste chord row";
      pasteBtn.addEventListener("click", () => {
        ...
        chordTa.value = notes8ToMusicChordLine(getLineNotes8(line));
      });
      ...
      addBtn.textContent = "+";
      addBtn.title = "Add a new music row below";
      ...
      delBtn.textContent = "×";
      delBtn.title = "Delete this music row";
      delBtn.addEventListener("click", () => {
        if(lines.length <= 1){
          if(!confirm("Clear this row?")) return;
          editProject("musicClearRow", () => {
            lines[0] = newLine();
            syncFullTextFromSections();
          });
        }else{
      ...
      chordTa.placeholder = "Chord row — tap anywhere to type. Long-press to copy, then tap another empty chord row to paste.";
      chordTa.value = notes8ToMusicChordLine(getLineNotes8(line));

      let chordHoldTimer = null;
      let holdStartX = 0;
      let holdStartY = 0;
      let chordCopiedThisTouch = false;
      const clearChordHold = () => {
        if(chordHoldTimer){
      ...
      const armHold = (e) => {
        ...
        holdStartX = Number(pt?.clientX || 0);
        holdStartY = Number(pt?.clientY || 0);
        chordCopiedThisTouch = false;
        chordHoldTimer = setTimeout(() => {
          chordHoldTimer = null;
          const txt = String(chordTa.value || "").trim();
          if(!txt) return;
          setMusicChordClipboardText(txt);
          chordCopiedThisTouch = true;
          try{
            chordTa.classList.add("musicChordCopied");
            setTimeout(() => { try{ chordTa.classList.remove("musicChordCopied"); }catch{} }, 260);
          }catch{}
          forceCollapseMobileKeyboard(chordTa);
        }, 420);
      };

      chordTa.addEventListener("touchstart", armHold, { passive:true });
      chordTa.addEventListener("pointerdown", armHold);
      chordTa.addEventListener("touchmove", (e) => {
        const pt = e.touches && e.touches[0] ? e.touches[0] : null;
        if(!pt || !chordHoldTimer) return;
        if(Math.abs(Number(pt.clientX||0) - holdStartX) > 10 || Math.abs(Number(pt.clientY||0) - holdStartY) > 10){
          clearChordHold();
        }
      }, { passive:true });
      chordTa.addEventListener("pointermove", (e) => {
        if(!chordHoldTimer) return;
        if(Math.abs(Number(e.clientX||0) - holdStartX) > 10 || Math.abs(Number(e.clientY||0) - holdStartY) > 10){
          clearChordHold();
        }
      });
      ["touchend","touchcancel","pointerup","pointercancel","blur"].forEach(evt => {
        chordTa.addEventListener(evt, () => clearChordHold(), { passive:true });
      });

      chordTa.addEventListener("click", () => {
        if(chordCopiedThisTouch){
          chordCopiedThisTouch = false;
          return;
        }
        if(hasChordClipboard() && !String(chordTa.value || "").trim()){
      ...
      chordTa.addEventListener("input", () => {
        ...
        chordTimer = setTimeout(() => {
          editProject("musicChordEdit", () => {
            setLineNotes8(line, buildAlignedChordSlotsForMusicView(chordTa.value, lyricTa.value));
            syncFullTextFromSections();
          });
          updateKeyFromAllNotes();
          refreshDisplayedNoteCells();
      ...
      _attachCaretRemember(lyricTa);
      lyricTa.addEventListener("focus", () => {
        lastLyricsTextarea = lyricTa;
        refreshRhymesFromActive();
      });
      lyricTa.addEventListener("click", () => {
        lastLyricsTextarea = lyricTa;
        refreshRhymesFromActive();
      });
      let lyricTimer = null;
      lyricTa.addEventListener("input", () => {
        ...
        lyricTimer = setTimeout(() => {
          editProject("musicLyricEdit", () => {
            ...
            if(state.autoSplit){
              line.beats = autosplitBeatsFromLyrics(line.lyrics);
            }
            setLineNotes8(line, buildAlignedChordSlotsForMusicView(chordTa.value, lyricTa.value));
            syncFullTextFromSections();
          });
