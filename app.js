...
function _attachCaretRemember(ta){
  ...
  const fn = () => {
    _rememberCaret(ta);
    try{
      if(ta && ta.tagName === "TEXTAREA" && (ta.classList.contains("lyrics") || ta.classList.contains("fullBox") || ta.classList.contains("fullSectionText"))){
        lastLyricsTextarea = ta;
        if(el && el.rhymeDock && el.rhymeDock.style.display === "block") refreshRhymesFromActive();
      }
    }catch{}
  };
  ta.addEventListener("focus", fn, {passive:true});
...
const RHYME_CONTEXT_THEMES = {
  worship: new Set(["faith","grace","mercy","pray","prayer","praise","glory","heaven","spirit","holy","god","jesus","cross","saved","save","redeemed","worship","amen","hope","peace","light","truth","believe"]),
  love: new Set(["love","heart","kiss","touch","hold","baby","darling","lover","mine","yours","forever","together","stay","need","want","feel","close","arms","eyes","body"]),
  hurt: new Set(["hurt","pain","broken","cry","tears","alone","lonely","sorry","empty","lost","gone","cold","dark","shadow","fear","ache","bleed","miss","regret"]),
  uplift: new Set(["rise","alive","strong","free","shine","gold","better","whole","open","higher","victory","hope","peace","glory","fire","light"]),
  night: new Set(["night","tonight","dark","moon","stars","shadow","midnight","dream","sleep","late","city","glow"]),
  weather: new Set(["rain","storm","wind","fire","flame","water","ocean","wave","sky","sun","snow","dust"]),
  road: new Set(["road","run","drive","ride","go","gone","way","walk","moving","miles","highway","fly","fall","rise","drift","home"]),
  home: new Set(["home","house","room","door","bed","rest","safe","stay","warm","belong","family","mother","father","child","inside"]),
  time: new Set(["time","tonight","morning","day","days","night","nights","forever","never","always","again","now","then","yesterday","tomorrow","still"])
};
const RHYME_THEME_HINTS = {
  worship: new Set(["grace","mercy","faith","pray","praise","heaven","spirit","holy","glory","cross","save","saved","believe","truth"]),
  love: new Set(["heart","love","touch","kiss","hold","stay","forever","mine","close","arms","body","eyes"]),
  hurt: new Set(["pain","broken","alone","lonely","cry","gone","sorry","cold","empty","fear","dark","ache","miss"]),
  uplift: new Set(["rise","higher","alive","strong","free","shine","gold","whole","victory","light","fire"]),
  night: new Set(["night","tonight","moon","stars","shadow","dream","midnight","city","glow"]),
  weather: new Set(["rain","storm","wind","fire","flame","water","ocean","wave","sky","sun","snow"]),
  road: new Set(["road","run","drive","ride","go","way","walk","moving","fly","drift","home"]),
  home: new Set(["home","rest","safe","warm","belong","stay","inside","family"]),
  time: new Set(["time","again","forever","never","always","now","still","morning","day","night"])
};


const RHYME_CACHE = new Map();
let rhymeRenderToken = 0;
let rhymeRefreshTimer = 0;
let lastRhymeRequestKey = "";
let lastRenderedRhymeKey = "";

function makeRhymeContextKey(context){
  if(!context) return "";
  const themes = Array.isArray(context.strongThemes) ? context.strongThemes.join("|") : "";
  const tone = context.hasConversationalTone ? "talk" : "plain";
  const syll = Number(context.preferredSyllables) || 0;
  return `${themes}__${tone}__${syll}`;
}
function getRhymeCacheEntry(kind, word){
  const key = `${kind}:${normalizeWord(word)}`;
  const hit = RHYME_CACHE.get(key);
  if(!hit) return null;
  if((Date.now() - hit.ts) > 1000 * 60 * 30){
    RHYME_CACHE.delete(key);
    return null;
  }
  return hit.data;
}
function setRhymeCacheEntry(kind, word, data){
  const key = `${kind}:${normalizeWord(word)}`;
  RHYME_CACHE.set(key, { ts: Date.now(), data: Array.isArray(data) ? data.slice() : [] });
  if(RHYME_CACHE.size > 120){
    const first = RHYME_CACHE.keys().next();
    if(!first.done) RHYME_CACHE.delete(first.value);
  }
}
function clearRhymeRefreshTimer(){
  if(rhymeRefreshTimer){
    try{ clearTimeout(rhymeRefreshTimer); }catch{}
    rhymeRefreshTimer = 0;
  }
}

function normalizeDatamuseWord(raw){
  return String(raw || "").trim().toLowerCase().replace(/^[^a-z0-9']+|[^a-z0-9']+$/gi, "");
}
...
function tokenizeLyricText(text){
  return String(text || "").toLowerCase().replace(/[^a-z0-9'\s]/g, " ").split(/\s+/).map(s => s.trim()).filter(Boolean);
}
function getTextBeforeCursorForTextarea(ta){
  if(!ta) return "";
  if(ta === el.notesEditor || ta.getAttribute?.("contenteditable") === "true" || ta.classList?.contains("notesEditor")){
    const full = getNotesEditorText(ta);
    const pos = clamp(getCaretCharOffsetInEditor(ta), 0, full.length);
    return full.slice(0, pos);
  }
  let pos = (ta.value || "").length;
  if(typeof ta.__rhymeSelStart === "number" && typeof ta.__rhymeSelTs === "number" && (Date.now() - ta.__rhymeSelTs) < 4000) pos = ta.__rhymeSelStart;
  else if(document.activeElement === ta && typeof ta.selectionStart === "number") pos = ta.selectionStart;
  else if(typeof ta.__lastSelStart === "number") pos = ta.__lastSelStart;
  return String(ta.value || "").slice(0, pos);
}
function collectNearbyLyricContext(ta, seedWord){
  const seed = normalizeWord(seedWord);
  const before = getTextBeforeCursorForTextarea(ta);
  const cleanedLines = normalizeLineBreaks(before)
    .split("\n")
    .map(line => String(line || "").replace(/\u00A0/g, " ").trim())
    .filter(line => line && !isSectionHeadingLine(line) && line !== BREAK_LINE && line !== "_" && !looksLikeChordInputLine(line));
  const recentLines = cleanedLines.slice(-4);
  const tokens = tokenizeLyricText(recentLines.join(" \n ")).filter(tok => tok !== seed).slice(-28);

  const themeWeights = new Map();
  for(const tok of tokens){
    for(const [theme, lex] of Object.entries(RHYME_CONTEXT_THEMES)){
      if(lex.has(tok)) themeWeights.set(theme, (themeWeights.get(theme) || 0) + 1);
    }
  }
  const pronouns = new Set(tokens.filter(t => /^(i|me|my|mine|you|your|yours|we|us|our|ours)$/.test(t)));
  const hasConversationalTone = pronouns.size >= 2 || tokens.some(t => /^(baby|darling|mama|mom|lord|god|jesus)$/.test(t));

  let preferredSyllables = 0;
  const recentTail = tokens.filter(t => t.length > 2).slice(-6);
  if(recentTail.length){
    preferredSyllables = Math.round(recentTail.reduce((sum, t) => sum + estimateSyllables(t), 0) / recentTail.length);
  }
  const strongThemes = Array.from(themeWeights.entries()).sort((a,b) => b[1] - a[1]).filter(([,count]) => count > 0).slice(0,3).map(([theme]) => theme);
  return { tokens, themeWeights, strongThemes, preferredSyllables, hasConversationalTone };
}
...
function scoreContextFit(candidate, context){
  const word = normalizeDatamuseWord(candidate && candidate.word);
  if(!word || !context) return 0;
  let score = 0;
  const syll = Number(candidate && candidate.numSyllables) || estimateSyllables(word);
  if(context.hasConversationalTone && /^(stay|gone|know|need|want|hold|close|home|heart|love|grace|faith|light|night|time|again|strong|higher|fire|rain)$/.test(word)) score += 12;
  if(context.preferredSyllables){
    const diff = Math.abs(syll - context.preferredSyllables);
    if(diff === 0) score += 14;
    else if(diff === 1) score += 8;
    else if(diff >= 3) score -= 6;
  }
  for(const [theme, weight] of context.themeWeights.entries()){
    const hints = RHYME_THEME_HINTS[theme];
    if(hints && hints.has(word)) score += Math.min(34, 10 + (weight * 6));
  }
  if(context.strongThemes.includes("hurt") && /^(light|shine|alive|glory|victory)$/.test(word)) score -= 6;
  if(context.strongThemes.includes("worship") && /^(baby|body|kiss|lover)$/.test(word)) score -= 10;
  if(context.strongThemes.includes("love") && /^(glory|holy|mercy|cross)$/.test(word)) score -= 8;
  return score;
}
function rankRhymeCandidates(seedWord, perfectList, nearList, contextInfo){
  const merged = new Map();
  const push = (entry) => {
    if(!entry || !entry.word) return;
    const existing = merged.get(entry.word);
    if(!existing){ merged.set(entry.word, entry); return; }
    if(existing.kind !== "perfect" && entry.kind === "perfect"){ merged.set(entry.word, { ...existing, ...entry, kind: "perfect" }); return; }
  ...
  const all = Array.from(merged.values()).map(entry => {
    const lyricScore = scoreLyricFriendliness(entry, seedWord);
    const contextScore = scoreContextFit(entry, contextInfo);
    return { ...entry, lyricScore, contextScore, rankScore: lyricScore + contextScore };
  }).filter(entry => entry.rankScore > -200);

  const perfect = all.filter(entry => entry.kind === "perfect").sort((a,b) => (b.rankScore - a.rankScore) || (b.contextScore - a.contextScore) || (b.score - a.score) || a.word.localeCompare(b.word)).slice(0, 10);
  const used = new Set(perfect.map(x => x.word));
  const near = all.filter(entry => !used.has(entry.word)).sort((a,b) => (b.rankScore - a.rankScore) || (b.contextScore - a.contextScore) || (b.score - a.score) || a.word.localeCompare(b.word)).slice(0, 12);
  return { perfect, near };
}
async function fetchDatamuseRhymes(word, max = 36){
  const w = normalizeWord(word);
  if(!w) return [];
  const cached = getRhymeCacheEntry("perfect", w);
  ...
  try{
    ...
    if(!res.ok) return [];
    const data = (await res.json() || []).map(x => buildDatamuseWordEntry(x, "perfect")).filter(Boolean).slice(0, max);
    setRhymeCacheEntry("perfect", w, data);
    return data;
  }catch{
    return cached ? cached.slice(0, max) : [];
  }
}
async function fetchDatamuseNearRhymes(word, max = 36){
  const w = normalizeWord(word);
  if(!w) return [];
  const cached = getRhymeCacheEntry("near", w);
  if(cached) return cached.slice(0, max);
  const url = `https://api.datamuse.com/words?rel_nry=${encodeURIComponent(w)}&md=sf&max=${max}`;
  try{
    const res = await fetch(url, { cache: "force-cache" });
    if(!res.ok) return [];
    const data = (await res.json() || []).map(x => buildDatamuseWordEntry(x, "near")).filter(Boolean).slice(0, max);
    setRhymeCacheEntry("near", w, data);
    return data;
  }catch{
    return cached ? cached.slice(0, max) : [];
  }
...
function buildRhymeChip(word, kindLabel){
  ...
  if(kindLabel === "near") b.style.background = "#ffd666";
  ...
  const markStart = (x, y) => {
    ...
    if(state.notesOpen && el.notesEditor) cacheNotesSelection();
  };
  const markMove = (x, y) => {
    if(!Number.isFinite(x) || !Number.isFinite(y)) return;
    if(!Number.isFinite(b.__dragStartX) || !Number.isFinite(b.__dragStartY)) return;
    if(Math.abs(x - b.__dragStartX) > MOVE_PX || Math.abs(y - b.__dragStartY) > MOVE_PX) b.__dragMoved = true;
  };
  ...
  b.addEventListener("pointerdown", (e) => markStart(e.clientX, e.clientY));
  b.addEventListener("pointermove", (e) => markMove(e.clientX, e.clientY));
  b.addEventListener("pointerup", insertFromChip, {passive:false});
  b.addEventListener("pointercancel", () => { b.__dragMoved = true; });
  b.addEventListener("mousedown", (e) => markStart(e.clientX, e.clientY));
  b.addEventListener("touchstart", (e) => { const t = e.touches && e.touches[0]; markStart(t ? t.clientX : NaN, t ? t.clientY : NaN); }, {passive:true});
  b.addEventListener("touchmove", (e) => { const t = e.touches && e.touches[0]; markMove(t ? t.clientX : NaN, t ? t.clientY : NaN); }, {passive:true});
  b.addEventListener("touchend", (e) => { const t = e.changedTouches && e.changedTouches[0]; markMove(t ? t.clientX : NaN, t ? t.clientY : NaN); insertFromChip(e); }, {passive:false});
  b.addEventListener("touchcancel", () => { b.__dragMoved = true; });
...
function appendRhymeGroup(parent, title, items, kindLabel){
  ...
  items.forEach(item => group.appendChild(buildRhymeChip(item.word, kindLabel)));
  parent.appendChild(group);
}

async function renderRhymes(seed, sourceTa = null){
  const ta = sourceTa || getBestRhymeTextarea();
  let word = normalizeWord(seed);
  if(!word && ta){
    const before = getTextBeforeCursorForTextarea(ta);
    word = normalizeWord(getLastWord(before) || getLastWord(ta.value || "") || "");
  }
  const contextInfo = collectNearbyLyricContext(ta, word);
  const contextKey = makeRhymeContextKey(contextInfo);
  const requestKey = `${word}__${contextKey}`;
  const requestToken = ++rhymeRenderToken;
  lastRhymeRequestKey = requestKey;
  ...
  if(!word){
    ...
    empty.textContent = "Tap a lyrics box and type a line.";
    el.rhymeWords.appendChild(empty);
    lastRenderedRhymeKey = "";
    return;
  }

  if(lastRenderedRhymeKey !== requestKey){
    el.rhymeWords.innerHTML = "";
  ...
  if(requestToken !== rhymeRenderToken || lastRhymeRequestKey !== requestKey) return;

  const ranked = rankRhymeCandidates(word, perfectRaw, nearRaw, contextInfo);
  el.rhymeWords.innerHTML = "";
  ...
  if((ranked.perfect.length + ranked.near.length) === 0){
    ...
    el.rhymeWords.appendChild(status);
    lastRenderedRhymeKey = requestKey;
    return;
  }

  if(contextInfo && contextInfo.strongThemes && contextInfo.strongThemes.length){
    const hint = document.createElement("div");
    hint.style.flex = "0 0 100%";
    hint.style.fontSize = "11px";
    hint.style.fontWeight = "900";
    hint.style.color = "#7a7a7a";
    hint.style.marginBottom = "2px";
    hint.textContent = `Context boost: ${contextInfo.strongThemes.map(t => t.charAt(0).toUpperCase() + t.slice(1)).join(" • ")}`;
    el.rhymeWords.appendChild(hint);
  }
...
function getBestRhymeTextarea(){
  ...
  // Strong fallback: use the last tracked lyrics textarea first.
  if(lastLyricsTextarea && document.contains(lastLyricsTextarea)) return lastLyricsTextarea;

  // In Full view, there can be many section textareas. Prefer the visible one with text,
  // recent caret info, or the one most likely being edited.
  const candidates = Array.from(document.querySelectorAll("textarea.lyrics, textarea.fullBox, textarea.fullSectionText"));
  const viewportH = window.innerHeight || document.documentElement.clientHeight || 0;
  const visible = candidates.filter(ta => {
    try{
      const r = ta.getBoundingClientRect();
      return r.width > 0 && r.height > 0 && r.bottom > 0 && r.top < viewportH;
    }catch{
      return false;
    }
  });
  ...
  try{
    const focused = document.querySelector("textarea.lyrics:focus, textarea.fullBox:focus, textarea.fullSectionText:focus");
  ...
  const scoreTa = (ta) => {
    let score = 0;
    try{
      if(typeof ta.__rhymeSelTs === "number") score += Math.min(5000, Date.now() - ta.__rhymeSelTs) * -1 + 5000;
      else if(typeof ta.__lastSelStart === "number") score += 1200;
    }catch{}
    const val = String(ta.value || "");
    if(val.trim()) score += Math.min(900, val.trim().length);
    if(ta.classList.contains("fullSectionText")) score += 80;
    if(ta.classList.contains("fullBox")) score += 70;
    if(ta.classList.contains("lyrics")) score += 60;
    return score;
  };

  pool.sort((x,y) => scoreTa(y) - scoreTa(x));
  return pool[0] || null;
}

function refreshRhymesFromActive(force = false){
  if(el.rhymeDock.style.display !== "block") return;
  const ta = getBestRhymeTextarea();
  let seed = getSeedFromTextarea(ta);
  if(!seed && ta){
    const before = getTextBeforeCursorForTextarea(ta);
    seed = getLastWord(before) || getLastWord(ta.value || "") || "";
  }
  const contextInfo = collectNearbyLyricContext(ta, seed);
  const nextKey = `${normalizeWord(seed)}__${makeRhymeContextKey(contextInfo)}`;
  clearRhymeRefreshTimer();
  if(!force && nextKey && nextKey === lastRenderedRhymeKey) return;
  rhymeRefreshTimer = setTimeout(() => {
    rhymeRefreshTimer = 0;
    renderRhymes(seed, ta);
  }, force ? 0 : 120);
}
