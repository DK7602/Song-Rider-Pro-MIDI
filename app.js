...
async function handleAuthSessionChange(session, reason=""){
  ...
  if(cloud.user){
    ...
    try{
      ...
      setCloudStatus("ok");
      try{ renderRecordings(); }catch{}
      try{ backfillAllLocalRecordingsToCloud(); }catch{}
...
function guessAudioExtFromMimeOrName(mimeOrName="", fallback=".audio"){
  const s = String(mimeOrName || "").toLowerCase();
  if(s.includes("mpeg") || s.endsWith(".mp3")) return ".mp3";
  if(s.includes("wav") || s.endsWith(".wav")) return ".wav";
  if(s.includes("m4a") || s.endsWith(".m4a") || s.includes("mp4")) return ".m4a";
  if(s.includes("ogg") || s.endsWith(".ogg")) return ".ogg";
  if(s.includes("webm") || s.endsWith(".webm")) return ".webm";
  if(s.includes("aac") || s.endsWith(".aac")) return ".aac";
  return fallback;
}

function normalizeCloudAudioMeta(meta){
  const m = (meta && typeof meta === "object") ? meta : {};
  return {
    id: String(m.id || uuid()),
    projectId: String(m.projectId || ""),
    kind: String(m.kind || "upload"),
    createdAt: Number(m.createdAt) || now(),
    title: String(m.title || ""),
    offsetSec: Number(m.offsetSec || 0) || 0,
    mimeType: String(m.mimeType || ""),
    cloudPath: String(m.cloudPath || ""),
    cloudUrl: String(m.cloudUrl || ""),
    fileSize: Number(m.fileSize || 0) || 0
  };
}

function getProjectAudioFiles(project){
  const p = project || state.project;
  return Array.isArray(p?.audioFiles) ? p.audioFiles.map(normalizeCloudAudioMeta) : [];
}

function findProjectAudioMetaById(id, project){
  return getProjectAudioFiles(project).find(x => String(x.id) === String(id)) || null;
}

function upsertAudioMetaInProject(project, meta){
  if(!project || !meta || !meta.id) return project;
  if(!Array.isArray(project.audioFiles)) project.audioFiles = [];
  const row = normalizeCloudAudioMeta(meta);
  const i = project.audioFiles.findIndex(x => String(x?.id) === String(row.id));
  if(i >= 0) project.audioFiles[i] = { ...normalizeCloudAudioMeta(project.audioFiles[i]), ...row };
  else project.audioFiles.unshift(row);
  project.audioFiles = project.audioFiles.map(normalizeCloudAudioMeta).sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0));
  project.updatedAt = now();
  return project;
}

async function upsertProjectAudioMetaForProject(projectId, meta){
  const pid = String(projectId || meta?.projectId || "").trim();
  if(!pid || !meta?.id) return;
  const all = loadAllProjects().map(normalizeProject).filter(Boolean);
  let changed = false;
  for(const project of all){
    if(String(project?.id || "") !== pid) continue;
    upsertAudioMetaInProject(project, { ...meta, projectId: pid });
    changed = true;
    break;
  }
  if(!changed) return;
  saveAllProjects(all);
  if(state.project && String(state.project.id || "") === pid){
    const found = all.find(p => String(p.id||"") === pid);
    if(found) state.project = normalizeProject(deepClone(found));
  }
  try{ if(getAuthUserId() && cloud.client) await syncAllProjectsToCloud("audioMeta"); }catch(err){ console.warn("Audio metadata sync failed:", err); }
  try{ renderRecordings(); }catch(err){ console.warn(err); }
}

async function removeProjectAudioMetaForProject(projectId, id){
  ...
  for(const project of all){
    if(String(project?.id || "") !== pid) continue;
    project.audioFiles = getProjectAudioFiles(project).filter(x => String(x.id) !== String(id));
    project.updatedAt = now();
    changed = true;
    break;
  }
  ...
  try{ if(getAuthUserId() && cloud.client) await syncAllProjectsToCloud("audioDeleteMeta"); }catch(err){ console.warn(err); }
}

function mergeRecordingsForProject(localAll){
  const pid = String(state.project?.id || "");
  const local = (Array.isArray(localAll) ? localAll : [])
    .filter(r => String(r.projectId || "") === pid)
    .map(r => {
      const meta = findProjectAudioMetaById(r.id);
      return {
        ...r,
        title: String((r.title && r.title.trim()) || (meta?.title || "")).trim(),
        offsetSec: Number((r.offsetSec ?? meta?.offsetSec) || 0) || 0,
        cloudPath: String(r.cloudPath || meta?.cloudPath || ""),
        cloudUrl: String(r.cloudUrl || meta?.cloudUrl || ""),
        mimeType: String((r.blob && r.blob.type) || r.mimeType || meta?.mimeType || ""),
        fileSize: Number((r.blob && r.blob.size) || r.fileSize || meta?.fileSize || 0) || 0
      };
    });
  const seen = new Set(local.map(r => String(r.id)));
  const remoteOnly = getProjectAudioFiles()
    .filter(r => String(r.projectId || "") === pid && !seen.has(String(r.id)))
    .map(r => ({ ...r, blob: null }));
  return [...local, ...remoteOnly].sort((a,b)=>(Number(b.createdAt)||0)-(Number(a.createdAt)||0));
}

async function uploadRecordingBlobToCloud(item){
  if(!cloud.client || !getAuthUserId() || !item?.blob || !item?.projectId) return null;
  const ext = guessAudioExtFromMimeOrName((item.blob && item.blob.type) || (item.blob && item.blob.name) || item.title || "", ".audio");
  const userId = getAuthUserId();
  const projectId = String(item.projectId || "");
  const recId = String(item.id || uuid());
  const path = `${userId}/${projectId}/${recId}${ext}`;
  const fileOpts = { cacheControl: "3600", upsert: true, contentType: String(item.blob.type || "") || undefined };
  const { error } = await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).upload(path, item.blob, fileOpts);
  if(error) throw error;
  const pub = cloud.client.storage.from(CLOUD_AUDIO_BUCKET).getPublicUrl(path);
  const publicUrl = String(pub?.data?.publicUrl || "");
  if(!publicUrl) throw new Error("Could not create public audio URL.");
  const meta = normalizeCloudAudioMeta({
    id: recId,
    projectId,
    kind: item.kind || "upload",
    createdAt: Number(item.createdAt) || now(),
    title: String(item.title || "").trim() || "Audio",
    offsetSec: Number(item.offsetSec || 0) || 0,
    mimeType: String(item.blob.type || ""),
    cloudPath: path,
    cloudUrl: publicUrl,
    fileSize: Number(item.blob.size || 0) || 0
  });
  try{ await dbPut({ ...item, id: recId, cloudPath: path, cloudUrl: publicUrl, mimeType: meta.mimeType, fileSize: meta.fileSize, offsetSec: meta.offsetSec, title: meta.title, projectId }); }catch(err){ console.warn(err); }
  await upsertProjectAudioMetaForProject(projectId, meta);
  return meta;
}

async function ensureLocalRecCloudBackfill(rec){
  try{
    if(!rec?.blob || !getAuthUserId() || !cloud.client) return;
    if(rec.cloudUrl) return;
    const meta = findProjectAudioMetaById(rec.id);
    if(meta?.cloudUrl && meta?.cloudPath) return;
    await uploadRecordingBlobToCloud(rec);
    setCloudStatus("ok");
  }catch(err){
    console.warn("Audio cloud backfill failed:", err);
    setCloudStatus("error", err?.message || "Audio upload failed");
  }
}

async function backfillAllLocalRecordingsToCloud(){
  if(!cloud.client || !getAuthUserId()) return;
  let all = [];
  try{ all = await dbGetAll(); }catch{ return; }
  for(const rec of (Array.isArray(all) ? all : [])){
    if(!rec || !rec.blob || rec.cloudUrl) continue;
    await ensureLocalRecCloudBackfill(rec);
  }
}

async function renameRecordingEverywhere(rec, name){
  const nextTitle = String(name || "").trim();
  rec.title = nextTitle;
  try{ await dbPut(rec); }catch{}
  await upsertProjectAudioMetaForProject(rec.projectId || state.project?.id || "", { ...(findProjectAudioMetaById(rec.id) || rec), title: nextTitle, projectId: rec.projectId || state.project?.id || "" });
}

async function updateRecordingOffsetEverywhere(recId, offsetSec){
  const t = Number(offsetSec || 0) || 0;
  try{
    const all = await dbGetAll();
    const rec = all.find(r => String(r.id) === String(recId));
    if(rec){ rec.offsetSec = t; await dbPut(rec); }
  }catch{}
  const meta = findProjectAudioMetaById(recId);
  if(meta) await upsertProjectAudioMetaForProject(meta.projectId || state.project?.id || "", { ...meta, offsetSec: t });
}
...
function normalizeProject(p){
  ...
  if(!Array.isArray(p.audioFiles)) p.audioFiles = [];
...
async function uploadAudioFile(){
  ...
  inp.onchange = async () => {
    ...
    if(isAudio){
      ...
      await dbPut(item);
      try{ await ensureLocalRecCloudBackfill(item); }catch(err){ console.warn("Audio upload sync failed:", err); }
...
async function renderRecordings(){
  let all = [];
  try{ all = await dbGetAll(); }catch(e){ console.warn('Recordings unavailable (IndexedDB blocked or failed):', e); all = []; }
  const mine = mergeRecordingsForProject(all);

  el.recordingsList.innerHTML = "";
  if(getAuthUserId() && cloud.client){
    mine.filter(r => r && r.blob && !r.cloudUrl).forEach(r => { ensureLocalRecCloudBackfill(r); });
  }
  ...
  mine.forEach(rec => {
    ...
    edit.addEventListener("click", async () => {
      ...
      if(name === null) return;
      await renameRecordingEverywhere(rec, name);
      renderRecordings();
    });
    ...
    stop.addEventListener("click", () => { stopAudioSync(); });
    play.addEventListener("click", async () => {
      if(!rec.blob && !rec.cloudUrl) return;
    ...
    download.addEventListener("click", () => {
      if(rec.blob){
        const blob = rec.blob;
        let ext = guessAudioExtFromMimeOrName((blob && blob.type) || (blob && blob.name) || rec.title || "", ".audio");
        const safeBase = (rec.title && rec.title.trim() ? rec.title.trim() : "recording").replace(/[\/:*?"<>|]+/g,"").trim() || "recording";
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
    ...
    del.addEventListener("click", async () => {
      ...
      try{ if(rec.cloudPath && cloud.client) await cloud.client.storage.from(CLOUD_AUDIO_BUCKET).remove([rec.cloudPath]); }catch(err){ console.warn(err); }
      try{ await removeProjectAudioMetaForProject(rec.projectId || state.project?.id || "", rec.id); }catch{}
      renderRecordings();
    });
...
async function startAudioSyncFromRec(rec){
  if(!rec || (!rec.blob && !rec.cloudUrl)) return;
  ...
  const url = rec.blob ? URL.createObjectURL(rec.blob) : String(rec.cloudUrl || "");
...
await dbPut(item);
try{ await ensureLocalRecCloudBackfill(item); }catch(err){ console.warn("Audio recording sync failed:", err); }
await renderRecordings();
