...
if(state.currentSection === "Full"){
  ...
  function quickSaveFullText(){
    // Keep Full text current in memory while typing, but do NOT run the heavy
    // history/save path on each key. On Android that can destabilize the editor
    // and collapse the keyboard.
    try{
      state.project.fullText = blocksToFullText(blocks);
  ...
  blocks.forEach((b, idx) => {
    ...
    bindTap(addBtn, () => {
      withPillActionLock(() => {
        ...
        requestAnimationFrame(() => {
          try{
            const allTextareas = (el.sheetInner || el.sheetBody).querySelectorAll("textarea.fullSectionText");
            const targetTa = allTextareas[idx + 1];
            if(targetTa){
              targetTa.focus({ preventScroll:false });
              const len = String(targetTa.value || "").length;
