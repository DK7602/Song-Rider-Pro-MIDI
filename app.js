...
- stable bottom anchoring for floating R button
- avoids top-based viewport glitches/freezes on Android
***********************/
{
  ...
  const applyRhymeViewportUI = () => {
    ...
    if(rBtn){
      const bottomGap = keyboardOpen ? 12 : 18;
      rBtn.style.transition = "bottom 140ms ease, opacity 120ms ease";
      rBtn.style.top = "auto";
      rBtn.style.transform = "translateY(0)";
      rBtn.style.bottom = `${overlap + bottomGap}px`;
  ...
  if(window.visualViewport){
    window.visualViewport.addEventListener("resize", scheduleRhymeViewportUI);
    window.visualViewport.addEventListener("scroll", scheduleRhymeViewportUI);
  }
