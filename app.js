...
function getNotesDoc(){
  return sanitizeNotesHtml(state?.project?.notesDoc || "");
}

function saveNotesDoc(){
  if(!state.project || !el.notesEditor) return;
  state.project.notesDoc = sanitizeNotesHtml(el.notesEditor.innerHTML);
  upsertProject(state.project);
}
...
if(el.capoInput){
  ...
  if(el.notesUndoBtn) el.notesUndoBtn.addEventListener("click", () => execNotesCommand("undo"));
  if(el.notesRedoBtn) el.notesRedoBtn.addEventListener("click", () => execNotesCommand("redo"));
