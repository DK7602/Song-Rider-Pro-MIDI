...
function parseFullTextToSectionCards(fullText){
  ...
  function lineIsMostlyChords(line){
    ...
    for(const t0 of toks0){
      ...
      if(/^\(?X?\d+\)?$/.test(up)) continue;
      if(/^\d+X$/.test(up)) continue;
  ...
  function buildSlotsFromAlignedChordLine(chordLineRaw, lyricLineRaw){
    ...
    const reWord = /\S+/g;
    let wm;
    while((wm = reWord.exec(lyricLine))){
      lyricWords.push({
        start: wm.index,
        end: wm.index + wm[0].length - 1,
        center: wm.index + ((wm[0].length - 1) / 2)
      });
    }

    const firstTokCol = toks[0].col;
    const lastTokCol = toks[toks.length - 1].col;
    const chordSpan = Math.max(1, lastTokCol - firstTokCol);
    const lyricSpan = Math.max(1, lyricLine.length - 1);

    toks.forEach((item, idx) => {
      let slot;

      if(lyricWords.length){
        const ratio = (item.col - firstTokCol) / chordSpan;
        const projectedCol = clamp(Math.round(ratio * lyricSpan), 0, lyricSpan);

        let best = lyricWords[0];
        let bestDist = Math.abs(projectedCol - best.center);
        for(const w of lyricWords){
          const d = Math.abs(projectedCol - w.center);
          if(d < bestDist){
            best = w;
            bestDist = d;
          }
        ...
        let anchorCol = best.start;
        if(idx === toks.length - 1 && lyricWords.length){
          anchorCol = best.start;
        }else if(idx > 0 && idx < toks.length - 1){
          anchorCol = Math.round(best.center);
        }
        slot = clamp(Math.round((anchorCol / lyricSpan) * 7), 0, 7);
      }else{
        slot = toks.length > 8
          ? clamp(Math.floor(idx * 8 / toks.length), 0, 7)
          : clamp(Math.round(((item.col - firstTokCol) / chordSpan) * 7), 0, 7);
      }

      if(!slots[slot]) slots[slot] = item.tok;
      else {
        const firstEmpty = slots.findIndex(v => !String(v || "").trim());
        if(firstEmpty >= 0) slots[firstEmpty] = item.tok;
  ...
  function appendToLastCard(sec, extraLyrics){
    const add = String(extraLyrics || "").replace(/\s+/g, " ").trim();
    if(!add) return false;
    ensureBucket(sec);
    ...
    last.lyrics = String(last.lyrics || "").trim();
    last.lyrics = last.lyrics ? `${last.lyrics} ${add}`.replace(/\s+/g, " ").trim() : add;
    return true;
  }
  ...
  function flushPendingChordLine(){
  ...
  function handleLyricLine(rawLine){
    const { cleaned, chords: inlineChords } = extractInlineChords(rawLine);
    const lyricText = String(cleaned || "").trim();

    if(pendingChordLine){
      const slots = buildSlotsFromAlignedChordLine(pendingChordLine, lyricText);
      pendingChordLine = null;
      pushCard(cur, lyricText, slots);
      canContinueLyricRun = true;
      return;
    }
    ...
    if(inlineChords.length){
      ...
      const wordStarts = [];
      let wm;
      while((wm = reWord.exec(lyricText))){ wordStarts.push(wm.index); }
      const denom = Math.max(1, lyricText.length - 1);
      const n = inlineChords.length;
      for(let k=0; k<n; k++){
        let slot = 0;
        if(wordStarts.length){
          const wIdx = (n === 1) ? 0 : Math.round(k * (wordStarts.length - 1) / Math.max(1, n - 1));
          const col = wordStarts[clamp(wIdx, 0, wordStarts.length - 1)] || 0;
          slot = clamp(Math.round((col / denom) * 7), 0, 7);
        }else{
          slot = clamp(Math.floor((k / Math.max(1, n - 1)) * 7), 0, 7);
        }
        if(!slots[slot]) slots[slot] = inlineChords[k];
        else {
          const firstEmpty = slots.findIndex(v => !String(v || "").trim());
          if(firstEmpty >= 0) slots[firstEmpty] = inlineChords[k];
    ...
    if(lyricText && canContinueLyricRun && appendToLastCard(cur, lyricText)){
      canContinueLyricRun = true;
      return;
    }

    pushCard(cur, lyricText, Array(8).fill(""));
    canContinueLyricRun = false;
  }

  function processContentLine(rawLine, nextHeadingFn=null){
    const line = String(rawLine ?? "");

    if(line.trim() === ""){
      if(pendingChordLine && lineIsMostlyChords(pendingChordLine)){
        let k = processContentLine._index + 1;
        while(k < lines.length && String(lines[k] || "").trim() === "") k++;
        const nxt = k < lines.length ? String(lines[k] || "") : "";
        const nxtHeading = nextHeadingFn ? nextHeadingFn(nxt) : null;
        if(nxt && !_parseBreakLine(nxt) && !nxtHeading && !lineIsMostlyChords(nxt)){
          return;
        }
      }
    ...
    handleLyricLine(line);
  }

  if(hasBreaks){
    let pageIdx = -1;
    for(let i=0; i<lines.length; i++){
      processContentLine._index = i;
    ...
    for(let i=0; i<lines.length; i++){
      ...
      processContentLine(line, headingTitleFromLine);
    }
    flushPendingChordLine();
  }
...
function applyFullTextToProjectSections(fullText){
  ...
  secsToApply.forEach(sec => {
    ...
    for(let i=0; i<want.length; i++){
      ...
      if(Array.isArray(w.chords)){
        base.notes = Array.from({length:8}, (_,k) => String(w.chords[k] || "").trim());
      }else{
...
function refreshDisplayedNoteCells(){
  ...
  // Keep Full Song View visually in sync with capo/step changes too.
  if(state.currentSection === "Full" && state.project){
    const keepTop = Number(el?.sheetBody?.scrollTop) || 0;
    syncFullTextFromSections();
    renderSheet();
    if(el?.sheetBody) el.sheetBody.scrollTop = keepTop;
  }
}
...
if(state.currentSection === "Full"){
  ...
  let fullCommitDirty = false;

  function isActivelyEditingFullSection(){
    try{
      const a = document.activeElement;
      if(!a) return false;
      if(a.classList && (a.classList.contains("fullSectionText") || a.classList.contains("songPartPill"))) return true;
      return false;
    }catch{
      return false;
    }
  ...
  function commitDebounced(){
    fullCommitDirty = true;
    clearCommitTimer();
    commitTimer = setTimeout(() => {
      commitTimer = null;
      // ✅ Do not run the heavy full commit while the user is actively typing
      // in a Full Song View section. On Android this can collapse the IME after
      // the first keystroke. Keep the live text in memory and wait for blur or
      // another non-typing action to flush the structural sync.
      if(isActivelyEditingFullSection()) return;
      if(!fullCommitDirty) return;
      commitNow();
      refreshRhymesFromActive();
    }, 160);
  }
  ...
  // ✅ Use click for Full-view + / × controls.
  // Android can fire pointerup, then a synthetic click after the DOM re-renders.
  // When that happens, one tap can add a section and then immediately hit a new
  // control in the same spot, making the section appear to flash and disappear.
  ...
  function quickSaveFullText(opts = {}){
    // During active typing on Android, aggressive project persistence can cause
    // the focused textarea to lose its IME/keyboard state. Keep the current
    // fullText in memory immediately, and only do the heavier project save when
    // explicitly requested (blur, add/delete, idle commit).
    try{
      const newText = blocksToFullText(blocks);
      state.project.fullText = newText;
  ...
  blocks.forEach((b, idx) => {
    ...
    ta.addEventListener("input", () => {
      ...
      // Keep in-memory fullText current without doing the heavier save path on every key.
      quickSaveFullText();
      commitDebounced();
    });
    ta.addEventListener("blur", () => {
      // Persist once typing settles / field loses focus.
      quickSaveFullText({ persist:true });
      if(fullCommitDirty) commitNow();
      else commitDebounced();
    });
