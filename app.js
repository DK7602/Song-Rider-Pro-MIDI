...
function pianoNote(ctx, freq, durMs, vel=0.9){
  // Smooth electric piano with stable pitch and warm R&B tone
  const t0 = ctx.currentTime;
  const dur = Math.max(0.12, durMs / 1000);
  const baseFreq = clamp(freq, 40, 2500);
  ...
  o3.connect(g3); g3.connect(sum);

  // Short attack click for key/tine feel
  const nLen = 0.012;
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    data[i] = (Math.random() * 2 - 1) * falloff * 0.35;
  }
  const ns = ctx.createBufferSource();
  ns.buffer = buffer;
  ...
  nf.type = "bandpass";
  nf.frequency.value = Math.min(2200, baseFreq * 3.5);
  nf.Q.value = 0.9;

  const ng = ctx.createGain();
  ng.gain.value = 0.010 * v;

  ns.connect(nf);
  nf.connect(ng);
  ng.connect(sum);

  // Gentle chorus movement on main oscillator only
  ...
  env.gain.linearRampToValueAtTime(peak * 0.82, t0 + 0.11);

  const endTime = t0 + dur;
  env.gain.setValueAtTime(peak * 0.72, endTime);

  const tail = clamp(1.3 + dur * 0.75, 1.3, 5.0);
  ...
  const stopAt = endTime + tail + 0.15;
  o1.stop(stopAt);
  o2.stop(stopAt);
  o3.stop(stopAt);
  lfo.stop(stopAt);
  ...
  scheduleCleanup([o1,o2,o3,g1,g2,g3,sum,ns,nf,ng,lfo,lfoGain,hp,body,lp,env], (durMs + tail*1000 + 1200));
  return { out: env, nodes:[o1,o2,o3,g1,g2,g3,sum,ns,nf,ng,lfo,lfoGain,hp,body,lp,env] };
}
/***********************
...
function playAcousticChord(ch, durMs, fracMul=1){
  ...
  const bpm = clamp(state.bpm||95, 40, 220);
  const rollMs = clamp(Math.round(15_000 / bpm), 6, 16);

  for(let i=0;i<freqs.length;i++){
    const f = freqs[i];
    const delayMs = getHumanizedMs(i * rollMs, 2);
