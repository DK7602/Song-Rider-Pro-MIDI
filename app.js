  ...
  try{
    ...
    if(state.notesLastRange){
      try{
        ...
        if(focusNode?.focus) focusNode.focus({ preventScroll:true });
        else el.notesEditor.focus({ preventScroll:true });
        sel.addRange(range);
        return true;
      }catch{}
    }
...
if(el.capoInput){
  ...
  if(el.notesToolbar){
    ...
    el.notesToolbar.querySelectorAll("[data-cmd]").forEach(btn => {
      ...
      btn.addEventListener("touchend", (e) => runToolBtn(btn, e), {passive:false});
      btn.addEventListener("click", (e) => runToolBtn(btn, e));
    });
  }
  ...
  if(el.notesTextColor){
    ...
    el.notesTextColor.addEventListener("change", () => applyNotesColor(el.notesTextColor.value || "#151515"));
  }
