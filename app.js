...
- brute-force keyboard tracking for Android Chrome
- portals rhyme dock + floating R to body so parent transforms cannot trap them
- uses the actual visible viewport bottom and a live tracker while keyboard animates
***********************/
{
  ...
  function getVisibleBottom(){
    if(vv){
      return Math.round((vv.offsetTop || 0) + (vv.height || 0));
    }
  ...
  function refreshBaselines(){
    ...
    const docH = Math.round((document.documentElement && document.documentElement.clientHeight) || 0);
    baselineVisibleBottom = Math.max(baselineVisibleBottom || 0, visibleBottom || 0);
    baselineInnerHeight = Math.max(baselineInnerHeight || 0, innerH || 0);
    baselineDocHeight = Math.max(baselineDocHeight || 0, docH || 0);
  }
  ...
  function getViewportState(){
    ...
    const shrinkFromInner = Math.max(0, baselineInnerHeight - innerH);
    const shrinkFromDoc = Math.max(0, baselineDocHeight - docH);
    const keyboardInset = Math.max(insetFromVisibleBottom, shrinkFromInner, shrinkFromDoc);
    const keyboardOpen = keyboardInset > KEYBOARD_THRESHOLD;
    const vvHeight = vv ? Math.round(vv.height || 0) : innerH;
    return { visibleBottom, keyboardInset, keyboardOpen, vvHeight };
  }

  function placeNode(node, gapPx, fallbackBottomPx){
    if(!node) return;
    const state = getViewportState();
    forceStyle(node, 'transition', 'top 90ms linear, bottom 90ms linear, opacity 90ms linear');
    forceStyle(node, 'transform', 'none');

    if(!state.keyboardOpen){
      forceStyle(node, 'top', 'auto');
      forceStyle(node, 'bottom', `${fallbackBottomPx}px`);
      return;
    }

    const rect = node.getBoundingClientRect();
    const h = Math.max(40, Math.round(rect.height || node.offsetHeight || 48));
    const top = Math.max(6, Math.round(state.visibleBottom - h - gapPx));
    forceStyle(node, 'bottom', 'auto');
    forceStyle(node, 'top', `${top}px`);
  }
  ...
  function applyRhymeViewportUI(){
    ...
    if(dock){
      placeNode(dock, DOCK_GAP, DEFAULT_DOCK_BOTTOM);
      if(state.keyboardOpen){
        forceStyle(dock, 'max-height', `${Math.max(120, Math.round(state.vvHeight * 0.42))}px`);
      }else{
    ...
    if(rBtn){
      placeNode(rBtn, BUTTON_GAP, DEFAULT_BTN_BOTTOM);
      if(dockShown){
  ...
  function stopPulse(){
    ...
    if(stopPulseTimer){ clearTimeout(stopPulseTimer); stopPulseTimer = 0; }
  }

  function startPulse(ms = PULSE_SETTLE_MS){
    stopPulse();
    queueApply();
    pulseTimer = setInterval(queueApply, PULSE_MS);
    stopPulseTimer = setTimeout(() => {
      stopPulse();
      queueApply();
    }, ms);
  }
  ...
  document.addEventListener('selectionchange', settle, true);
