...
function applyTick(){
  ...
  // ✅ FULL SONG VIEW: flash the BSP-style 4-quadrant highlight in sync with the main beat clock
  // (no independent timer — prevents drift and matches BSP behavior)
  if(state.currentSection === "Full"){
    if(state.fullViewAutoScrollOn){
      try{ fullViewPulseQuadrant(bIdx); }catch{}
    }
    return;
  }
...
function stopFullViewAutoScroll(){
  ...
  state.fullViewAutoScrollLastTs = 0;
  stopFullViewBeatTimer();
  if(state.fullViewAutoScrollRaf){
    cancelAnimationFrame(state.fullViewAutoScrollRaf);
    state.fullViewAutoScrollRaf = null;
  }
  renderFullViewAutoScrollControl();
  removeFullViewAutoHighlight();
  // keep top buttons in sync (important when Full-view button toggles it)
  $("autoPlayBtn")?.classList.remove("on");
  $("mScrollBtn")?.classList.remove("on");
}
...
function fullViewAutoScrollFrame(ts){
  ...
  // ✅ BSP-style: advance EXACTLY 1 text line per BAR (8 eighth-notes)
  // This keeps the yellow highlight aligned to the beat grid.
  try{
    const t8 = uiTick8();
    const bars = Math.max(0, (t8 - (state.fullViewBaseTick8||0)) / 8);
    const targetTop = (state.fullViewBaseScrollTop||0) + bars * (state.fullViewLineH || 24);
    sb.scrollTop = targetTop;
  }catch{}

  updateFullViewAutoHighlight();

  // Stop at bottom
  if(sb.scrollTop + sb.clientHeight >= sb.scrollHeight - 2){
    stopFullViewAutoScroll();
    return;
  }

  state.fullViewAutoScrollRaf = requestAnimationFrame(fullViewAutoScrollFrame);
}
...
function startFullViewAutoScroll(){
  ...
  // ✅ Rebase to the current beat clock so Full auto-scroll matches BSP (1 line per BAR)
  try{
    state.fullViewBaseTick8 = uiTick8();
    state.fullViewBaseScrollTop = (el.sheetBody ? el.sheetBody.scrollTop : 0);
    // cache a reasonable line height for BAR stepping
    const ta = document.querySelector("textarea.fullSectionText, textarea.fullBox");
    if(ta){
      const cs = getComputedStyle(ta);
      const lh = parseFloat(cs.lineHeight);
      if(Number.isFinite(lh) && lh > 0) state.fullViewLineH = lh;
    }
  }catch{}
