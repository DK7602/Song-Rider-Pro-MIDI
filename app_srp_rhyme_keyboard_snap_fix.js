...
***********************/
{
  ...
  const readBaseBottom = (node, fallbackPx) => {
    if(!node) return fallbackPx;
    const raw = parseFloat(getComputedStyle(node).bottom || "");
    return Number.isFinite(raw) ? raw : fallbackPx;
  };

  const baseDockBottom = readBaseBottom(dock, 12);
  const baseRBtnBottom = readBaseBottom(rBtn, 18);
  ...
  const getKeyboardInset = () => {
    ...
    const layoutBottom = Math.round(window.innerHeight || document.documentElement.clientHeight || 0);
    const inset = Math.max(0, layoutBottom - viewportBottom);
    return inset > KEYBOARD_THRESHOLD ? inset : 0;
  };

  const applyKeyboardLift = () => {
    rafId = 0;
    const keyboardInset = getKeyboardInset();
    const keyboardOpen = keyboardInset > 0;
    const lift = keyboardOpen ? (keyboardInset + KEYBOARD_GAP) : 0;
    const dockShown = !!(dock && dock.style.display === "block");

    if(dock){
      dock.style.transform = "translateY(0px)";
      dock.style.bottom = `${baseDockBottom + lift}px`;
      dock.style.transition = "bottom 140ms ease, opacity 120ms ease";
    }

    if(rBtn){
      rBtn.style.transform = "translateY(0px)";
      rBtn.style.bottom = `${baseRBtnBottom + lift}px`;
      rBtn.style.transition = "bottom 140ms ease, opacity 120ms ease";
  ...
  const updateRhymeViewportUI = () => {
    ...
    rafId = requestAnimationFrame(applyKeyboardLift);
  };

  const settleRhymeViewportUI = () => {
    updateRhymeViewportUI();
    setTimeout(updateRhymeViewportUI, 60);
    setTimeout(updateRhymeViewportUI, 180);
    setTimeout(updateRhymeViewportUI, 320);
  };

  if(vv){
    vv.addEventListener("resize", updateRhymeViewportUI);
    vv.addEventListener("scroll", updateRhymeViewportUI);
  }
  ...
  // Re-run after the dock/modal is toggled so the R button visibility/position stays correct.
