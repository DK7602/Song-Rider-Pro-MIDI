...
function keepFullTextareaCaretVisible(ta, opts = {}){
  if(!ta || !ta.classList || !ta.classList.contains("fullSectionText")) return;

  const host = (typeof el !== "undefined" && el && el.sheetBody)
    ? el.sheetBody
    : (document.getElementById("sheetBody") || document.scrollingElement);
  if(!host) return;

  const allowUp = opts.allowUp !== false;
  const mode = opts.mode || "caret";
  ...
  const run = () => {
    ...
    const safeBottom = viewportBottom - 22;

    const rect = ta.getBoundingClientRect();

    // While typing on Android, wrapped lines can reflow after a space.
    // Newline-count caret math can then overshoot and cause the page to jump.
    // For live input, keep the textarea bottom above the keyboard instead.
    if(mode === "input"){
      const targetBottom = rect.bottom + 10;
      if(targetBottom > safeBottom){
        const delta = Math.ceil(targetBottom - safeBottom + 12);
    ...
    const lineHeight = parseFloat(cs.lineHeight) || Math.max(24, Math.round((parseFloat(cs.fontSize) || 18) * 1.45));
    const padTop = parseFloat(cs.paddingTop) || 0;

    let sel = 0;
    try{
      sel = (typeof ta.selectionStart === "number") ? ta.selectionStart : ta.value.length;
    }catch{
      sel = ta.value.length;
    }

    const before = String(ta.value || "").slice(0, sel);
    const lineIndex = (before.match(/\n/g) || []).length;
    const caretTop = rect.top + padTop + (lineIndex * lineHeight);
    const caretBottom = caretTop + lineHeight;

    if(caretBottom > safeBottom){
      const delta = Math.ceil(caretBottom - safeBottom + 14);
      host.scrollTop += delta;
    }else if(allowUp && caretTop < safeTop){
      const delta = Math.ceil(safeTop - caretTop + 10);
...
if(state.currentSection === "Full"){
  ...
  blocks.forEach((b, idx) => {
    ...
    ta.addEventListener("focus", () => {
      ...
      refreshRhymesFromActive();
      keepFullTextareaCaretVisible(ta);
    });
    ...
    ta.addEventListener("input", () => {
      ...
      quickSaveFullText();
      commitDebounced();
      keepFullTextareaCaretVisible(ta, { allowUp:false, mode:"input" });
    });
