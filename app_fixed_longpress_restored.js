...
function injectChordCopyModeStyle(){
  ...
  style.textContent = `
    body.chordCopyMode .noteCell{ transition: box-shadow .14s ease, filter .14s ease; }
    body.chordCopyMode .noteCell.chordCopyTarget{ box-shadow:0 0 0 2px rgba(37,99,235,.22),0 0 0 5px rgba(37,99,235,.10); }
    body.chordCopyMode .noteCell.chordCopySource{ box-shadow:0 0 0 2px rgba(22,163,74,.28),0 0 0 6px rgba(22,163,74,.14); filter:brightness(1.04); }
    body.chordCopyMode .card.chordCopySourceCard{ box-shadow:0 0 0 3px rgba(22,163,74,.16); border-radius:18px; }
  `;
  document.head.appendChild(style);
}
...
function clearChordCopyClasses(){
  try{
    ...
    document.querySelectorAll('.noteCell.chordCopyTarget, .noteCell.chordCopySource').forEach(elm => elm.classList.remove('chordCopyTarget','chordCopySource'));
    document.querySelectorAll('.card.chordCopySourceCard').forEach(elm => elm.classList.remove('chordCopySourceCard'));
  }catch{}
}
...
function cancelChordDragCopy(){
  ...
  try{ document.querySelectorAll('.noteCell[data-pending-tap-focus="1"]').forEach(elm => { delete elm.dataset.pendingTapFocus; try{ elm.readOnly = false; }catch{}; }); }catch{}
...
function activateChordDragCopy(sourceInput, line, card){
  ...
  try{
    if(sourceInput){
      ...
      if(typeof sourceInput.blur === 'function') sourceInput.blur();
      delete sourceInput.dataset.pendingTapFocus;
    }
    document.body.classList.add('chordCopyMode');
    document.querySelectorAll('.noteCell').forEach(elm => elm.classList.add('chordCopyTarget'));
...
function wireChordDragCopyGlobalCancel(){
  ...
  document.addEventListener('pointerup', () => { clearChordCopyTimer(); }, true);
  document.addEventListener('pointercancel', () => { clearChordCopyTimer(); }, true);

  const cancelOnScroll = () => { clearChordCopyTimer(); };
...
card.appendChild(top);
    ...
    for(let i=0;i<8;i++){
      ...
      inp.addEventListener("pointerdown", (e) => {
        ...
        if((e.pointerType === "mouse" && e.button !== 0) || inp.disabled) return;

        // Let a normal tap focus naturally so the keyboard opens.
        // A long press will blur/collapse the keyboard when copy mode activates.
